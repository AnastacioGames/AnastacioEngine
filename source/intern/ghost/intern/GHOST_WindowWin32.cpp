...
static HICON load_icon_from_path(const wchar_t *path)
{
...
HICON GHOST_WindowWin32::loadExternalAppIcon()
{
	...
	const wchar_t *icon_names[] = {
	    L"winrange.ico",
	    L"RangeEngine.ico",
	    L"RangeRuntime.ico",
	};

	for (int i = 0; i < (int)(sizeof(icon_names) / sizeof(icon_names[0])); i++) {
		wchar_t icon_path[MAX_PATH] = {0};
		wcsncpy(icon_path, exe_path, MAX_PATH - 1);
		wcsncat(icon_path, icon_names[i], MAX_PATH - wcslen(icon_path) - 1);

		HICON icon = load_icon_from_path(icon_path);
		if (icon) {
			return icon;
		}
	}
...
void GHOST_WindowWin32::applyWindowIcon()
{
	if (!m_hWnd) {
		return;
	}
	...
	if (m_appIconSmall) {
		::SendMessage(m_hWnd, WM_SETICON, ICON_SMALL, (LPARAM)m_appIconSmall);
	}
}
...
void GHOST_WindowWin32::ThemeRefresh()
{
	HKEY hKey;
	DWORD lightMode;
	DWORD dataSize = sizeof(lightMode);
	...
	result = RegQueryValueExW(hKey, 
							  L"AppsUseLightTheme", 
							  NULL, NULL, reinterpret_cast<LPBYTE>(&lightMode), &dataSize);
