...
{
	GHOST_TSuccess success = GHOST_kFailure;
	GHOST_ASSERT(m_displayManager, "GHOST_System::updateFullScreen(): invalid display manager");
	if (window->getState() == GHOST_kWindowStateFullScreen) {
		success = m_displayManager->setCurrentDisplaySetting(GHOST_DisplayManager::kMainDisplay, setting);
	}
...
void GHOST_System::setFullScreen(bool enable, GHOST_IWindow *window)
{
	window->setState(enable ? GHOST_kWindowStateFullScreen : GHOST_kWindowStateNormal);
}
