    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            if tex:
                ...
                layout.label(text="Empty", icon_value=icon)
...
class TEXTURE_PT_colors(TextureButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Ramp:", icon="COLOR")
        box.prop(tex, "use_color_ramp", text="Ramp")
        if tex.use_color_ramp:
            box.template_color_ramp(tex, "color_ramp", expand=True)

        box = layout.box()
        box.label(text="Colors:", icon="RESTRICT_COLOR_ON")
        split = box.split()
...
class TEXTURE_PT_clouds(TextureTypePanel, Panel):
    ...
    def draw(self, context):
        ...
        box.row().prop(tex, "noise_type", text="Type", expand=True)
        box.prop(tex, "noise_basis", text="Basis")
...
class TEXTURE_PT_blend(TextureTypePanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Progression:", icon="MOD_WAVE")
        box.prop(tex, "progression")
...
class TEXTURE_PT_image(TextureTypePanel, Panel):
    ...
    def draw(self, context):
        ...
        box.template_image(tex, "image", tex.image_user)
...
class TEXTURE_PT_image_sampling(TextureTypePanel, Panel):
    ...
    def draw_bi(self, context):
        ...
        box = layout.box()
        box.label(text="Sampling:", icon="FILTER")
        split = box.split()
    ...
    def draw_bge(self, context):
        ...
        col = split.column()
        col.prop(tex, "use_alpha", text="Read Alpha")
        col.prop(tex, "use_calculate_alpha", text="Calculate Alpha")
        col.prop(tex, "invert_alpha", text="Invert Alpha")
...
class TEXTURE_PT_image_mapping(TextureTypePanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Extension:", icon="MOD_UVPROJECT")
        box.prop(tex, "extension")
        ...
        box = layout.box()
        box.label(text="Crop:", icon="MESH_GRID")
        split = box.split()
    ...
    def draw(self, context):
        ...
        if env.source == 'IMAGE_FILE':
            ...
            if env.source == 'REALTIME':
                box.template_ID(tex, "image", new="image.new", open="image.open")
                box.template_image(tex, "image", tex.image_user, compact=True, cubemap=(env.mapping == 'CUBE'))
                box.prop(env, "filtering")
                if env.mapping == 'PLANE':
                    box.prop(env, "mode")
            box.prop(env, "mapping")

            if env.source != 'REALTIME' and env.mapping == 'PLANE':
                box.prop(env, "zoom")

            box.prop(env, "viewpoint_object")
...
class TEXTURE_PT_musgrave(TextureTypePanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Musgrave:", icon="RNDCURVE")
...
class TEXTURE_PT_voronoi(TextureTypePanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Voronoi:", icon="RNDCURVE")
        split = box.split()
...
class TEXTURE_PT_distortednoise(TextureTypePanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Distortion:", icon="FORCE_TURBULENCE")
        box.prop(tex, "noise_distortion")
        box.prop(tex, "noise_basis", text="Basis")
...
class TEXTURE_PT_voxeldata(TextureButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Source:", icon="MOD_FLUIDSIM")
        box.prop(vd, "file_format")
        if vd.file_format in {'BLENDER_VOXEL', 'RAW_8BIT'}:
            box.prop(vd, "filepath")
        if vd.file_format == 'RAW_8BIT':
            box.prop(vd, "resolution")
        elif vd.file_format == 'SMOKE':
            box.prop(vd, "domain_object")
            box.prop(vd, "smoke_data_type")
        elif vd.file_format == 'HAIR':
            box.prop(vd, "domain_object")
            box.prop(vd, "hair_data_type")
        ...
        box = layout.box()
        box.label(text="Sampling:", icon="FILTER")
        box.prop(vd, "interpolation")
        box.prop(vd, "extension")
        box.prop(vd, "intensity")
...
class TEXTURE_PT_pointdensity(TextureButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Source:", icon="PARTICLES")
        split = box.split()
        ...
        if pd.point_source == 'PARTICLE_SYSTEM':
            ...
            if pd.particle_color_source in {'PARTICLE_SPEED', 'PARTICLE_AGE'}:
                box.template_color_ramp(pd, "color_ramp", expand=True)
        else:
...
class TEXTURE_PT_pointdensity_turbulence(TextureButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Turbulence:", icon="FORCE_TURBULENCE")
        split = box.split()
...
class TEXTURE_PT_ocean(TextureTypePanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Ocean:", icon="MOD_OCEAN")
        col = box.column()
...
class TEXTURE_PT_game_mapping(TextureSlotPanel, Panel):
    ...
    def draw(self, context):
        ...
        if not isinstance(idblock, Brush):
            box = layout.box()
            box.label(text="Coordinates:", icon="MOD_UVPROJECT")
            ...
            elif tex.texture_coords == 'WINDOW':
                split = box.split(factor=0.3)
                split.label(text="Refraction:")
                split.prop(tex, "ior", text="IOR")

            elif tex.texture_coords == 'OBJECT':
                split = box.split(factor=0.3)
                split.label(text="Object:")
                split.prop(tex, "object", text="")
                split.active = False if tex.group_object_decal else True

                split = box.split(factor=0.3)
                split.label(text="Decals Group:")
                split.prop(tex, "group_object_decal", text="")
        ...
        if isinstance(idblock, Brush):
            if context.sculpt_object or context.image_paint_object:
                ...
                brush_texture_settings(box, idblock, context.sculpt_object)
        else:
            box = layout.box()
            box.label(text="Transform:", icon="OUTLINER_OB_LATTICE")
            split = box.split()
    ...
    def draw(self, context):
        ...
        if isinstance(idblock, Brush):
            ...
            if isinstance(idblock, FreestyleLineStyle):
                box = layout.box()
                box.label(text="Projection:", icon="AXIS_TOP")
                split = box.split(factor=0.3)
...
class TEXTURE_PT_game_parallax(TextureSlotPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Parallax:", icon="MOD_DISPLACE")
        split = box.split()
        col = split.column()
...
class TEXTURE_PT_game_influence(TextureSlotPanel, Panel):
    ...
    def draw(self, context):
        ...
        if isinstance(idblock, Material):
            ...
            factor_but(col, "use_map_sss", "sss_factor", "SSS")
            ...
            sub = col.column()
            sub.active = (tex.texture_coords in {"REFLECTION", "WINDOW"})

            sub.label(text="Layer Fresnel:")
            sub.prop(tex, "refraction_ratio", text="Ratio")

        elif isinstance(idblock, Lamp):
            box = layout.box()
            box.label(text="Lamp:", icon="LAMP_SUN")
            split = box.split()
        ...
        if not isinstance(idblock, ParticleSettings):
            box = layout.box()
            box.label(text="Blend:", icon="COLOR")
            split = box.split()

            col = split.column()
            col.prop(tex, "blend_type", text="Blend")
            row = col.row()
            row.prop(tex, "use_rgb_to_intensity")
            sub = row.row()
            sub.active = getattr(tex, "use_rgb_to_intensity")
            sub.prop(tex, "channel", text="")
            # color is used on gray-scale textures even when use_rgb_to_intensity is disabled.
            col.prop(tex, "color", text="")

            col = split.column()
            col.prop(tex, "use_rgb_to_orm", text="ORM")
            row = col.row()
            row.prop(tex, "use_stencil")
            sub = row.row()
            sub.prop(tex, "invert", text="Negative")
        ...
        if isinstance(idblock, Material):
            box = layout.box()
            box.label(text="Bump Mapping:", icon="MOD_DISPLACE")
    ...
    def draw(self, context):
        ...
        if isinstance(idblock, Material):
            if idblock.type in {'SURFACE', 'WIRE'}:
                ...
                box = layout.box()
                box.label(text="Halo:", icon="MATERIAL")
                ...
                box = layout.box()
                box.label(text="Volume:", icon="MOD_FLUIDSIM")
            ...
            box = layout.box()
            box.label(text="General:", icon="PARTICLES")
            split = box.split()
            ...
            hair_box = layout.box()
            hair_box.label(text="Hair:", icon="PARTICLES")

            split = hair_box.split()
            ...
            box = layout.box()
            box.label(text="Line Style:", icon="MATERIAL")
            split = box.split()
...
classes = (
    ...
    # TEXTURE_PT_custom_props,  # disabled: Custom Properties panel unused
)
