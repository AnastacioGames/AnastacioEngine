...
class DATA_PT_shape_curve(CurveButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Shape:", icon="CURVE_DATA")
...
class DATA_PT_curve_texture_space(CurveButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Texture Space:", icon="TEXTURE_DATA")
        row = box.row()
        ...
        box.operator("curve.match_texture_space")
...
class DATA_PT_geometry_curve(CurveButtonsPanelCurve, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Geometry:", icon="OUTLINER_OB_CURVE")
        split = box.split()
        ...
        if type(curve) is not TextCurve:
            box2 = layout.box()
            box2.label(text="Bevel Factor:", icon="MOD_BEVEL")
...
class DATA_PT_pathanim(CurveButtonsPanelCurve, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Path Animation:", icon="CURVE_PATH")
        col = box.column()
...
class DATA_PT_active_spline(CurveButtonsPanelActive, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Active Spline:", icon="CURVE_BEZCURVE")
        split = box.split()
        ...
        if is_poly:
            ...
            box.prop(act_spline, "use_smooth")
        else:
            col = split.column()
...
class DATA_PT_font(CurveButtonsPanelText, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Layout:", icon="ALIGN")
        split = box.split()
        ...
        if rd.engine == "BLENDER_GAME":
            box = layout.box()
            box.label(text="Game Engine Options:", icon="GAME")
            col = box.column()
            col.prop(text, "use_shadows")
            row = col.row()
            row.prop(text, "shadow_offset_x")
            row.prop(text, "shadow_offset_y")
            row = col.row()
            row.prop(text, "shadow_col", text="")
...
class DATA_PT_paragraph(CurveButtonsPanelText, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Alignment:", icon="ALIGN")
        box.label(text="Horizontal Alignment:")
        box.row().prop(text, "align_x", expand=True)

        box.label(text="Vertical Alignment:")
        box.row().prop(text, "align_y", expand=True)

        box2 = layout.box()
        box2.label(text="Spacing & Offset:", icon="ARROW_LEFTRIGHT")
        split = box2.split()
...
class DATA_PT_text_boxes(CurveButtonsPanelText, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Text Boxes:", icon="MESH_PLANE")
        split = box.split()
        col = split.column()
        col.operator("font.textbox_add", icon='ZOOMIN')
        col = split.column()

        for i, tbox in enumerate(text.text_boxes):
            ...
            col.prop(tbox, "width", text="Width")
            col.prop(tbox, "height", text="Height")

            col = split.column(align=True)

            col.label(text="Offset:")
            col.prop(tbox, "x", text="X")
            col.prop(tbox, "y", text="Y")
...
classes = (
    ...
    DATA_PT_paragraph,
    DATA_PT_text_boxes,
    # DATA_PT_custom_props_curve,  # disabled: Custom Properties panel unused
)
