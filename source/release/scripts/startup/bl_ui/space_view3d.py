...
class VIEW3D_HT_header(Header):
    ...
    def draw(self, context):
        ...
        # Layer
        if view.local_view:
            layout.operator("view3d.localview", text="Exit Local View")
        elif object_mode == "OBJECT" and context.object:
            active_object = context.object
            layer_length = len(active_object.layers)
            layer_space = scene if view.lock_camera_and_layers else view

            # Layer Object Is Complex Math To Create One Integer
            layer_bit = 0
            if active_object:
                for bit_index in range(layer_length):
                    if active_object.layers[bit_index]:
                        layer_bit += 2 ** bit_index
        ...
        if show_snap:
            ...
            icon = snap_items[tool_settings.snap_element].icon
...
class VIEW3D_PT_layer(Panel):
    ...
    def draw(self, context):
        layout = self.layout
        layout.label("Layers")
        layout.template_layer_layout_object(2, 2)
...
class VIEW3D_PT_shading(Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(view, "show_invisible_lods")

        if view.viewport_shade not in {'BOUNDBOX', 'WIREFRAME'}:
            layout.prop(view, "show_mist")
        ...
        row = layout.row()
        row.label("3D icons:")
        row.prop(view, "show_icon_components")
...
class VIEW3D_PT_overlay(Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(view, "show_outline_selected")
        layout.prop(view, "show_all_objects_origin")
        layout.prop(view, "show_relationship_lines")

        layout.prop(view, "show_floor", text="Grid")
        ...
        subsub.prop(view, "grid_lines", text="Floor Lines")
        ...
        if context.active_object and context.mode == 'EDIT_MESH':
            ...
            sub.prop(mesh, "show_faces", text="Faces")
            sub.prop(mesh, "show_edges", text="Edges")
            sub.prop(mesh, "show_edge_crease", text="Creases")
            sub.prop(mesh, "show_edge_seams", text="Seams")
            sub.prop(mesh, "show_weight", text="Weight")
            sub.prop(mesh, "show_edge_sharp", text="Sharp", text_ctxt=i18n_contexts.plural)
            sub.prop(mesh, "show_edge_bevel_weight", text="Bevel")
            if with_freestyle:
                sub.prop(mesh, "show_freestyle_edge_marks", text="Edge Marks")
                sub.prop(mesh, "show_freestyle_face_marks", text="Face Marks")

            col = split.column()
            sub = col.column(align=True)
            sub.prop(mesh, "show_extra_edge_length", text="Length")
            sub.prop(mesh, "show_extra_edge_angle", text="Angle")
            sub = col.column(align=True)
            sub.prop(mesh, "show_extra_face_area", text="Area")
            sub.prop(mesh, "show_extra_face_angle", text="Angle")
        ...
        # Curve Overlay
        if context.active_object and context.mode == 'EDIT_CURVE':
            col = layout.column()
            col.use_property_split = True
            col.use_property_decorate = False

            sub = col.column(align=True)
            sub.prop(curve, "show_handles", text="Handles")
            sub.prop(curve, "show_normal_face", text="Normals")

            layout.prop(scene.tool_settings, "normal_size", text="Size")

            layout.separator()
        
        # Motion Treck
        layout.prop(view, "show_reconstruction", text="Motion Tracking")
        ...
        # Mesh Stat Visualizer
        if context.active_object and context.mode == 'EDIT_MESH':
            layout.prop(mesh, "show_statvis", text="Mesh Analysis")
            if mesh.show_statvis:
                row = layout.row(align=True)
                row.label("Type")
                row.prop(statvis, "type", text="")
...
class VIEW3D_PT_pivot_point(Panel):
    ...
    def draw(self, context):
        ...
        col.prop(view, "pivot_point", expand=True)
...
class VIEW3D_PT_snapping(Panel):
    ...
    def draw(self, context):
        ...
        snap_elements = toolsettings.snap_element
        obj = context.active_object
        object_mode = 'OBJECT' if obj is None else obj.mode
...
class VIEW3D_PT_meshdisplay(Panel):
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'HEADER'
