...
class PARTICLE_MT_gpu_particle_presets(Menu):
    bl_label = "GPU Particle Presets"
    preset_subdir = "gpu_particle"
    preset_operator = "script.execute_preset"
    COMPAT_ENGINES = {'BLENDER_GAME'}
    draw = Menu.draw_preset

    # ExecutePreset/AddPresetBase update bl_label on this class, which is shared
    # across all objects. Mirror the choice onto the object itself so switching
    # the active object shows that object's own preset, not the last one picked.
    def post_cb(context):
        if context.object is not None:
            context.object.gpu_particle_active_preset = PARTICLE_MT_gpu_particle_presets.bl_label


bpy.types.Object.gpu_particle_active_preset = bpy.props.StringProperty(
    name="Active GPU Particle Preset", default="", options={'HIDDEN'})
...
class PARTICLE_PT_context_particles(ParticleButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        if context.scene.render.engine == 'BLENDER_GAME':
            # GPU particle settings are drawn by the PARTICLE_PT_gpu_* panels below,
            # which get a real header (and therefore the expand/collapse arrow).
            return
...
class GPUParticleButtonsPanel(ParticleButtonsPanel):
    COMPAT_ENGINES = {'BLENDER_GAME'}
...
class PARTICLE_PT_gpu_emitter(GPUParticleButtonsPanel, Panel):
    bl_label = "GPU Particle System"

    def draw_header(self, context):
        self.layout.prop(context.object, "use_gpu_particles", text="")

    def draw(self, context):
        ob = context.object
        gp = ob.gpu_particles
        layout = self.layout
        layout.enabled = ob.use_gpu_particles

        main_box = layout.box()

        row = main_box.row()
        preset_label = ob.gpu_particle_active_preset or bpy.types.PARTICLE_MT_gpu_particle_presets.bl_label
        row.menu("PARTICLE_MT_gpu_particle_presets", text=preset_label)
        row.operator("particle.gpu_particle_preset_add", text="", icon='ZOOMIN')
        row.operator("particle.gpu_particle_preset_add", text="", icon='ZOOMOUT').remove_active = True

        # ---- Emitter ----
        row = main_box.row(align=True)
        row.prop(ob, "show_expanded_gpu_emitter", text="Emitter",
                 icon='TRIA_DOWN' if ob.show_expanded_gpu_emitter else 'TRIA_RIGHT', emboss=True)

        if ob.show_expanded_gpu_emitter:
            split = main_box.split()
            col = split.column()
            box = col.box()
            box.label(text="Debug:", icon="SETTINGS")
            box.prop(gp, "use_debug_ui", text="Live Debug UI (in-game)")
            box.operator("particle.import_gpu_debug_values", icon="IMPORT")
            col = split.column()
            box = col.box()
            box.label(text="Emission:", icon="PLAY")
            box.prop(gp, "enabled", text="Emit at Game Start")

            split = main_box.split()
            col = split.column()
            box = col.box()
            box.label(text="Pool:", icon="MOD_PARTICLES")
            box.prop(gp, "particle_count")
            col = split.column()
            box = col.box()
            box.label(text="Lifetime:", icon="TIME")
            box.prop(gp, "lifetime", text="Seconds")

            split = main_box.split()
            col = split.column()
            box = col.box()
            box.label(text="Emitter:", icon="OUTLINER_DATA_EMPTY")
            box.prop(gp, "emitter_position")
            box.prop(gp, "emitter_radius")
            col = split.column()
            box = col.box()
            box.label(text="Emission Cone:", icon="MESH_CONE")
            box.prop(gp, "emission_direction")
            box.prop(gp, "emission_angle")

            split = main_box.split()
            col = split.column()
            box = col.box()
            box.label(text="Physics:", icon="PHYSICS")
            box.prop(gp, "collision_mode", text="")
            if gp.collision_mode == 'GROUND':
                box.label(text="Ground Plane:", icon="MESH_PLANE")
                box.prop(gp, "collision_ground_object")
                sub = box.row()
                sub.enabled = gp.collision_ground_object is None
                sub.prop(gp, "collision_height")
                box.prop(gp, "collision_bounce")
                box.prop(gp, "collision_friction")
            elif gp.collision_mode == 'DEPTH':
                box.label(text="Screen-Space:", icon="RENDERLAYERS")
                box.prop(gp, "collision_bounce")
                box.prop(gp, "collision_friction")
                box.label(text="Collides against objects marked 'Object collider with GPU Particle' below.", icon="INFO")
            col = split.column()
            box = col.box()
            row = box.row()
            row.label(text="Object collider with GPU Particle:", icon="MOD_PHYSICS")
            row.prop(ob, "use_gpu_particle_collider", text="")
            box.label(text="Renders this object's depth into the Screen-Space collision pass.")

        # ---- Motion ----
        row = main_box.row(align=True)
        row.prop(ob, "show_expanded_gpu_motion", text="Motion",
                 icon='TRIA_DOWN' if ob.show_expanded_gpu_motion else 'TRIA_RIGHT', emboss=True)

        if ob.show_expanded_gpu_motion:
            box = main_box.box()
            box.label(text="Gravity & Velocity:", icon="FORCE_FORCE")
            split = box.split()
            col = split.column()
            col.label(text="Gravity:")
            col.prop(gp, "gravity", text="")
            col = split.column()
        ...
        row.prop(ob, "show_expanded_gpu_appearance", text="Appearance",
                 icon='TRIA_DOWN' if ob.show_expanded_gpu_appearance else 'TRIA_RIGHT', emboss=True)
        ...
        if ob.show_expanded_gpu_appearance:
            ...
            col.prop(gp, "color")
            col = split.column()
            col.prop(gp, "end_size")
            col.prop(gp, "end_color")

            split = main_box.split()
            col = split.column()
            box = col.box()
            box.label(text="Texture:", icon="TEXTURE")
            box.prop(gp, "texture", text="")
            col = split.column()
            box = col.box()
            box.label(text="Blending:", icon="IMAGE_RGB_ALPHA")
            box.prop(gp, "blend_mode", text="")

            box = main_box.box()
            box.label(text="Billboard:", icon="MESH_PLANE")
            row = box.row()
            row.prop(gp, "billboard_mode", text="")
            sub = row.row()
            sub.enabled = gp.billboard_mode != 'CAMERA_FACING'
            sub.prop(gp, "use_backface_culling")

        # ---- Curves ----
        row = main_box.row(align=True)
        row.prop(ob, "show_expanded_gpu_curves", text="Curves",
                 icon='TRIA_DOWN' if ob.show_expanded_gpu_curves else 'TRIA_RIGHT', emboss=True)

        if ob.show_expanded_gpu_curves:
            row = main_box.row()

            box = row.box()
            box.label(text="Size over Lifetime:", icon="FULLSCREEN_ENTER")
            box.prop(gp, "use_size_curve")
            if gp.use_size_curve and gp.size_curve:
                box.template_curve_mapping(gp, "size_curve", brush=False)

            box = row.box()
            box.label(text="Color over Lifetime:", icon="COLOR")
            box.prop(gp, "use_color_curve")
...
_GPU_DEBUG_SIDECAR_NAME = "gpu_particles_debug.json"
...
def _apply_gpu_debug_values(filepath):
    with open(filepath, "r", encoding="utf-8") as f:
        data = json.load(f)

    applied = []
    for object_name, values in data.items():
        obj = bpy.data.objects.get(object_name)
        if obj is None:
            continue
        gp = obj.gpu_particles
        for key in ("gravity", "lifetime", "emitter_radius", "velocity",
...
class PARTICLE_OT_import_gpu_debug_values(Operator):
    bl_idname = "particle.import_gpu_debug_values"
    bl_label = "Import Debug Values"
    bl_description = (
        "Apply particle values tweaked in the standalone player's live debug UI "
        "(gpu_particles_debug.json, next to this .blend) to the objects here")
...
def _gpu_debug_load_post_handler(dummy):
    path = _gpu_debug_sidecar_path()
    if not path or not os.path.exists(path):
        return
    try:
        ...
        os.replace(path, path[:-len(".json")] + ".applied.json")
        if applied:
            print("[AnastacioEngine] Particle debug values auto-applied for: %s" % ", ".join(applied))
    except (IOError, OSError, ValueError) as e:
        print("[AnastacioEngine] Failed to auto-apply %s: %s" % (path, e))
...
classes = (
    ...
    PARTICLE_MT_gpu_particle_presets,
    PARTICLE_UL_particle_systems,
    PARTICLE_PT_context_particles,
    PARTICLE_PT_gpu_emitter,
    PARTICLE_OT_import_gpu_debug_values,
    ...
    # PARTICLE_PT_custom_props,  # disabled: Custom Properties panel unused
)

if _gpu_debug_load_post_handler not in bpy.app.handlers.load_post:
    bpy.app.handlers.load_post.append(_gpu_debug_load_post_handler)
