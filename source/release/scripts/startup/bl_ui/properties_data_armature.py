...
class DATA_PT_skeleton(ArmatureButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Pose Position:", icon="ARMATURE_DATA")
        ...
        if context.scene.render.engine == 'BLENDER_GAME':
            box = layout.box()
            box.label(text="Deform:", icon="MOD_ARMATURE")
            col = box.column()
            col.prop(arm, "deform_method", expand=True)
            col.prop(arm, "culling_method", expand=False)
...
class DATA_PT_display(ArmatureButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box.row().prop(arm, "draw_type", expand=True)
...
class DATA_PT_bone_groups(ArmatureButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Bone Groups:", icon="GROUP_BONE")
        row = box.row()
        ...
        box = layout.box()
        box.label(text="Actions:", icon="POSE_DATA")
        row = box.row()
...
class DATA_PT_ghost(ArmatureButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Ghost:", icon="GHOST_ENABLED")
        box.row().prop(arm, "ghost_type", expand=True)
...
class DATA_PT_iksolver_itasc(ArmatureButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="IK Solver:", icon="CONSTRAINT_BONE")
        box.prop(ob.pose, "ik_solver")

        if itasc:
            box.row().prop(itasc, "mode", expand=True)
            simulation = (itasc.mode == 'SIMULATION')

            box2 = layout.box()
            box2.label(text="Settings:", icon="SETTINGS")
            if simulation:
                box2.label(text="Reiteration:")
                box2.row().prop(itasc, "reiteration_method", expand=True)
            ...
            box2.prop(itasc, "solver")
            if simulation:
                box2.prop(itasc, "feedback")
                box2.prop(itasc, "velocity_max")
            if itasc.solver == 'DLS':
                row = box2.row()
...
classes = (
    ...
    # DATA_PT_custom_props_arm,  # disabled: Custom Properties panel unused
)
