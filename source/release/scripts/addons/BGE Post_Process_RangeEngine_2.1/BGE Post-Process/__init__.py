#_orignaly the addon has 20 filter but i add more 25 and update some
#Updated by Discord["osama_m.s.a"]


bl_info = {"name": "Post-Processing Filters",
"category": "Game Engine",
"author": "Tim Crellin (Thatimst3r) updated by discord[osama_m.s.a]",
"blender": (2,79,2),
"location": "Game Render Panel",
"description":" 20 + 25 fragment shaders (2dFilters) to use in range engine.",
"version":(2,1)}
    ...
    def draw(self, context):
        ...
        if bpy.context.mode == 'OBJECT':
            ...
            if bpy.context.scene.render.engine != 'BLENDER_GAME':
                ...
                descriptions = {'D.O.F':'Adds depth of field',
                'S.S.A.O': 'Darkens edges of objects',
                'S.S.G.I': 'Improved global lighting quality',
                ...
                'CARTOON':'An effect that makes the game look cartoonish',
                ...
                'F.X.A.A':'Efficient Anti-Aliasing', 'GAMEBOY COLOR':'Gameboy style pixelization.',
                ...
                'H.D.R':'Adjusts light levels to suit environment',
                'V.H.S.I':'An effect that makes the screen vibrate like the old one',
                'V.H.S.II':'The effect makes the screen scene distorted and old',
                ...
                'P.S.X':'DownScale Image Pixles Rate To make image appear more pixel',
                ...
                'C.R.T':'Screen Filter thst make image like old school screens',
                'B.C.R.T':'It creates refraction with blur shader',
                ...
                elif result == True and scn.Chosen_filter == 'RADIOSITY':
                    row = layout.row()
                    layout.label('Warning: This filter may require a High-performance graphics card to work!', icon='ERROR')
                elif result == True and scn.Chosen_filter == 'SSR REFLECTION':
                    row = layout.row()
                    layout.label('Warning: This filter may require a High-performance graphics card to work!', icon='ERROR')
...
def addFilter(new_filter,directory,pass_num):
    ...
    'RADIOSITY':[('xs', 'INT', 10)],
    'TOON':[],
    'H.D.R':[('avgL','FLOAT',0.0),('HDRamount','FLOAT',0.1)],
    'V.H.S.I':[('Timer','TIMER',0.0)],
    'V.H.S.II':[('timer','TIMER',0.0)],
    ...
    'P.S.X':[('Pixeliz','FLOAT', 5.0)],
    'POSTERIZE':[('posterizeLevel','FLOAT',10.0)],
...
def removeFilter(selected_filter):
    if selected_filter != 'MOTION BLUR':
        ...
        timer_props = ['NOISE.glsl','WATER.glsl','ETHEREAL GLOW.glsl','FRACTAL WAVEFORM.glsl','GLITCH.glsl','VGA GLITCH.glsl','WATER RIPPLE.glsl','WAVES.glsl','FADEABLE GLITCH.glsl','CRT.glsl','VHSI.glsl','VHSII.glsl']
...
def register():
    ...
    bpy.types.Scene.Chosen_filter = EnumProperty(items = [('H.D.R','H.D.R','Adjusts light levels to suit environment'),
    ...
    ('D.O.F','D.O.F','Adds depth of field'),
    ('DITHERED','DITHERED','It gives an old style to the game'),
    ('C.R.T','C.R.T','Old Big Screens'),
    ...
    ('B.C.R.T','B.C.R.T','It creates refraction with blur shader'),
    ...
    ('S.S.A.O','S.S.A.O', 'Darkens edges of objects'),
    ('S.S.G.I','S.S.G.I', 'Improved global lighting quality'),
    ...
    ('V.H.S.I','V.H.S.I','An effect that makes the screen vibrate like the old one'),
    ('V.H.S.II','V.H.S.II','The effect makes the screen scene distorted and old'),
    ...
    ('F.X.A.A','F.X.A.A','Efficient Anti-Aliasing'),
