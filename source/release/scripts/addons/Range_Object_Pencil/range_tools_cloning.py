...
def draw_callback_3d(operator, context):
        ...
        bgl.glPushMatrix()
        if operator.rayCastCircle and operator.rayCastCircle[0]:
...
class MFTDrawClones(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        is_mouse_event = event.type in {"MOUSEMOVE", "LEFTMOUSE", "RIGHTMOUSE"}
        if is_mouse_event:
            region = context.region
            ...
            self.rayCastCircle = mft_obj_ray_cast(
                self.dupliList[0][0], self.dupliList[0][1],
                view_vector_mouse, ray_origin_mouse)
        ...
        if self.deleteMode and is_mouse_event:
            radius = int(mifthCloneTools.drawRandomStrokeScatter)
            ...
            indices = []
            if pencilPos is not None:
                removed_any = False
                for indexStrokesList, stroke in enumerate(self.allStrokesList):
                    listtemp = []
                    for index, obj in enumerate(stroke):
    ...
    def invoke(self, context, event):
        ...
        if context.space_data.type == 'VIEW_3D':
            ...
            if not self.dupliList:
                finish_drawing(self, context)
                self.report({'WARNING'}, "Select at least one mesh to draw clones on")
                return {'CANCELLED'}
...
def mft_rotate_object_world(obj, angle, axis):
    """Rotate one clone around its own origin without a context-dependent operator."""
    if axis.length_squared > 0.0:
        obj.matrix_world = Matrix.Rotation(angle, 4, axis.normalized()) * obj.matrix_world
...
def mft_pick_and_clone(self, context, event, ray_max=10000.0):
    ...
    # Check for stroke length after the closest hit has been determined.
    if best_obj is not None and self.currentStrokeList:
        previousHit = self.currentStrokeList[-1]
        ...
        if (best_obj_pos - previousHit.hitPoint).length < strokeLength:
            best_obj = None  # Don't do cloning

    # random scatter things
    if mifthCloneTools.drawRandomStrokeScatter > 0.0 and best_obj is not None:
        # Random Vec
    ...
    if best_obj is not None:
        ...
        if mifthCloneTools.drawClonesNormalRotate is True:
            ...
            mft_rotate_object_world(newDup, angleRotate, rotateAxis)

        # Change Axis
        if mifthCloneTools.drawClonesAxis == 'Y':
            mft_rotate_object_world(newDup, math.radians(90), get_obj_axis(newDup, 'X'))
        elif mifthCloneTools.drawClonesAxis == '-Y':
            mft_rotate_object_world(newDup, math.radians(-90), get_obj_axis(newDup, 'X'))
            mft_rotate_object_world(newDup, math.radians(180), get_obj_axis(newDup, 'Y'))
        elif mifthCloneTools.drawClonesAxis == '-Z':
            mft_rotate_object_world(newDup, math.radians(180), get_obj_axis(newDup, 'X'))
        elif mifthCloneTools.drawClonesAxis == 'X':
            mft_rotate_object_world(newDup, math.radians(-90), get_obj_axis(newDup, 'Y'))
        elif mifthCloneTools.drawClonesAxis == '-X':
            mft_rotate_object_world(newDup, math.radians(90), get_obj_axis(newDup, 'Y'))
        ...
        if mifthCloneTools.drawClonesRadialRotate is True or mifthCloneTools.drawClonesDirectionRotate is True:
            ...
            if newDirRotLookAtt is not None:
                ...
                if tempYCross != Vector((0.0, 0.0, 0.0)):
                    ...
                    mft_rotate_object_world(newDup, xyAngleRotate, best_obj_nor)
        ...
        if mifthCloneTools.randNormalRotateClone > 0.0:
            ...
            mft_rotate_object_world(newDup, randNorAngle, Vector(randNorAxis))
        ...
        if mifthCloneTools.randDirectionRotateClone > 0.0:
            ...
            mft_rotate_object_world(newDup, randDirAngle, randDirVec)
