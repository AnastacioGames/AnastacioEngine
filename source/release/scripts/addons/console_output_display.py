...
class Thief:
    ...
    def write(self, text):
        self.buffer += text
        # Garante que o texto continue aparecendo no console real do sistema em tempo real
        if self.source:
            try:
...
class Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not scene.console_thief:
            return
        
        # Prints
        if scene.show_outputs:
            val = organizeMessage( sys.stdout.getvalue() )
            val = [v for v in val if v != ""] # Remove linhas vazias residuais
                        
            if len(val) > 0:
                if len(val) > 30:
                    val = val[-30:] # Limita para não travar a interface
                    col.label(text="... (Showing last 30 lines) ...", icon="INFO")
...
bl_info = {
    ...
    "author": "G. T. Nunes (Uniday Studio) update by (AnastacioGames)",
    "version": (2, 0),
    "blender": (2, 79, 0),
    "location": "Properties > Render Layers > Game Engine: Console Output",
    "category": "Game Engine"
}         

def update_console_thief(self, context):
    if self.console_thief:
...
def register():
    ...
    bpy.types.Scene.console_thief = bpy.props.BoolProperty(name="Show Console", default=False, update=update_console_thief)
