...
/// Builds internal polygon neighbor connectivity data. Made non-static (upstream
/// keeps this file-local) so it can be exposed through recast-capi for callers
/// (e.g. navmesh_conversion.c) that build polygons directly instead of via rcBuildPolyMesh.
...
struct rcSpan
{
	unsigned int smin : RC_SPAN_HEIGHT_BITS; ///< The lower limit of the span. (Inclusive) [Limit: < #smax]
	unsigned int smax : RC_SPAN_HEIGHT_BITS; ///< The upper limit of the span. (Exclusive) [Limit: <= #RC_SPAN_MAX_HEIGHT]
