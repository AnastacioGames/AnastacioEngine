...
OpenALDevice::OpenALHandle::OpenALHandle(OpenALDevice* device, ALenum format, std::shared_ptr<IReader> reader, bool keep) :
	...
	m_relative(1), m_device(device), m_effect(nullptr), m_effectType(aud::SoundEffectType::AUDIO_EFFECT_INVALID), m_filterType(aud::SoundFilterType::AUDIO_FILTER_INVALID),
	m_toBeStopped(false), m_stopTime(std::chrono::time_point<std::chrono::steady_clock>::min())
{
	DeviceSpecs specs = m_device->m_specs;
	specs.specs = m_reader->getSpecs();
...
OpenALDevice::OpenALHandle::~OpenALHandle()
{
	if (m_effect)
	{
		removeEffect();
...
/******************************************************************************/
/*********************** OpenALHandle HandleEffect Code ***********************/
/******************************************************************************/

OpenALEffect *OpenALDevice::OpenALHandle::getEffect()
{
	return m_effect;
}

void OpenALDevice::OpenALHandle::setEffect(int effectType, int filterType)
{
	std::lock_guard<ILockable> lock(*m_device);

	if (!m_effect && !m_eos) {
		// Get Effect Params Type and set effect
		aud::SoundEffectType type = (aud::SoundEffectType)effectType;
		aud::SoundFilterType effectFilterType = (aud::SoundFilterType)filterType;
		ALuint filterType_AL = GetFilterEffectType(effectFilterType);
		IOpenALEffectParams *params = CreateInterfaceEffectParams(type);
		OpenALEffect *effect = new OpenALEffect(params, m_source, filterType_AL);
			
		m_effect = effect;
		m_effectType = type;
		m_filterType = effectFilterType;
		alSource3i(m_source, AL_AUXILIARY_SEND_FILTER, m_effect->getSlot(), 0, AL_FILTER_NULL);
	}
}

bool OpenALDevice::OpenALHandle::hasEffect()
{
	return (m_effect != nullptr);
}

int OpenALDevice::OpenALHandle::getEffectType()
{
	int value = m_effectType;
	return value;
}

int OpenALDevice::OpenALHandle::getFilterType()
{
  int value = m_filterType;
  return value;
}

void OpenALDevice::OpenALHandle::removeEffect()
{
  std::lock_guard<ILockable> lock(*m_device);
  ...
  if (m_effect) {
		...
		delete m_effect;
		m_effect = nullptr;
		m_effectType = AUDIO_EFFECT_INVALID;
		m_filterType = AUDIO_FILTER_INVALID;
  }
}

bool OpenALDevice::OpenALHandle::setReverbDensity(float density)
{
	std::lock_guard<ILockable> lock(*m_device);

	if (!checkValidEffect(aud::SoundEffectType::AUDIO_EFFECT_REVERB))
		return false;

	OpenALReverbEffect *params = dynamic_cast<OpenALReverbEffect*>(m_effect->GetInterfaceEffectParams());
...
bool OpenALDevice::OpenALHandle::setReverbDiffusion(float diffusion)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setDiffusion(diffusion);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbGain(float gain)
{
	std::lock_guard<ILockable> lock(*m_device);
...
bool OpenALDevice::OpenALHandle::setReverbGainHF(float gain_hf)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setGainHF(gain_hf);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbDecayTime(float decay_time)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setDecayTime(decay_time);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbDecayHFRatio(float decay_hf_ratio)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setDecayHFRatio(decay_hf_ratio);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbReflectionsGain(float reflection_gain)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setReflectionsGain(reflection_gain);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbReflectionsDelay(float reflection_delay)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setReflectionsDelay(reflection_delay);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbLateReverbGain(float late_reverb_gain)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setLateReverbGain(late_reverb_gain);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbLateReverbDelay(float late_reverb_delay)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setLateReverbDelay(late_reverb_delay);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbAirAbsorptionGainHF(float air_absorption)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setAirAbsorptionGainHF(air_absorption);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbRoomRolloffFactor(float rolloff_factor)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setRoomRolloffFactor(rolloff_factor);

	return true;
}

bool OpenALDevice::OpenALHandle::setReverbDecayLimitHF(int decay_limit_hf)
{
	std::lock_guard<ILockable> lock(*m_device);
	...
	if (params)
		params->setDecayLimitHF(decay_limit_hf);

	return true;
}

/******************************************************************************/
/****************** OpenALHandle HandleFilterEffect Code **********************/
/******************************************************************************/

bool OpenALDevice::OpenALHandle::setFilterGain(float gain)
{
  std::lock_guard<ILockable> lock(*m_device);
  ...
  if (m_effect)
    m_effect->applyFilterParameter(m_effect->GetFilterParameter(m_filterType, 0), gain);

  return true;
}

bool OpenALDevice::OpenALHandle::setFilterGainLF(float gainLF)
{
  std::lock_guard<ILockable> lock(*m_device);
  ...
  if (m_effect)
    m_effect->applyFilterParameter(m_effect->GetFilterParameter(m_filterType, 2), gainLF);

  return true;
}

bool OpenALDevice::OpenALHandle::setFilterGainHF(float gainHF)
{
  std::lock_guard<ILockable> lock(*m_device);
  ...
  if (m_effect)
    m_effect->applyFilterParameter(m_effect->GetFilterParameter(m_filterType, 1), gainHF);
...
{
	// The current OpenAL context is thread-local state; it was only made current on the
	// thread that constructed this device, not on this dedicated streaming thread.
				...
				if(info != AL_PLAYING)
				{
					...
					if(sound->m_eos && info != AL_INITIAL)
					{
						...
						// stop
							...
							// If it has an effect, we will delete the sound later.
							if (sound->hasEffect()) {
								// not set to be stopped ? set now.
								if (!sound->m_toBeStopped) {
									sound->m_toBeStopped = true;
									sound->m_stopTime = std::chrono::steady_clock::now() + std::chrono::milliseconds(1300);
								}
								else {
									auto now = std::chrono::steady_clock::now();
									if (now >= sound->m_stopTime) {
										// finally, we can stop this sound.
