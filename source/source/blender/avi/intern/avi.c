...
AviError AVI_open_compress(char *name, AviMovie *movie, int streams, ...)
{
	...
	write_ok &= awrite(movie, &list, 1, sizeof(AviList), movie->fp, AVI_LIST);
	...
	for (i = 0; i < movie->header->Streams; i++) {
		...
		write_ok &= awrite(movie, &movie->streams[i].sh, 1, sizeof(AviStreamHeader), movie->fp, AVI_STREAMH);
...
AviError AVI_write_frame(AviMovie *movie, int frame_num, ...)
{
	...
	for (stream = 0; stream < movie->header->Streams; stream++) {
		...
		/* Write the chunk */
		write_ok &= awrite(movie, buffer, 1, size, movie->fp, AVI_RAW);
		MEM_freeN(buffer);

		if (size % 4) write_ok &= awrite(movie, &tbuf, 1, 4 - size % 4, movie->fp, AVI_RAW);
