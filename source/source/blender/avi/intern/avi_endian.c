...
bool awrite(AviMovie *movie, void *datain, int block, int size, FILE *fp, int type)
{
	bool ok;
#ifdef __BIG_ENDIAN__
	void *data;
	...
	switch (type) {
		case AVI_RAW:
			ok = (fwrite(data, block, size, fp) == (size_t)size);
			break;
		case AVI_CHUNK:
	...
	(void)movie; /* unused */
	(void)type; /* unused */
	ok = (fwrite(datain, block, size, fp) == (size_t)size);
#endif /* __BIG_ENDIAN__ */
	return ok;
}
