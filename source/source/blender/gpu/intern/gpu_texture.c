...
static struct GPUTextureGlobal {
	 ...
	 * space. See RAS_Rasterizer::UpdateGlobalDepthTexture / RAS_ParticleBuffer::Update. */
	float depth_viewproj[16];
	int depth_viewproj_valid;
	/* GPU particle screen-space collision, "GPU Particle Collider" flavor: separate texture/
	 * matrix pair, populated by KX_KetsjiEngine::RenderCollisionDepthBuffer from objects
	 * flagged use_gpu_particle_collider. Kept independent from depth_tex/depth_viewproj above
	 * so this mechanism doesn't depend on (or interfere with) Depth Transparency materials. */
	GPUTexture *collider_depth_tex;
	float collider_depth_viewproj[16];
	int collider_depth_viewproj_valid;
...
static unsigned char *GPU_texture_convert_pixels(size_t length, const float *fpixels)
{
	unsigned char *pixels, *p;
	const float *fp = fpixels;
	const size_t len = 4 * length;

	p = pixels = MEM_callocN(sizeof(unsigned char) * len, "GPUTexturePixels");

	for (size_t a = 0; a < len; a++, p++, fp++)
...
void GPU_texture_set_global_depth_viewproj(const float viewproj[16])
{
	...
	GG.depth_viewproj_valid = 1;
}

int GPU_texture_get_global_depth_viewproj(float r_viewproj[16])
{
	if (!GG.depth_viewproj_valid) {
		return 0;
	}
	memcpy(r_viewproj, GG.depth_viewproj, sizeof(GG.depth_viewproj));
	return 1;
}

GPUTexture **GPU_texture_global_collider_depth_ptr(void)
{
	return &GG.collider_depth_tex;
}

void GPU_texture_set_global_collider_depth(GPUTexture *depthtex)
{
	if (depthtex) {
		GG.collider_depth_tex = depthtex;
	}
	else {
		GG.collider_depth_tex = GG.invalid_tex_2D;
		GG.collider_depth_viewproj_valid = 0;
	}
}

void GPU_texture_set_global_collider_depth_viewproj(const float viewproj[16])
{
	memcpy(GG.collider_depth_viewproj, viewproj, sizeof(GG.collider_depth_viewproj));
	GG.collider_depth_viewproj_valid = 1;
}

int GPU_texture_get_global_collider_depth_viewproj(float r_viewproj[16])
{
	if (!GG.collider_depth_viewproj_valid) {
		return 0;
	}
	memcpy(r_viewproj, GG.collider_depth_viewproj, sizeof(GG.collider_depth_viewproj));
	return 1;
}
