...
extern char datatoc_gpu_shader_fx_bloom_frag_glsl[];
extern char datatoc_gpu_shader_fx_tonemap_frag_glsl[];
...
extern char datatoc_gpu_shader_light_glow_vert_glsl[];
extern char datatoc_gpu_shader_light_glow_frag_glsl[];
...
{
#ifdef WITH_GL_PROFILE_CORE
	strcat(defines, "#define USE_CORE_PROFILE\n");
#endif
	...
	if (use_skinning) {
		strcat(defines, "#define USE_SKINNING\n");
		BLI_snprintf(defines + strlen(defines), MAX_DEFINE_LENGTH - strlen(defines), "#define MAX_BONES %d\n", GPU_MAX_SKINNING_BONES);
	}

	if (use_foliage) {
		strcat(defines, "#define USE_FOLIAGE\n");
	}
	...
	if (use_user_code) {
		strcat(defines, "#define USE_USER_CODE\n");
	}
                                            ...
                                            const char *diagnostic_name)
{
	...
	bool use_skinning = (flags & GPU_SHADER_FLAGS_SPECIAL_SKINNING) != 0;
	GLint status;
	GLchar log[5000];
	GLsizei length = 0;
	GPUShader *shader;
	...
	gpu_shader_standard_defines(standard_defines,
								...
								use_instancing,
								use_skinning,
								(flags & GPU_SHADER_FLAGS_FOLIAGE) != 0,
...
GPUShader *GPU_shader_get_builtin_shader(GPUBuiltinShader shader)
{
	...
	switch (shader) {
		...
		case GPU_SHADER_2D_BOX:
			...
			break;
		case GPU_SHADER_LIGHT_GLOW:
			if (!GG.shaders.light_glow)
				GG.shaders.light_glow = GPU_shader_create(
					datatoc_gpu_shader_light_glow_vert_glsl, datatoc_gpu_shader_light_glow_frag_glsl,
					NULL, NULL, NULL, 0, 0, 0);
			retval = GG.shaders.light_glow;
			break;
	}
	...
	if (!GG.shaders.fx_shaders[offset]) {
		...
		switch (effect) {
			...
			case GPU_SHADER_FX_BLOOM:
				shader = GPU_shader_create(datatoc_gpu_shader_fx_vert_glsl, datatoc_gpu_shader_fx_bloom_frag_glsl, NULL, datatoc_gpu_shader_fx_lib_glsl, defines, 0, 0, 0);
				break;

			case GPU_SHADER_FX_TONEMAP:
				shader = GPU_shader_create(datatoc_gpu_shader_fx_vert_glsl, datatoc_gpu_shader_fx_tonemap_frag_glsl, NULL, datatoc_gpu_shader_fx_lib_glsl, defines, 0, 0, 0);
				break;

			case GPU_SHADER_FX_LIGHTSCATTER:
				shader = GPU_shader_create(datatoc_gpu_shader_fx_vert_glsl, datatoc_gpu_shader_fx_light_scatter_frag_glsl, NULL, datatoc_gpu_shader_fx_lib_glsl, defines, 0, 0, 0);
				break;

			case GPU_SHADER_FX_SSR:
				shader = GPU_shader_create(datatoc_gpu_shader_fx_vert_glsl, datatoc_gpu_shader_fx_ssr_frag_glsl, NULL, datatoc_gpu_shader_fx_lib_glsl, defines, 0, 0, 0);
				break;
...
void GPU_shader_free_builtin_shaders(void)
{
	...
	if (GG.shaders.light_glow) {
		GPU_shader_free(GG.shaders.light_glow);
		GG.shaders.light_glow = NULL;
	}
