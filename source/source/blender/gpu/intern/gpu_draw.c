...
static void gpu_set_alpha_blend(GPUBlendMode alphablend)
{
	...
	else if (alphablend == GPU_BLEND_MULTIPLY) {
		glEnable(GL_BLEND);
...
#else
	/* GL_ALPHA_TEST/glAlphaFunc removed under core profile -- blend-mode-only fallback.
	 * Alpha-clip cutout (GPU_BLEND_CLIP/GPU_BLEND_ALPHA_TO_COVERAGE) has no alpha-test
	 * equivalent here; a real port would need a shader-side discard, out of scope. */
	if (alphablend == GPU_BLEND_SOLID) {
		glDisable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	}
		...
		/* No alpha-test discard under core -- blend instead of a hard cutout, so
		 * the texture's opaque backing color (e.g. green on leaf/cutout textures)
		 * doesn't show through solid where GL_ALPHA_TEST used to clip it away. */
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	}
	else if (alphablend == GPU_BLEND_ALPHA_TO_COVERAGE) {
...
static struct GPUMaterialState {
	...
	float (gftime); /* frame time from viewport. used in "time" node */
...
int GPU_object_material_bind(int nr, void *attribs)
{
	...
	if (GMS.lastretval) {
		...
		if (gattribs && GMS.gmatbuf[nr]) {
			...
			GPU_material_bind(
			        gpumat, GMS.glay, (double)GMS.gftime, !(GMS.gob->mode & OB_MODE_TEXTURE_PAINT),
			        GMS.gviewmat, GMS.gviewinv, GMS.gviewcamtexcofac, GMS.gscenelock, NULL);
			...
			// Range: For size of GL_POINTS. With HALO type enabled.
			if (mat->material_type == MA_TYPE_HALO) {
				glPointSize(mat->point_size);
