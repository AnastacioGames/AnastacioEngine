...
/** \name Link to LibLoad: link an ID into bmain without instantiating it anywhere.
 *
 * Reuses wm_link_do() with scene == NULL, same trick already used internally by
 * lib_relocate_do() ("We do not want any instantiation here!"). The linked ID only
 * gets a fake user so it survives save/reload; runtime addObject() picks it up later
 * straight from bmain (see BL_Converter::FindOrConvertMainObject).
 * \{ */

static int wm_link_to_libload_exec(bContext *C, wmOperator *op)
{
	Main *bmain = CTX_data_main(C);
	WMLinkAppendData *lapp_data;
	char path[FILE_MAX_LIBEXTRA], root[FILE_MAXDIR], libname[FILE_MAX_LIBEXTRA], relname[FILE_MAX];
	...
	/* check if something is indicated for linking */
	{
	...
	/* scene == NULL, v3d == NULL: nothing gets instantiated anywhere. */
	wm_link_do(lapp_data, op->reports, bmain, NULL, NULL);

	IMB_colormanagement_check_file_config(bmain);

	/* Without a real user (no scene/collection referencing it), the linked ID would be
	 * dropped on next save/reload; a fake user keeps it around so addObject() can find it
	 * later. Mark its Library too: this persistently records that this file was registered
	 * through LibLoad, including when the selected ID is a Group whose member Objects have
	 * real users from the Group itself. */
	{
		LinkNode *itemlink;
		for (itemlink = lapp_data->items.list; itemlink; itemlink = itemlink->next) {
			ID *new_id = ((WMLinkAppendDataItem *)(itemlink->link))->new_id;
			if (new_id) {
				...
				id_fake_user_set(&new_id->lib->id);
			}
		}
	}
...
void WM_OT_link_to_libload(wmOperatorType *ot)
{
	ot->name = "Link to LibLoad";
	ot->idname = "WM_OT_link_to_libload";
	ot->description = "Register a library's data-block address in this file without instantiating it in any "
	                   "scene; runtime addObject() can load it later";

	ot->invoke = wm_link_append_invoke;
	ot->exec = wm_link_to_libload_exec;
	ot->poll = wm_link_append_poll;
	...
	/* wm_link_append_invoke() is shared with the regular Link operator. Declare
	 * its standard options even though this operator always links silently, so
	 * the file selector never queries missing RNA properties. */
	wm_link_append_properties_common(ot, true);
}

/* Import one Text dependency displayed by External Files.  This deliberately
 * bypasses the generic Append operator: no file browser, scene instancing or
 * recursive library conversion is wanted here. */
static int wm_import_libload_text_exec(bContext *C, wmOperator *op)
{
	Main *bmain = CTX_data_main(C);
	BlendHandle *bh;
	Main *mainl;
	...
	char text_name[MAX_NAME];

	RNA_string_get(op->ptr, "filepath", filepath);
	RNA_string_get(op->ptr, "text", text_name);

	if (BKE_libblock_find_name(bmain, ID_TXT, text_name) != NULL) {
		BKE_reportf(op->reports, RPT_WARNING, "Text '%s' already exists in this file", text_name);
		return OPERATOR_CANCELLED;
	}

	bh = BLO_blendhandle_from_file(filepath, op->reports);
	if (bh == NULL) {
		return OPERATOR_CANCELLED;
	}

	BKE_main_id_tag_all(bmain, LIB_TAG_PRE_EXISTING, true);
	mainl = BLO_library_link_begin(bmain, &bh, filepath);
	text_id = BLO_library_link_named_part(mainl, &bh, ID_TXT, text_name);
	BLO_library_link_end(mainl, &bh, 0, NULL, NULL);
	BLO_blendhandle_close(bh);

	if (text_id == NULL) {
		BKE_main_id_tag_all(bmain, LIB_TAG_PRE_EXISTING, false);
		BKE_reportf(op->reports, RPT_ERROR, "Text '%s' was not found in '%s'", text_name, filepath);
		return OPERATOR_CANCELLED;
	}

	/* The just-linked Text has no local user, so force its one datablock local
	 * instead of making every ID from this registered LibLoad library local. */
	id_make_local(bmain, text_id, false, true);
	local_id = text_id->newid ? text_id->newid : text_id;
	id_fake_user_set(local_id);
	BKE_id_clear_newpoin(text_id);
	BKE_main_id_tag_all(bmain, LIB_TAG_PRE_EXISTING, false);

	WM_event_add_notifier(C, NC_WINDOW, NULL);
	BKE_reportf(op->reports, RPT_INFO, "Imported Text '%s' into this file", text_name);
	return OPERATOR_FINISHED;
}

void WM_OT_import_libload_text(wmOperatorType *ot)
{
	ot->name = "Import LibLoad Text";
	ot->idname = "WM_OT_import_libload_text";
	ot->description = "Copy one external Text datablock into this file";
	ot->exec = wm_import_libload_text_exec;
	ot->poll = wm_link_append_poll;
	ot->flag = OPTYPE_UNDO;

	RNA_def_string(ot->srna, "filepath", NULL, FILE_MAX, "Filepath", "External library containing the Text");
	RNA_def_string(ot->srna, "text", NULL, MAX_NAME, "Text", "Name of the external Text datablock");
}
