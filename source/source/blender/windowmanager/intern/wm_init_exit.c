...
void WM_init(bContext *C, int argc, const char **argv)
{
	...
	/* Safe mode: ensure the user config/scripts/datafiles folders exist before any
	 * UI/init code runs. If these are missing entirely (e.g. user deleted the whole
	 * config directory), some startup code assumes they exist and can crash. */
	BKE_appdir_folder_id_create(BLENDER_USER_CONFIG, NULL);
	BKE_appdir_folder_id_create(BLENDER_USER_SCRIPTS, NULL);
	BKE_appdir_folder_id_create(BLENDER_USER_DATAFILES, NULL);
	...
	if (U.uiflag2 & USER_KEEP_SESSION) {
		...
		/* init view3D Icons */
		// printf("Initializing View3D Icons\n");
		ED_view3d_icons_init(bmain);
	}
}
