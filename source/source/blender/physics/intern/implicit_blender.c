...
{
	/* to/from must have matching layout; both are allocated once from the
	 * same numverts/numsprings pair in BPH_mass_spring_solver_create() and
	 * never resized independently. */
	BLI_assert(to[0].vcount == from[0].vcount && to[0].scount == from[0].scount);
