...
static int game_components_copy_exec(bContext *C, wmOperator *UNUSED(op))
{
	Object *ob = ED_object_active_context(C);
	...
	{
		if (ob != ob_iter) {
			/* first: free all components */
			BKE_python_component_free_list(&ob_iter->components);

			/* now copy it */
			BKE_python_component_copy_list(&ob_iter->components, &ob->components);
		}
	}
	CTX_DATA_END;

	WM_event_add_notifier(C, NC_GAME, NULL);

	return OPERATOR_FINISHED;
}

void OBJECT_OT_game_components_copy(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Copy Components to Selected";
	ot->description = "Copy Game Components to other selected objects";
	ot->idname = "OBJECT_OT_game_components_copy";

	/* api callbacks */
	ot->exec = game_components_copy_exec;
	ot->poll = ED_operator_object_active_editable;
