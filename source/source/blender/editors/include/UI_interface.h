...
enum {
	...
	UI_BUT_ACTIVATE_ON_INIT= (1 << 31),  /** Use for popups to start editing the button on initialization. */
};
...
void UI_draw_anti_tria(float x1, float y1, float x2, float y2, float x3, float y3, const float color[4], const float width);
...
void UI_but_link_set(struct uiBut *but,  void **poin,  void ***ppoin,  short *tot,  int from, int to, char color[3]);
...
void    UI_block_event_func_set(uiBlock *block,
                                int (*func)(const struct bContext *, uiBlock *, const struct wmEvent *));
...
/* used in logic bricks. */
void uiLayoutBoxSetCustomColor(uiLayout *layout, const unsigned char col[3]);
...
uiLayout *uiLayoutGridFlow(
        uiLayout *layout, bool row_major, int num_columns, bool even_columns, bool even_rows, bool align);
...
void uiTemplateScreenTabs(uiLayout *layout, struct bContext *C);
void uiTemplateID(
