...
static void region_draw_azone_tria(AZone *az)
{
	...
	switch (az->edge) {
		case AE_TOP_TO_BOTTOMRIGHT:
			UI_draw_anti_tria((float)az->x1, (float)az->y1, (float)az->x2, (float)az->y1, (float)(az->x1 + az->x2) / 2, (float)az->y2, color, 1.0f);
			break;

		case AE_BOTTOM_TO_TOPLEFT:
			UI_draw_anti_tria((float)az->x1, (float)az->y2, (float)az->x2, (float)az->y2, (float)(az->x1 + az->x2) / 2, (float)az->y1, color, 1.0f);
			break;

		case AE_LEFT_TO_TOPRIGHT:
			UI_draw_anti_tria((float)az->x2, (float)az->y1, (float)az->x2, (float)az->y2, (float)az->x1, (float)(az->y1 + az->y2) / 2, color, 1.0f);
			break;

		case AE_RIGHT_TO_TOPLEFT:
			UI_draw_anti_tria((float)az->x1, (float)az->y1, (float)az->x1, (float)az->y2, (float)az->x2, (float)(az->y1 + az->y2) / 2, color, 1.0f);
			break;
...
static const EditorTypeItem animation_editors[] = {
	{SPACE_TIME, "Timeline", ICON_TIME},
	{SPACE_IPO, "Graph Editor", ICON_IPO},
	{SPACE_ACTION, "Dope Sheet", ICON_ACTION},
	{0, NULL, 0}
};

static const EditorTypeItem scripting_editors[] = {
	{SPACE_TEXT, "Text Editor", ICON_TEXT},
	{SPACE_LOGIC, "Logic Bricks Editor", ICON_LOGIC},
	{SPACE_OUTLINER, "Outliner", ICON_OOPS},
	{SPACE_BUTS, "Properties", ICON_BUTS},
	{0, NULL, 0}
};

static const EditorTypeItem more_editors[] = {
	{SPACE_SEQ, "Video Sequencer", ICON_SEQUENCE},
	{SPACE_CLIP, "Movie Clip Editor", ICON_CLIP},
	{SPACE_NLA, "NLA Editor", ICON_NLA},
	{SPACE_CONSOLE, "Python Console", ICON_CONSOLE},
	{SPACE_INFO, "Info", ICON_INFO},
	{SPACE_FILE, "File Browser", ICON_FILESEL},
	{SPACE_USERPREF, "User Preferences", ICON_PREFERENCES},
	{0, NULL, 0}
};
...
static void ui_area_type_menu_item(uiLayout *layout, ScrArea *sa, const EditorTypeItem *item)
{
...
static void ui_area_type_menu_column(uiLayout *layout, ScrArea *sa, const char *title, const EditorTypeItem *items)
{
	...
	for (int i = 0; items[i].label != NULL; i++) {
		ui_area_type_menu_item(layout, sa, &items[i]);
	}
}

static void ui_area_type_menu_more(bContext *C, uiLayout *layout, void *arg1)
{
	ScrArea *sa = (ScrArea *)arg1;

	for (int i = 0; more_editors[i].label != NULL; i++) {
		ui_area_type_menu_item(layout, sa, &more_editors[i]);
	}
}

static void ui_area_type_menu_main(bContext *C, uiLayout *layout, void *arg1)
{
	ScrArea *sa = (ScrArea *)arg1;
...
int ED_area_header_switchbutton(const bContext *C, uiBlock *block, int yco)
{
	...
	/* Keep this as a direct menu button, like the File button in the header.
	 * Creating a second layout inside the absolute header block leaves the item
	 * outside the header's resolved bounds on Blender 2.79. */
...
void ED_region_header(const bContext *C, ARegion *ar)
{
	...
	HeaderType *ht;
	Header header = {NULL};
	bool region_layout_based = false; // ar->flag & RGN_FLAG_DYNAMIC_SIZE;
	...
	for (ht = ar->type->headertypes.first; ht; ht = ht->next) {
		...
		/*
		 * LATE :
		 * Hack For Problematic Separator Spacer .
		 */
...
{
	const int header_size = (U.header_size != 0) ? U.header_size : HEADERY;
	return (int)(header_size * UI_DPI_FAC);
}
