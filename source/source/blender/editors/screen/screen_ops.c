...
static int screen_set_exec(bContext *C, wmOperator *op)
{
	...
	if (screen_name[0]) {
		for (screen_target = bmain->screen.first; screen_target; screen_target = screen_target->id.next) {
			if (STREQ(screen_target->id.name + 2, screen_name)) {
				break;
			}
		}

		if (screen_target && screen_set_is_ok(screen_target, screen_prev)) {
...
/* -------------------------------------------------------------------- */
/** \name Rename Screen Operator
 * \{ */

static int screen_rename_exec(bContext *C, wmOperator *op)
{
	Main *bmain = CTX_data_main(C);
	char screen_name[MAX_ID_NAME - 2];
	char new_name[MAX_ID_NAME - 2];

	RNA_string_get(op->ptr, "screen_name", screen_name);
	RNA_string_get(op->ptr, "new_name", new_name);
	...
	for (bScreen *screen = bmain->screen.first; screen; screen = screen->id.next) {
		if (STREQ(screen->id.name + 2, screen_name)) {
			BKE_libblock_rename(bmain, &screen->id, new_name);
			WM_event_add_notifier(C, NC_SCREEN | ND_SCREENBROWSE, screen);
			return OPERATOR_FINISHED;
		}
	}
...
static int screen_rename_invoke(bContext *C, wmOperator *op, const wmEvent *UNUSED(event))
{
	char screen_name[MAX_ID_NAME - 2];
	RNA_string_get(op->ptr, "screen_name", screen_name);
	RNA_string_set(op->ptr, "new_name", screen_name);
	return WM_operator_props_dialog_popup(C, op, 10 * UI_UNIT_X, UI_UNIT_Y);
}

static void SCREEN_OT_screen_rename(wmOperatorType *ot)
{
	ot->name = "Rename Screen";
	ot->description = "Rename the selected screen tab";
	ot->idname = "SCREEN_OT_screen_rename";

	ot->invoke = screen_rename_invoke;
	ot->exec = screen_rename_exec;
	ot->poll = ED_operator_screenactive;

	/* rna */
	PropertyRNA *prop = RNA_def_string(ot->srna, "screen_name", NULL, MAX_ID_NAME - 2, "Screen", "Screen to rename");
	RNA_def_property_flag(prop, PROP_HIDDEN | PROP_SKIP_SAVE);
	RNA_def_string(ot->srna, "new_name", NULL, MAX_ID_NAME - 2, "New Name", "New name for the screen");
}
