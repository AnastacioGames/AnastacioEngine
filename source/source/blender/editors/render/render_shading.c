...
/********************** world game property operators *********************/

static int world_game_property_new_exec(bContext *C, wmOperator *op)
{
	...
	BLI_uniquename(&world->prop, prop, DATA_("Property"), '.', offsetof(bProperty, name), sizeof(prop->name));

	WM_event_add_notifier(C, NC_LOGIC, NULL);
	return OPERATOR_FINISHED;
}

static bool world_game_property_poll(bContext *C)
{
	return CTX_data_scene(C)->world != NULL;
}
...
void WORLD_OT_game_property_new(wmOperatorType *ot)
{
	...
	/* api callbacks */
	ot->exec = world_game_property_new_exec;
	ot->poll = world_game_property_poll;

	/* flags */
	ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;

	RNA_def_enum(ot->srna, "type", rna_enum_gameproperty_type_items, GPROP_FLOAT, "Type", "Type of property to add");
	RNA_def_string(ot->srna, "name", NULL, MAX_NAME, "Name", "Name of the property to add");
}

static int world_game_property_remove_exec(bContext *C, wmOperator *op)
{
	World *world = CTX_data_scene(C)->world;
	bProperty *prop;
	...
	prop = BLI_findlink(&world->prop, index);

	if (prop) {
		BLI_remlink(&world->prop, prop);
...
void WORLD_OT_game_property_remove(wmOperatorType *ot)
{
	...
	/* api callbacks */
	ot->exec = world_game_property_remove_exec;
	ot->poll = world_game_property_poll;
...
#define WORLD_GAME_PROPERTY_MOVE_UP    1
#define WORLD_GAME_PROPERTY_MOVE_DOWN -1

static int world_game_property_move(bContext *C, wmOperator *op)
{
	World *world = CTX_data_scene(C)->world;
	bProperty *prop;
	...
	if (dir == WORLD_GAME_PROPERTY_MOVE_UP) {
		otherprop = prop->prev;
	}
	else if (dir == WORLD_GAME_PROPERTY_MOVE_DOWN) {
		otherprop = prop->next;
	}
	else {
		BLI_assert(0);
	...
	if (prop && otherprop) {
		BLI_listbase_swaplinks(&world->prop, prop, otherprop);
...
void WORLD_OT_game_property_move(wmOperatorType *ot)
{
	static const EnumPropertyItem direction_property_move[] = {
		{WORLD_GAME_PROPERTY_MOVE_UP,   "UP",   0, "Up",   ""},
		{WORLD_GAME_PROPERTY_MOVE_DOWN, "DOWN", 0, "Down", ""},
		{0, NULL, 0, NULL, NULL}
	};
	PropertyRNA *prop;
	...
	/* api callbacks */
	ot->exec = world_game_property_move;
	ot->poll = world_game_property_poll;
...
#undef WORLD_GAME_PROPERTY_MOVE_UP
#undef WORLD_GAME_PROPERTY_MOVE_DOWN
