...
/* backup of the mutable/animatable fields of a single constraint,
 * so preview-driven changes to them (e.g. via drivers/fcurves on
 * pose.bones["x"].constraints["y"].influence) can be undone on cancel */
typedef struct tPoseLib_ConstraintBackup {
	struct tPoseLib_ConstraintBackup *next, *prev;

	bConstraint *con;   /* constraint this backup is for (not owned) */
	float enforce;
	short flag;
} tPoseLib_ConstraintBackup;
...
typedef struct tPoseLib_Backup {
	...
	ListBase constraint_backups; /* tPoseLib_ConstraintBackup structs, one per pchan constraint */
} tPoseLib_Backup;

/* Makes a copy of the current pose for restoration purposes */
	...
	for (agrp = pld->act->groups.first; agrp; agrp = agrp->next) {
		...
		if (pchan) {
			...
			for (bConstraint *con = pchan->constraints.first; con; con = con->next) {
				tPoseLib_ConstraintBackup *cb = MEM_mallocN(sizeof(tPoseLib_ConstraintBackup), "tPoseLib_ConstraintBackup");
				cb->con = con;
				cb->enforce = con->enforce;
				cb->flag = con->flag;
				BLI_addtail(&plb->constraint_backups, cb);
			}
...
static void poselib_backup_restore(tPoseLib_PreviewData *pld)
{
	...
	for (plb = pld->backups.first; plb; plb = plb->next) {
		...
		/* restore constraint settings that the preview may have changed
		 * (e.g. via drivers/fcurves targeting influence) */
		for (tPoseLib_ConstraintBackup *cb = plb->constraint_backups.first; cb; cb = cb->next) {
			cb->con->enforce = cb->enforce;
			cb->con->flag = cb->flag;
		}
	}
}
...
static void poselib_backup_free_data(tPoseLib_PreviewData *pld)
{
	...
	for (plb = pld->backups.first; plb; plb = plbn) {
		...
		BLI_freelistN(&plb->constraint_backups);
