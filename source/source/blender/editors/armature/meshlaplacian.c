...
static void harmonic_coordinates_bind(Scene *UNUSED(scene), MeshDeformModifierData *mmd, MeshDeformBind *mdb)
{
	...
	/* guard against totvert * totcagevert overflowing the int arithmetic
	 * used below and in modifier_mdef_compact_influences() */
	if ((double)mdb->totvert * (double)mdb->totcagevert > (double)INT_MAX) {
		modifier_setError((ModifierData *)mmd, "Too many vertices for mesh deform bind");
		return;
	}
