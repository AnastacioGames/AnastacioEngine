...
static bool edit_sensor_poll(bContext *C)
{
	...
	if (ptr.data && ID_IS_LINKED(ptr.id.data)) {
		return 0;
	}
	return 1;
}
...
static void LOGIC_OT_python_component_register(wmOperatorType *ot)
{
	...
	PropertyRNA *parm;
	parm = RNA_def_string(ot->srna, "component_name", "module.Component", FILE_MAX, "Component", "The component class name with module (module.ComponentName)");
