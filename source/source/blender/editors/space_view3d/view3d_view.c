...
static void game_set_commmandline_options(GameData *gm)
{
	...
	if ((syshandle = SYS_GetSystem())) {
		...
		test = (gm->flag & GAME_SHOW_DEBUG_MODE);
		SYS_WriteCommandLineInt(syshandle, "show_debug_mode", test);

		test = (gm->flag & GAME_SHOW_CONSOLE);
		SYS_WriteCommandLineInt(syshandle, "show_console", test);

		test = (gm->flag & GAME_SHOW_RENDER_QUERIES);
		SYS_WriteCommandLineInt(syshandle, "show_render_queries", test);

		test = (gm->flag & GAME_PLAYER_IGNORE_EXIT_KEY);
		SYS_WriteCommandLineInt(syshandle, "ignore_exit_key", test);
