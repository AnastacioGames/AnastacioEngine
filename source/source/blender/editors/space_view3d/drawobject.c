...
/* calculate a rotation matrix for a view3d_icon (RanGE) so that it is always facing the camera in a 3D rendered scene.
/* The matrix is calculated from the position of the camera (taken from the view matrix),
/* the position of the object/plane and the direction vector from the object to the camera. */
static void calculate_icon_lookAt(float view[4][4], float objPos[3], float *billboardrot) {
	float *viewmat = view;

	// Extract camera position from viewmat matrix
	float camPos[3] = { -viewmat[12], -viewmat[13], -viewmat[14] };
	float direction[3] = {camPos[0] + objPos[0],
						  camPos[1] + objPos[1],
						  camPos[2] + objPos[2]};
	...
	// Calculates the vector from up to the plane
	float up[3] = { viewmat[4], viewmat[5], viewmat[6] };

	// Calculates the right vector for the plane
	float right[3] = {up[1] * direction[2] - up[2] * direction[1],
					  up[2] * direction[0] - up[0] * direction[2],
					  up[0] * direction[1] - up[1] * direction[0]};
	
	// Normalizes the vector of right
	normalize_v3(right);
	...
	// static set the rotation matrix
	billboardrot[0] = -right[0];
	billboardrot[1] = -right[1];
	billboardrot[2] = -right[2];
	billboardrot[3] = 0;
	...
	billboardrot[8] = -direction[0];
	billboardrot[9] = -direction[1];
	billboardrot[10] = -direction[2];
	billboardrot[11] = 0;

	billboardrot[12] = objPos[0];
	billboardrot[13] = objPos[1];
	billboardrot[14] = objPos[2];
	billboardrot[15] = 1;
}

/* Function to draw an image on an Object */
extern ImBuf *view3D_icons[];
static void draw_ob_icon(Main *bmain, Object *ob, const short dflag, const unsigned char ob_wire_col[4], eStereoViews sview, RegionView3D *rv3d, float col[4], draw3d_iconType type)
{
	const float pixsize = ED_view3d_pixel_size(rv3d, ob->obmat[3]);

	// Load image buffer from allocated array view3D_icons
	ImBuf *ibuf = view3D_icons[type];
	...
	/* Calculate the scale center based on object's origin */
	float ofs_x = -0.5f * ima_x;
	float ofs_y = -0.5f * ima_y;
	
	float billboardrot[16];
	calculate_icon_lookAt(rv3d->viewinv, ob->obmat[3], billboardrot);
	/* we first draw only the screen aligned & fixed scale stuff */
	glPushMatrix();
	glLoadMatrixf(rv3d->viewmat);

	glMultMatrixf(billboardrot);
	
	/* Calculate the scale */
	float scale = 0.1f * (pixsize * ((float)U.obcenter_dia * (type != ICON_3D_AREALAMP ? 0.45f : 0.8f)));
	if (scale < 0.003f) {
		scale = 0.003f;
	}
	...
	if (ibuf && ibuf->rect) {
		...
		/* color and alpha */
		if (col) {
			// calculate max/min distance for fade in/out, based on pixsize, I think it's not a good approach ...
			float distance_range = 0.00001f - 0.002f;
			float alpha = fmaxf(0.0f, fminf(1.0f, 1.0f - (pixsize - 0.002f) / distance_range));
...
void drawcircball_col(int mode, const float cent[3], float rad, const float tmat[4][4], const float color[3]) {
	GLfloat oldColor[4];
	glGetFloatv(GL_CURRENT_COLOR, oldColor);

	glColor3f(color[0], color[1], color[2]);
	drawcircball(mode, cent, rad, tmat);
...
static void draw_shadow_volume(const float box[8][3], const float color[3])
{
	const float red = color ? color[0] : 0.2f;
	const float green = color ? color[1] : 0.2f;
	const float blue = color ? color[2] : 0.2f;
	...
	glBlendFunc(GL_ONE, GL_ONE);
	glColor4f(red, green, blue, 0.35f);
...
static void draw_csm_camera_slice(const Object *lampob,
                                  const Object *camob,
                                  const Camera *camera,
                                  const float aspect,
...
{
	float lampinv[4][4];
	float cameraCorner[8][3];
	const float nearHalfWidth = clipnear * camera->sensor_x / (2.0f * camera->lens);
	const float farHalfWidth = clipfar * camera->sensor_x / (2.0f * camera->lens);
	const float nearHalfHeight = nearHalfWidth / aspect;
	const float farHalfHeight = farHalfWidth / aspect;

	/* Camera space looks down -Z.  Keep draw_box()'s historical vertex order. */
	copy_v3_fl3(cameraCorner[0], -farHalfWidth, -farHalfHeight, -clipfar);
	copy_v3_fl3(cameraCorner[3], -farHalfWidth, farHalfHeight, -clipfar);
	copy_v3_fl3(cameraCorner[4], farHalfWidth, -farHalfHeight, -clipfar);
	copy_v3_fl3(cameraCorner[7], farHalfWidth, farHalfHeight, -clipfar);
	copy_v3_fl3(cameraCorner[1], -nearHalfWidth, -nearHalfHeight, -clipnear);
	copy_v3_fl3(cameraCorner[2], -nearHalfWidth, nearHalfHeight, -clipnear);
	copy_v3_fl3(cameraCorner[5], nearHalfWidth, -nearHalfHeight, -clipnear);
	copy_v3_fl3(cameraCorner[6], nearHalfWidth, nearHalfHeight, -clipnear);

	invert_m4_m4(lampinv, lampob->obmat);
	for (int i = 0; i < 8; ++i) {
		float worldCorner[3];
		mul_v3_m4v3(worldCorner, camob->obmat, cameraCorner[i]);
		mul_v3_m4v3(box[i], lampinv, worldCorner);
	}
}

static void draw_transp_sun_shadow_volume(Lamp *la, Object *lampob, Object *camob, Scene *scene)
{
	float box[8][3];
	...
	if (la->shadow_cascade_count != 0 && (la->csm_flag & LA_CSM_DEBUG) &&
	    camob && camob->type == OB_CAMERA && camob->data)
	{
		/* Cascade+Debug together always show the per-cascade camera-slice box,
		 * independent of "Show Shadow Box". The runtime CSM is fit to slices of
		 * the active camera frustum, not to a volume centered on the Sun.
		 * Mirror that geometry in the editor. */
		Camera *camera = camob->data;
		const float cliprange = la->clipend - la->clipsta;
		const float split0 = la->clipsta + la->shadow_cascade_proportion_near * cliprange;
		const float split1 = split0 + la->shadow_cascade_proportion_middle * cliprange;
		const float aspect = ((float)scene->r.xsch * scene->r.xasp) /
		...
		const float ranges[4] = {la->clipsta, split0, split1, la->clipend};

		for (int cascade = 0; cascade < 3; ++cascade) {
			const float nearclip = ranges[cascade];
			const float farclip = ranges[cascade + 1];
			draw_csm_camera_slice(lampob, camob, camera, aspect, nearclip, farclip, box);
			draw_shadow_volume(box, colors[cascade]);
		}
	}
	else if (la->mode & LA_SHOW_SHADOW_BOX) {
		/* Plain shadow box, shown whenever "Show Shadow Box" is on by itself
		 * (cascades off, or cascades on without Debug). */
		draw_shadow_volume(box, NULL);
	}
}
...
static void drawlamp(Main *bmain, Scene *scene, View3D *v3d, RegionView3D *rv3d, Base *base,
                     const char dt, const short dflag, const unsigned char ob_wire_col[4], const bool is_obact)
{
	...
	float lampsize;
	float imat[4][4];
	draw3d_iconType lampIcon = ICON_3D_LAMP;
	...
	const bool drawcone = ((dt > OB_WIRE) &&
	                       ...
	                       !is_view);
	float lampcol[4] = { la->r, la->g, la->b, 1.0f };
	...
	const bool drawshadowbox = (
	        ...
	        ((la->type == LA_SUN) && (la->shadow_cascade_count != 0) &&
	        (la->csm_flag & LA_CSM_DEBUG))) &&
	        !(base->flag & OB_FROMDUPLI) &&
	        !is_view);
#else
...
#ifdef WITH_GAMEENGINE
		if (drawshadowbox) {
			draw_transp_sun_shadow_volume(la, ob, scene->camera, scene);
		}
#endif
	...
	switch (la->type) {
		case LA_SUN:
			...
			break;
		case LA_SPOT:
			lampIcon = ICON_3D_SPOTLAMP;
			break;
		case LA_HEMI:
			lampIcon = ICON_3D_HEMILAMP;
			break;
		case LA_AREA:
			lampIcon = ICON_3D_AREALAMP;
			break;
	}

	draw_ob_icon(bmain, ob, dflag, ob_wire_col, v3d->multiview_eye, rv3d, lampcol, lampIcon);
}
...
static void drawcamera(Main *bmain, Scene *scene, View3D *v3d, RegionView3D *rv3d, Base *base,
                       const short dflag, const unsigned char ob_wire_col[4])
{
	...
	const bool is_selection_camera_stereo = (G.f & G_PICKSEL) &&
	                                        ...
	                                        is_stereo3d_view;
	draw3d_iconType cameraIcon = is_active ? ICON_3D_CAMERA : ICON_3D_CAMERA_INACTIVE;
	...
	/* Range Engine: display the actual clip volume used by camera culling.
	 * It intentionally lives in the editor (unlike Show Frustum, which is a
	 * runtime debug overlay) so artists can inspect it before starting a game. */
#ifdef WITH_GAMEENGINE
	if (cam->gameflag & GAME_CAM_SHOW_CULLING_BOX) {
		float near_plane[4][3], far_plane[4][3];
		const float drawdepth = fabsf(vec[0][2]);
		const bool perspective = (cam->type == CAM_PERSP);

		for (int i = 0; i < 4; ++i) {
			copy_v3_v3(near_plane[i], vec[i]);
			copy_v3_v3(far_plane[i], vec[i]);
			if (perspective) {
				mul_v3_fl(near_plane[i], cam->clipsta / drawdepth);
				mul_v3_fl(far_plane[i], cam->clipend / drawdepth);
			}
			else {
				near_plane[i][2] = -cam->clipsta;
				far_plane[i][2] = -cam->clipend;
			}
		}
	...
	/* draw camera 3D icon */
	float camcol[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
	draw_ob_icon(bmain, ob, dflag, ob_wire_col, v3d->multiview_eye, rv3d, camcol, cameraIcon);
}

/* RANGE ENGINE: EXPERIMENTAL, need to move to another location (like other gizmos BF branch)
 * First OGL 3.2 ToDo. */
static void draw_gizmo_speaker(Object *ob)
{
  Speaker *speaker = (Speaker *)ob->data;
  draw_empty_sphere(speaker->distance_reference);
}
	...
	if (check_object_draw_editweight(me, finalDM)) {
		...
		if (use_occlude_wire) {
			...
			if (draw_glsl_material(scene, ob, v3d, dt)) {
				...
				finalDM->drawMappedFacesGLSL(finalDM, GPU_object_material_bind, false,
	         ...
	         check_object_draw_texture(scene, v3d, dt))
	{
		...
		if (draw_glsl_material(scene, ob, v3d, dt) && !(draw_flags & DRAW_MODIFIERS_PREVIEW)) {
			...
			if ((v3d->flag2 & V3D_SHOW_SOLID_MATCAP) && ob->sculpt && (p = BKE_paint_get_active(scene))) {
				...
				dm->drawFacesGLSL(dm, GPU_object_material_bind, object_is_halo(scene, ob));
...
static bool drawCurveDerivedMesh(Scene *scene, View3D *v3d, RegionView3D *rv3d, Base *base, const char dt)
{
	...
	if (dt > OB_WIRE && dm->getNumPolys(dm)) {
		...
		if (!glsl)
			...
			dm->drawFacesGLSL(dm, GPU_object_material_bind, false);
...
void draw_object(Main *bmain, Scene *scene, ARegion *ar, View3D *v3d, Base *base, const short dflag)
{
	...
	if (ob != scene->obedit) {
		const bool restrict_lod = (!(v3d->flag2 & V3D_LOD_INVISIBLE_SHOW) && ob->currentlod && (ob->currentlod->flags & OB_LOD_USE_INVISIBLE));
		if (ob->restrictflag & OB_RESTRICT_VIEW || restrict_lod)
			return;
		...
		switch (ob->type) {
			...
			case OB_SPEAKER:
				if (!render_override) {
					...
					float camcol[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
					draw_ob_icon(bmain, ob, dflag, ob_wire_col, v3d->multiview_eye, rv3d, camcol, ICON_3D_SPEAKER);
				}
				break;
			case OB_LATTICE:
				if (!render_override) {
	...
	/* draw SCRIPT icon in the object */
	if (ob->components.first && !render_override && (v3d->flag2 & V3D_RENDER_SHOW_COMPONENTS)) {
		float col[4] = {1.0f, 1.0f, 1.0f, 1.0f};
		glDisable(GL_DEPTH_TEST);
		draw_ob_icon(bmain, ob, dflag, ob_wire_col, v3d->multiview_eye, rv3d, col, ICON_3D_SCRIPT);
		glEnable(GL_DEPTH_TEST);
	}
