...
static int buttons_context_path_particle(ButsContextPath *path)
{
	...
	if (buttons_context_path_object(path)) {
		...
		/* GPU particle emitters can live on Empty objects too, not just meshes. */
		if (ob && (ob->type == OB_MESH || ob->type == OB_EMPTY)) {
...
static int buttons_context_path(const bContext *C, ButsContextPath *path, int mainb, int flag)
{
	...
	switch (mainb) {
		...
		case BCONTEXT_VEHICLE:
		{
			...
			if (ob && (ob->gameflag & OB_DYNAMIC) && !(ob->gameflag & OB_SOFT_BODY)) {
				found = buttons_context_path_object(path);
			}
			else {
				found = 0;
