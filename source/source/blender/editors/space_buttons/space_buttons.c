...
static void buttons_main_region_draw(const bContext *C, ARegion *ar)
{
	...
	case BCONTEXT_GAME:
		contexts[0] = "game";
		break;
	case BCONTEXT_VEHICLE:
		contexts[0] = "vehicle";
		break;
	}

	if (contexts[0]) {
		ED_region_panels_ex(C, ar, contexts, sbuts->mainb, vertical);
	}
		...
		case NC_OBJECT:
			switch (wmn->data) {
				...
				case ND_MODIFIER:
					...
					buttons_area_redraw(sa, BCONTEXT_VEHICLE);
					break;
				case ND_CONSTRAINT:
...
void ED_spacetype_buttons(void)
{
	...
	/* Pin/breadcrumb row (BUTTONS_PT_context) removed: unused by this project and
	 * conflicted with the reworked navigation bar/header layout. */
