...
/**
 * Draw the selectable screens as horizontal tabs instead of an ID search field.
 * Screens owned by another window and temporary/fullscreen screens cannot be
 * activated, so keep this list in sync with SCREEN_OT_screen_set.
 */
void uiTemplateScreenTabs(uiLayout *layout, bContext *C)
{
	...
	/* Make the buttons participate in the caller's horizontal layout.  Without
	 * this, their literal (0, 0) coordinates overlap other header controls. */
	...
	for (screen = bmain->screen.first; screen; screen = screen->id.next) {
		...
		/* ED_screen_set() stamps winid on whichever screen is currently active for a
		 * window ("prevent multiwin errors"), so the active screen of *this* window
		 * always has a nonzero winid too -- only exclude screens owned by another
		 * window, not the one we're currently showing. */
		if ((screen->winid != 0 && screen->winid != window->winid) || screen->temp ||
		...
		/* UI_BTYPE_TAB is only fully implemented for RNA enum properties in this
		 * UI version.  A normal button is reliable for operator-backed tabs. */
		but = uiDefIconTextButO(block, UI_BTYPE_BUT, "SCREEN_OT_screen_set", WM_OP_INVOKE_DEFAULT,
		                ICON_SPLITSCREEN, screen->id.name + 2, 0, 0, UI_UNIT_X * 6, UI_UNIT_Y,
		                TIP_("Switch to this screen"));
		opptr = UI_but_operator_ptr_get(but);
		RNA_string_set(opptr, "screen_name", screen->id.name + 2);
...
/*
 * LATE :
 * For Customizable Layer Display .
 */
static void ui_template_layer_layout_ex(uiLayout *layout,
																				PointerRNA *ptr,
																				const char *propname,
																				const int layers_used,
																				const int layers_active,
																				const int column,
																				const int group)
{
	/* Find Property */
	...
	if (length % (column * group) != 0 || length < (column * group)) {
		RNA_warning("on %s.%s length %d is not divisible by %d columm and %d group", RNA_struct_identifier(ptr->type), propname, length, column, group);
		return;
	}

	/* Draw */
	uiLayout *layout_base = uiLayoutRow(layout, true);
	uiLayout *layout_col = uiLayoutColumn(layout_base, true);
	uiBlock *block = uiLayoutGetBlock(layout);

	/* Count Button Index Each Iteration */
	int layer_index = 0;

	/* For Each Column */
	for (int column_i = 0; column_i < column; column_i++) {
		uiLayout *layout_row = uiLayoutRow(layout_col, true);

		/* For Each Group Per Column */
		for (int group_i = 0; group_i < group; group_i++) {
			uiLayout *layout_sub = uiLayoutRow(layout_row, true);

			UI_block_align_begin(block);

			/* For Each Button */
			for (int button_i = 0; button_i < (length / (column * group)); button_i++) {
				const uint layer_flag = 1u << layer_index;

				int icon = ICON_BLANK1;

				if (layers_used & layer_flag) {
					if (layers_active & layer_flag)
						icon = ICON_LAYER_ACTIVE;
					else
						icon = ICON_LAYER_USED;
				}
				...
				if (RNA_property_subtype(prop) == PROP_LAYER_MEMBER) {
					UI_but_func_set(but, handle_layer_buttons, but, POINTER_FROM_INT(layer_index));
				}
...
/*
 * LATE :
 * Similar Argument With `uiTemplateLayers` .
 */
void uiTemplateLayerLayout(uiLayout *layout,
													PointerRNA *ptr,
													const char *propname,
													PointerRNA *used_ptr,
													...
													const int column)
{
	/* Property */
	PropertyRNA *prop = RNA_struct_find_property(ptr, propname);
	PropertyRNA *used_prop = RNA_struct_find_property(used_ptr, used_propname);
	...
	if (!used_prop) {
		RNA_warning("used property not found: %s.%s", RNA_struct_identifier(used_ptr->type), used_propname);
		return;
	}

	/* Initialize */
	const int length = RNA_property_array_length(ptr, prop);
	const uint layers_used = RNA_property_boolean_get_index(used_ptr, used_prop, length);

	ui_template_layer_layout_ex(layout, ptr, "layers", layers_used, layers_active, column, group);
}
 ...
 * For Object .
 */
void uiTemplateLayerLayoutObject(uiLayout *layout,
																bContext *C,
																const int group,
																const int column)
{
	...
	const uint layers_used = v3d->lay_used;
	const uint layers_active = object ? object->lay : 0;

	ui_template_layer_layout_ex(layout, v3d->scenelock ? &sceneptr : &v3dptr, "layers", layers_used, layers_active, column, group);
}
 ...
 * For Bone .
 */
void uiTemplateLayerLayoutBone(uiLayout *layout,
															PointerRNA *ptr,
															const char *propname,
															const int group,
															const int column)
{
	/* Initialize */
	uint layers_used = 0;
	uint layers_active = 0;

	/* If Armature */
	if (ptr->type == &RNA_Armature) {
		bArmature *arm = (bArmature *)ptr->data;

		layers_used = arm->layer_used;

		if (arm->edbo) {
			if (arm->act_edbone) {
				layers_active |= arm->act_edbone->layer;
			}
		}
		else {
			if (arm->act_bone) {
				layers_active |= arm->act_bone->layer;
			}
		}
	}
	else {
		RNA_warning("not an armature: %s", RNA_struct_identifier(ptr->type));
		return;
	}

	ui_template_layer_layout_ex(layout, ptr, propname, layers_used, layers_active, column, group);
}
