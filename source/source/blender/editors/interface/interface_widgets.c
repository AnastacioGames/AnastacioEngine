...
void UI_draw_anti_tria(float x1, float y1, float x2, float y2, float x3, float y3,
                       const float color[4], const float width)
{
	float tri_arr[4][2] = { {x1, y1}, {x3, y3}, {x2, y2}, {x3, y3} };
...
void ui_draw_anti_tria_rect(const rctf *rect, char dir, const float color[4])
{
	...
	if (dir == 'h') {
		...
		UI_draw_anti_tria(rect->xmin + down, rect->ymin, rect->xmin + down, rect->ymax, rect->xmax - 0.5f, rect->ymin + half, color, 2.0f);
	}
	else {
		float half = 0.5f * BLI_rctf_size_x(rect);
		UI_draw_anti_tria(rect->xmin, rect->ymax - down, rect->xmax, rect->ymax - down, rect->xmin + half, rect->ymin + 0.5f, color, 2.0f);
	}
}
...
void UI_draw_anti_fan(float tri_array[][2], unsigned int length, const float color[4])
{
	...
	glColor4fv(draw_color);

	/* for each AA step */
	for (int j = 0; j < WIDGET_AA_JITTER; j++) {
		glBegin(GL_TRIANGLE_FAN);
		glVertex2f(tri_array[0][0], tri_array[0][1]);
		glVertex2f(tri_array[1][0], tri_array[1][1]);

		/* We jitter only the middle of the fan, the extremes are pinned. */
		for (int i = 2; i < length - 1; i++) {
			glVertex2f(tri_array[i][0] + jit[j][0], tri_array[i][1] + jit[j][1]);
		}

		glVertex2f(tri_array[length - 1][0], tri_array[length - 1][1]);
		glEnd();
	}
...
static void widget_draw_icon(
        ...
        const rcti *rect, char mono_color[4])
{
	...
	/* Shrink the icon (by inflating aspect) when the button itself is
	 * smaller than the default icon size, so it never draws past its own
	 * rect (e.g. the small toggle buttons in the header layers grid). */
	{
		const float max_dim = (float)min_ii(BLI_rcti_size_x(rect), BLI_rcti_size_y(rect));
		if (max_dim > 0.0f && height > max_dim) {
			aspect *= height / max_dim;
			height = max_dim;
		}
	}
...
static void widget_state_pulldown(uiWidgetType *wt, int state, int UNUSED(drawflag))
{
	wt->wcol = *(wt->wcol_theme);
...
static void widget_swatch(uiBut *but, uiWidgetColors *wcol, rcti *rect, int state, int roundboxalign)
{
	...
	if ((state & (UI_BUT_ANIMATED | UI_BUT_ANIMATED_KEY | UI_BUT_DRIVEN | UI_BUT_REDALERT)) || //UI_BUT_OVERRIDEN
        ...
        const float mval_origin[2])
{
	...
	/* Extracted from 'widget_menu_back', keep separate to avoid menu changes breaking popovers */
	{
		...
		widget_softshadow(rect, roundboxalign, 0.125f * U.widget_unit);

		round_box_edges(&wtb, roundboxalign, rect, 0.125f * U.widget_unit);
		wtb.draw_emboss = false;
		widgetbase_draw(&wtb, wcol);
	}
	...
	if (ELEM(direction, UI_DIR_UP, UI_DIR_DOWN)) {
		...
		if (direction == UI_DIR_DOWN) {
			...
			glVertex2f(cent_x - unit_half, y);
			glVertex2f(cent_x + unit_half, y);
			glVertex2f(cent_x, y + unit_half);
		}
		else {
			const float y = rect->ymin;
			glVertex2f(cent_x - unit_half, y);
			glVertex2f(cent_x + unit_half, rect->ymin);
			glVertex2f(cent_x, y - unit_half);
		}
		glEnd();
	}
