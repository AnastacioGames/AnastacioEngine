...
static void ui_linkline_add(ListBase *listb, uiBut *but, uiBut *bt, short deactive, char color[3])
{
	uiLinkLine *line;
	...
	copy_v3_v3_char(line->color, color);
}
					...
					for (a = 0; a < *(link->totlink); a++) {
						...
						if (bt) {
							if ((but->flag & UI_BUT_SCA_LINK_GREY) || (bt->flag & UI_BUT_SCA_LINK_GREY)) {
								ui_linkline_add(&link->lines, but, bt, true, link->color);
							}
							else {
								ui_linkline_add(&link->lines, but, bt, false, link->color);
							}
...
void UI_but_link_set(uiBut *but, void **poin, void ***ppoin, short *tot, int from, int to, char color[3])
{
	uiLink *link;
	...
	link->fromcode = from;
	link->tocode = to;
	copy_v3_v3_char(link->color, color);
}
...
void UI_block_event_func_set(
    uiBlock *block, int (*func)(const bContext *, uiBlock *, const wmEvent *))
{
  block->block_event_func = func;
}
