...
void UI_ColorPtrBlendShade3ubv(const uchar cp1[3], const uchar cp2[3], float fac, int offset)
{
	int r, g, b;
	CLAMP(fac, 0.0f, 1.0f);
	r = offset + floorf((1.0f - fac) * cp1[0] + fac * cp2[0]);
...
bool UI_GetIconThemeColor4fv(int colorid, float col[4])
{
	...
	// Range: Change ugly SPACE_BUTS name to SPACE_PROPERTIES
	if (!((theme_spacetype == SPACE_OUTLINER && theme_regionid == RGN_TYPE_WINDOW) ||
        (theme_spacetype == SPACE_BUTS && theme_regionid == RGN_TYPE_NAV_BAR) ||
