...
#define DEF_ICON_COLOR(name) {ICON_TYPE_TEXTURE, 0},
...
#define INIT_MATCAP_ICON(icon_id, name)                                       \
	{                                                                         \
		...
		DrawInfo *di;                                                         \
		                                                                      \
		di = def_internal_icon(NULL, icon_id, 0, 0, 96, ICON_TYPE_BUFFER, 0);   \
		di->data.buffer.image->datatoc_rect = rect;                           \
		di->data.buffer.image->datatoc_size = size;                           \
	} (void)0
...
static ImBuf *load_external_icon_atlas(const char *filename, const int min_width, const int min_height)
{
	...
	if (bbuf->x < min_width || bbuf->y < min_height) {
		printf("External icon atlas '%s' is too small (%dx%d); using built-in icons\n",
		       iconfilestr, bbuf->x, bbuf->y);
		IMB_freeImBuf(bbuf);
		return NULL;
	}
...
//	bTheme *btheme = UI_GetTheme();
	...
	b32buf = load_external_icon_atlas("blender_icons32.png", 1204, 1280);
	if ((b16buf == NULL) != (b32buf == NULL)) {
		if (b16buf)
...
static void icon_draw_size(
        ...
        enum eIconSizes size, int draw_size, const bool UNUSED(nocreate), const bool is_preview, const char mono_rgba[4])
{
	bTheme *btheme = UI_GetTheme();
	Icon *icon = NULL;
	DrawInfo *di = NULL;
	...
	else if (di->type == ICON_TYPE_MONO_TEXTURE) { /* Range: from new imm mode, converted to GL calls, low level. */
...
static void icon_draw_at_size(
        ...
        enum eIconSizes size, const bool nocreate, const char mono_color[4])
{
	int draw_size = get_draw_size(size);
	icon_draw_size(x, y, icon_id, aspect, alpha, NULL, size, draw_size, nocreate, false, mono_color);
}

void UI_icon_draw_aspect(float x, float y, int icon_id, float aspect, float alpha, const char mono_color[4])
{
	icon_draw_at_size(x, y, icon_id, aspect, alpha, ICON_SIZE_ICON, 0, mono_color);
}
...
void UI_icon_draw_aspect_color(float x, float y, int icon_id, float aspect, const float rgb[3], const char mono_color[4])
{
	...
	icon_draw_size(x, y, icon_id, aspect, 1.0f, rgb, ICON_SIZE_ICON, draw_size, false, false, mono_color);
}
...
{
	icon_draw_size(x, y, icon_id, 1.0f, alpha, NULL, ICON_SIZE_ICON, size, true, false, NULL);
}

void UI_icon_draw_preview(float x, float y, int icon_id)
{
	icon_draw_at_size(x, y, icon_id, 1.0f, 1.0f, ICON_SIZE_PREVIEW, 0, NULL);
}

void UI_icon_draw_preview_aspect(float x, float y, int icon_id, float aspect)
{
	icon_draw_at_size(x, y, icon_id, aspect, 1.0f, ICON_SIZE_PREVIEW, 0, NULL);
}

void UI_icon_draw_preview_aspect_size(float x, float y, int icon_id, float aspect, float alpha, int size)
{
	icon_draw_size(x, y, icon_id, aspect, alpha, NULL, ICON_SIZE_PREVIEW, size, false, true, NULL);
}
