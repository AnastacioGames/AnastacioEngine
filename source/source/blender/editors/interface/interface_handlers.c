...
static void ui_but_smart_controller_add(bContext *C, uiBut *from, uiBut *to)
{
	...
	if (WM_operator_name_call(C, "LOGIC_OT_controller_add", WM_OP_EXEC_DEFAULT, &props_ptr) & OPERATOR_FINISHED) {
		...
		UI_but_link_set(
		        ...
		        &cont->totlinks, from->link->tocode, (int)to->hardmin, cont->color);
...
static uiBut *ui_but_link_target_find(ARegion *ar, uiBut *but, int x, int y)
{
	...
	for (bt = but->block->buttons.first; bt; bt = bt->next) {
		...
		dx = (mx < bt->rect.xmin) ? bt->rect.xmin - mx :
		     (mx > bt->rect.xmax) ? mx - bt->rect.xmax : 0.0f;
...
{
	ARegion *ar = CTX_wm_region(C);
	uiBut *bt = ui_but_link_target_find(
	        ar, but, but->linkto[0] + ar->winrct.xmin, but->linkto[1] + ar->winrct.ymin);
	if (bt && bt != but) {
