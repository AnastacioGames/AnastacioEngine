...
void bezt_remap_times(KeyframeEditData *ked, BezTriple *bezt)
{
	...
	const float old_range = rmap->oldMax - rmap->oldMin;
	float scale;

	if (old_range == 0.0f) {
		/* degenerate source range: nothing to rescale against, leave handles untouched */
		return;
	}

	scale = (rmap->newMax - rmap->newMin) / old_range;
