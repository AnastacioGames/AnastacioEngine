        ...
        bool (*operation_cb)(bContext *, eOutliner_PropSceneOps, TreeElement *, TreeStoreElem *))
{
	...
	/* Recurse like set_operation_types() does -- scenes now live one level
	 * down, nested under the "Scenes" visual root, not as siblings of it. */
	for (te = lb->first; te; te = te->next) {
		tselem = TREESTORE(te);
		if (tselem->flag & TSE_SELECTED) {
		...
		if (outliner_do_scene_operation(C, event, &te->subtree, operation_cb)) {
			success = true;
		}
	}
...
static int outliner_scene_operation_exec(bContext *C, wmOperator *op)
{
	...
	if (outliner_do_scene_operation(C, event, &soops->tree, scene_cb) == false) {
		/* ED_screen_delete_scene() refuses to delete the last remaining scene
		 * and returns false silently -- report why so it doesn't look broken. */
		if (event == OL_SCENE_OP_DELETE) {
			BKE_report(op->reports, RPT_WARNING, "Cannot delete the last scene");
		}
...
/* **************************************** */
/* Open External Library File (LibLoad visualizer, "External Files" section) */

static int outliner_open_library_external_exec(bContext *C, wmOperator *op)
{
	Main *bmain = CTX_data_main(C);
	...
	BLI_strncpy(expanded, filepath, sizeof(expanded));
	BLI_path_abs(expanded, BKE_main_blendfile_path(bmain));

	if (!BLI_exists(expanded)) {
		BKE_reportf(op->reports, RPT_ERROR, "External file '%s' does not exist", expanded);
		return OPERATOR_CANCELLED;
	}
...
#ifdef WIN32
	{
		...
		wchar_t program_path_w[FILE_MAX];
		wchar_t expanded_w[FILE_MAX];
		wchar_t cmdline_w[FILE_MAX * 2 + 8];
		STARTUPINFOW startup_info;
		PROCESS_INFORMATION process_info;

		BLI_strncpy_wchar_from_utf8(program_path_w, program_path, ARRAY_SIZE(program_path_w));
		BLI_strncpy_wchar_from_utf8(expanded_w, expanded, ARRAY_SIZE(expanded_w));

		swprintf_s(cmdline_w, ARRAY_SIZE(cmdline_w), L"\"%ls\" \"%ls\"", program_path_w, expanded_w);

		memset(&startup_info, 0, sizeof(startup_info));
		startup_info.cb = sizeof(startup_info);
		memset(&process_info, 0, sizeof(process_info));

		if (CreateProcessW(program_path_w, cmdline_w, NULL, NULL, FALSE, 0, NULL, NULL, &startup_info, &process_info)) {
			CloseHandle(process_info.hProcess);
			CloseHandle(process_info.hThread);
		}
		else {
			BKE_reportf(op->reports, RPT_ERROR, "Failed to launch a new instance for '%s'", expanded);
			return OPERATOR_CANCELLED;
		}
	}
#else
	BKE_report(op->reports, RPT_ERROR, "Opening external files in a new instance is only supported on Windows");
	return OPERATOR_CANCELLED;
#endif
...
void OUTLINER_OT_open_library_external(wmOperatorType *ot)
{
	/* identifiers */
	ot->name = "Open External File";
	ot->idname = "OUTLINER_OT_open_library_external";
	ot->description = "Open this external library file in a new instance of the engine";

	/* callbacks */
	ot->exec = outliner_open_library_external_exec;
	ot->poll = ED_operator_outliner_active;

	ot->flag = 0;

	RNA_def_string_file_path(ot->srna, "filepath", NULL, FILE_MAX, "File Path", "Path of the external file to open");
}
