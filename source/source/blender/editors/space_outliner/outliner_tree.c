...
TreeElement *outliner_find_id(SpaceOops *soops, ListBase *lb, const ID *id)
{
	for (TreeElement *te = lb->first; te; te = te->next) {
		...
		else if (tselem->type == TSE_ID_BASE) {
			/* Custom visual roots (for example "Scenes") contain real IDs. */
			TreeElement *tes = outliner_find_id(soops, &te->subtree, id);
			if (tes) {
				return tes;
			}
...
/* True when the library was explicitly registered through "Link to LibLoad".
 * New registrations have a fake user on the Library itself. The Object fallback
 * preserves entries created before the Library marker existed; it deliberately
 * ignores data-block users, because a Group legitimately owns its member Objects. */
static bool outliner_library_is_libload(Main *mainvar, Library *lib)
{
	if (ID_FAKE_USERS(&lib->id)) {
		return true;
	}

	for (Object *ob = mainvar->object.first; ob; ob = ob->id.next) {
		if (ob->id.lib == lib && ID_FAKE_USERS(&ob->id) && ID_REAL_USERS(&ob->id) == 0) {
			return true;
		}
	}
...
/* LibLoad links a selected Object or Group, so the external Text datablocks
 * are not present in Main. Read only their names from the source file: this
 * keeps scripts visible without loading, copying, or changing them. */
static bool outliner_libload_text_is_available(Main *mainvar, const char *name)
{
	/* A linked Text with the same name still is not a runtime copy belonging to
	 * this project.  Only a local Text satisfies the component import. */
	for (ID *text = mainvar->text.first; text; text = text->next) {
		if (text->lib == NULL && STREQ(text->name + 2, name)) {
			return true;
		}
	}
...
static void outliner_add_libload_texts(Main *mainvar, SpaceOops *soops, TreeElement *parent, Library *lib)
{
	...
	blendhandle = BLO_blendhandle_from_file(lib->filepath, NULL);
	if (blendhandle == NULL) {
		return;
	}

	names = BLO_blendhandle_get_datablock_names(blendhandle, ID_TXT, &tottexts);
	BLO_blendhandle_close(blendhandle);

	if (names) {
		TreeElement *texts = outliner_add_element(soops, &parent->subtree, lib, parent, TSE_ID_BASE, 4);
		texts->name = BKE_idcode_to_name_plural(ID_TXT);
		texts->directdata = POINTER_FROM_INT(ICON_SCRIPT);
		texts->flag |= TE_DIRECTDATA_ICON;

		for (LinkNode *node = names; node; node = node->next, index++) {
			TreeElement *text = outliner_add_element(soops, &texts->subtree, lib, texts, TSE_ID_BASE, index);
			bool is_available;
			text->name = BLI_strdup((const char *)node->link);
			is_available = outliner_libload_text_is_available(mainvar, text->name);
			text->directdata = POINTER_FROM_INT(
		        is_available ? ICON_SCRIPT : ICON_LIBRARY_DATA_BROKEN);
			text->flag |= TE_DIRECTDATA_ICON | TE_FREE_NAME;
			if (!is_available) {
				text->flag |= TE_LIBLOAD_TEXT_IMPORT;
			}
		}
	}
...
void outliner_build_tree(Main *mainvar, Scene *scene, SpaceOops *soops)
{
	...
	if (soops->outlinevis == SO_LIBRARIES) {
		...
		Scene *sce;
		/*
		 * "External Files" lists the real Library IDs linked into this file
		 * (Main.library), so it can be expanded/opened like any other ID tree
		 * entry. nr=1 keeps it from colliding with scenes_root's (TSE_ID_BASE,
		 * mainvar, nr=0) persistence key in check_persistent() -- sharing that
		 * key previously made the two roots' open/closed and selection state
		 * bleed into each other across tree rebuilds, breaking outliner delete.
		 */
		TreeElement *ext_root = outliner_add_element(soops, &soops->tree, mainvar, NULL, TSE_ID_BASE, 1);
		ext_root->name = IFACE_("External Files");
		ext_root->directdata = POINTER_FROM_INT(ICON_LIBRARY_DATA_DIRECT);
		ext_root->flag |= TE_DIRECTDATA_ICON;
		/* Only force-open on first creation -- unconditionally clearing this on every
		 * rebuild undid the user's own collapse click on the very next redraw. */
		if (!TREESTORE(ext_root)->used)
			TREESTORE(ext_root)->flag &= ~TSE_CLOSED;

		for (Library *ext_lib = mainvar->library.first; ext_lib; ext_lib = ext_lib->id.next) {
			/* Only show libraries explicitly registered through the '+' button.
			 * Regular File > Link and Append operations never mark the Library,
			 * so they remain visible only in the standard Blender File tree. */
			if (!outliner_library_is_libload(mainvar, ext_lib)) {
				continue;
			}
			TreeElement *ext_item = outliner_add_element(soops, &ext_root->subtree, ext_lib, ext_root, 0, 0);

			/* Expanding the library entry reveals its absolute file path, the
			 * same way switching the outliner filter to "Blender File" shows
			 * each library's path -- gives this row a disclosure triangle
			 * even though outliner_add_id_contents() is skipped for entries
			 * parented under a TSE_ID_BASE (see outliner_add_element() above). */
			TreeElement *ext_path = outliner_add_element(soops, &ext_item->subtree, ext_lib, ext_item, TSE_ID_BASE, 2);
			ext_path->name = ext_lib->filepath;
			ext_path->directdata = POINTER_FROM_INT(ICON_FILE_BLEND);
			ext_path->flag |= TE_DIRECTDATA_ICON;

			/* Keep optional metadata below the path itself.  The path then owns
			 * a disclosure triangle, while its "texts" category remains
			 * independently collapsible. */
			outliner_add_libload_texts(mainvar, soops, ext_path, ext_lib);
		}

		/* nr=3 gives this root a persistence key distinct from every other
		 * (TSE_ID_BASE, mainvar, nr) root -- lets outliner_draw.c recognize it
		 * unambiguously to draw the "add scene" button (see ext_root above). */
		TreeElement *scenes_root = outliner_add_element(soops, &soops->tree, mainvar, NULL, TSE_ID_BASE, 3);
		scenes_root->name = IFACE_("Scenes");
		scenes_root->directdata = POINTER_FROM_INT(ICON_SCENE);
