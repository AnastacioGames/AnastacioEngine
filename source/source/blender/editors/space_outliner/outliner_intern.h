...
/* directdata holds an icon constant (POINTER_FROM_INT), not the ListBase* that
 * TSE_ID_BASE library-content headers normally store there -- without this flag
 * tselem_draw_icon() cannot tell the two apart and would draw an icon from a
 * truncated pointer for the ListBase case. */
#define TE_DIRECTDATA_ICON  16
/* External Files Text absent from the current Main.  The row owns an explicit
 * import button, while available Text rows remain plain informational items. */
#define TE_LIBLOAD_TEXT_IMPORT  32
...
void OUTLINER_OT_open_library_external(struct wmOperatorType *ot);
