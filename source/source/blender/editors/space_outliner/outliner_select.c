        ...
        TreeElement *te, const eOLSetState set)
{
	...
	if (set != OL_SETSEL_NONE) {
		...
		/* World isn't an object: clear object selection AND active object, so the
		 * outliner/properties and the 3D viewport agree on what's active. Leaving
		 * scene->basact set keeps buttons_context_path_object() finding a stale
		 * active object, which keeps Object/Data/Material/etc. tabs visible in the
		 * Properties editor even though nothing is selected anymore. */
		BKE_scene_base_deselect_all(scene);
		scene->basact = NULL;
		WM_event_add_notifier(C, NC_SCENE | ND_OB_SELECT, scene);
