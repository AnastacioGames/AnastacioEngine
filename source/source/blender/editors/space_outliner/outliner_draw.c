...
static void tselem_draw_icon_uibut(struct DrawIconArg *arg, int icon)
{
	...
	if (arg->x >= arg->xmax) {
		glEnable(GL_BLEND);
		UI_icon_draw_aspect(arg->x, arg->y, icon, 1.0f / UI_DPI_FAC, arg->alpha, NULL);
		glDisable(GL_BLEND);
	}
	else {
                             ...
                             float alpha)
{
	...
	if (tselem->type) {
		switch (tselem->type) {
			case TSE_ID_BASE:
				/* directdata is a ListBase* (not an icon) for the vanilla per-type library-content
				 * and orphaned-datablock headers -- only trust it as an icon when explicitly flagged
				 * (see TE_DIRECTDATA_ICON), otherwise POINTER_AS_INT() on that pointer would hand
				 * UI_icon_draw() a garbage icon id. */
				UI_icon_draw(x, y, (te->flag & TE_DIRECTDATA_ICON) ? POINTER_AS_INT(te->directdata) : ICON_DOT);
				break;
			case TSE_ANIM_DATA:
        ...
        TreeElement *te, int startx, int *starty, TreeElement **te_edit)
{
	...
	if (*starty + 2 * UI_UNIT_Y >= ar->v2d.cur.ymin && *starty <= ar->v2d.cur.ymax) {
		...
		int action_x = (int)(ar->v2d.cur.xmax - UI_UNIT_X);
		...
		/* "External Files" root (outliner_tree.c's ext_root, TSE_ID_BASE with nr == 1):
		 * button to register a library's data-block address in bmain without instantiating
		 * it in any scene (see WM_OT_link_to_libload) -- runtime addObject() finds it later. */
		if (tselem->type == TSE_ID_BASE && tselem->nr == 1) {
			uiDefIconButO(
			        block, UI_BTYPE_BUT, "WM_OT_link_to_libload", WM_OP_INVOKE_DEFAULT, ICON_ZOOMIN,
			        action_x, *starty, UI_UNIT_X, UI_UNIT_Y,
			        TIP_("Register a library's data-block in this file without instantiating it in any scene"));
		}

		/* A broken Text below External Files is only metadata until explicitly
		 * imported. Keep the action on that one row, so unrelated scripts from
		 * the same library are never copied implicitly. */
		if (te->flag & TE_LIBLOAD_TEXT_IMPORT) {
			uiBut *import_but = uiDefIconButO(
			        block, UI_BTYPE_BUT, "WM_OT_import_libload_text", WM_OP_EXEC_DEFAULT, ICON_IMPORT,
			        action_x, *starty, UI_UNIT_X, UI_UNIT_Y,
			        TIP_("Import this Text into the current file"));
			PointerRNA *import_ptr = UI_but_operator_ptr_get(import_but);
			RNA_string_set(import_ptr, "filepath", ((Library *)tselem->id)->filepath);
			RNA_string_set(import_ptr, "text", te->name);
		}

		/* "Scenes" root (outliner_tree.c's scenes_root, TSE_ID_BASE with nr == 3):
		 * button pops up a menu of scene.new's creation types (New, Copy Settings,
		 * Link Objects, Link Object Data, Full Copy). Deleting a scene is done via
		 * right-click on the scene entry itself (outliner's existing delete menu). */
		if (tselem->type == TSE_ID_BASE && tselem->nr == 3) {
			uiBut *addbut = uiDefIconButO(
			        block, UI_BTYPE_BUT, "WM_OT_call_menu", WM_OP_EXEC_DEFAULT, ICON_ZOOMIN,
			        action_x, *starty, UI_UNIT_X, UI_UNIT_Y,
			        TIP_("Add a new scene"));
			PointerRNA *addptr = UI_but_operator_ptr_get(addbut);
			RNA_string_set(addptr, "name", "OUTLINER_MT_scene_add");
		}
