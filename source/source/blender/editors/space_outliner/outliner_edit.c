...
/*
 * A library entry listed directly under the "External Files" root
 * (outliner_tree.c's ext_root, TSE_ID_BASE with nr == 1 -- distinct from
 * scenes_root's nr == 0, see outliner_tree.c for why) opens in a new engine
 * instance on double-click instead of entering the usual libpath rename.
 */
static bool do_outliner_item_rename_is_external_file_entry(TreeElement *te, TreeStoreElem *tselem)
{
	if (tselem->type != 0 || te->idcode != ID_LI || !te->parent) {
		return false;
	}

	TreeStoreElem *tsep = TREESTORE(te->parent);
	return (tsep->type == TSE_ID_BASE && tsep->nr == 1);
}

static void do_outliner_item_open_external_file(bContext *C, TreeStoreElem *tselem)
{
	Library *lib = (Library *)tselem->id;
	wmOperatorType *ot = WM_operatortype_find("OUTLINER_OT_open_library_external", false);
	PointerRNA ptr;

	WM_operator_properties_create_ptr(&ptr, ot);
	RNA_string_set(&ptr, "filepath", lib->filepath);
	WM_operator_name_call_ptr(C, ot, WM_OP_EXEC_DEFAULT, &ptr);
	WM_operator_properties_free(&ptr);
}

static int do_outliner_item_rename(bContext *C, ReportList *reports, ARegion *ar, TreeElement *te, const float mval[2])
{
	if (mval[1] > te->ys && mval[1] < te->ys + UI_UNIT_Y) {
		TreeStoreElem *tselem = TREESTORE(te);
		...
		if (mval[0] > te->xs + UI_UNIT_X * 2 && mval[0] < te->xend) {
			if (do_outliner_item_rename_is_external_file_entry(te, tselem)) {
				do_outliner_item_open_external_file(C, tselem);
			}
			else {
				do_item_rename(ar, te, tselem, reports);
			}
	...
	for (te = te->subtree.first; te; te = te->next) {
		if (do_outliner_item_rename(C, reports, ar, te, mval)) return 1;
	}
	return 0;
}
...
static int outliner_item_rename(bContext *C, wmOperator *op, const wmEvent *event)
{
	...
	for (te = soops->tree.first; te; te = te->next) {
		if (do_outliner_item_rename(C, op->reports, ar, te, fmval)) {
			changed = true;
			break;
		}
	}
...
static int outliner_show_active_exec(bContext *C, wmOperator *UNUSED(op))
{
	...
	if (te == NULL) {
		printf("Outliner Show Active: active object '%s' was not found in the tree\n", obact->id.name + 2);
	}
