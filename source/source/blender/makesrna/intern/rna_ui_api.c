...
void RNA_api_ui_layout(StructRNA *srna)
{
	...
	func = RNA_def_function(srna, "template_screen_tabs", "uiTemplateScreenTabs");
	RNA_def_function_flag(func, FUNC_USE_CONTEXT);
	RNA_def_function_ui_description(func, "Inserts the available screens as horizontal tabs");
	...
	func = RNA_def_function(srna, "template_layer_layout", "uiTemplateLayerLayout");
	api_ui_item_rna_common(func);
	parm = RNA_def_pointer(func, "used_layers_data", "AnyType", "", "Data from which to take property");
	RNA_def_parameter_flags(parm, 0, PARM_REQUIRED | PARM_RNAPTR);
	parm = RNA_def_string(func, "used_layers_property", NULL, 0, "", "Identifier of property in data");
	...
	parm = RNA_def_int(func, "active_layer", 0, 0, INT_MAX, "Active Layer", "", 0, INT_MAX);
	RNA_def_parameter_flags(parm, 0, PARM_REQUIRED);
	parm = RNA_def_int(func, "group", 1, 1, INT_MAX, "Group Per Column", "How many group the column would be divided to", 1, INT_MAX);
	parm = RNA_def_int(func, "column", 1, 1, INT_MAX, "Column", "How many column the layer would be drawn on", 1, INT_MAX);

	func = RNA_def_function(srna, "template_layer_layout_object", "uiTemplateLayerLayoutObject");
	RNA_def_function_flag(func, FUNC_USE_CONTEXT);
	parm = RNA_def_int(func, "group", 1, 1, INT_MAX, "Group Per Column", "How many group the column would be divided to", 1, INT_MAX);
	parm = RNA_def_int(func, "column", 1, 1, INT_MAX, "Column", "How many column the layer would be drawn on", 1, INT_MAX);

	func = RNA_def_function(srna, "template_layer_layout_bone", "uiTemplateLayerLayoutBone");
	api_ui_item_rna_common(func);
	parm = RNA_def_int(func, "group", 1, 1, INT_MAX, "Group Per Column", "How many group the column would be divided to", 1, INT_MAX);
	parm = RNA_def_int(func, "column", 1, 1, INT_MAX, "Column", "How many column the layer would be drawn on", 1, INT_MAX);
	...
	RNA_def_int(func, "rows", 9, 1, INT_MAX, "", "Maximum number of items to show", 1, INT_MAX);
