...
static const EnumPropertyItem gearbox_type_items[] = {
	{OB_GEARBOX_AUTOMATIC, "AUTOMATIC", 0, "Automatic", "Game logic shifts gears by RPM threshold"},
	{OB_GEARBOX_MANUAL, "MANUAL", 0, "Manual", "Player shifts gears by key input"},
	{0, NULL, 0, NULL, NULL}
};
...
static void rna_GameObjectSettings_is_vehicle_set(PointerRNA *ptr, bool value)
{
	Object *ob = ptr->data;
	const bool was_vehicle = (ob->gameflag2 & OB_VEHICLE) != 0;

	if (value) {
		ob->gameflag2 |= OB_VEHICLE;

		/* A newly enabled chassis has no wheel setup yet.  These values match
		 * the playable component's legacy 3200 engine-force behavior
		 * (320 Nm * scale 10) and its RPM fallback. */
		if (!was_vehicle && ob->vehicle_wheels.first == NULL &&
		    ob->vehicle_max_torque == 0.0f && ob->vehicle_max_rpm == 0.0f) {
			ob->vehicle_max_torque = 320.0f;
			ob->vehicle_max_rpm = 6000.0f;
		}
	}
	else {
		ob->gameflag2 &= ~OB_VEHICLE;
...
static void rna_Object_use_gpu_particles_update(Main *UNUSED(bmain), Scene *UNUSED(scene), PointerRNA *ptr)
{
	...
	if (ob->gameflag2 & OB_GPU_PARTICLES) {
		if (!BKE_bproperty_object_get(ob, "GPU_Particles_Enabled")) {
			...
			prop->data = 1; /* default: emissao ativa */
...
static void rna_GPUParticles_use_size_curve_update(Main *UNUSED(bmain), Scene *UNUSED(scene), PointerRNA *ptr)
{
	RangeGPUParticleSettings *gp = (RangeGPUParticleSettings *)ptr->data;

	if (gp->use_size_curve && !gp->size_curve) {
		gp->size_curve = curvemapping_add(1, 0.0f, 0.0f, 1.0f, 1.0f);
		curvemapping_initialize(gp->size_curve);
	}
}

static void rna_GPUParticles_use_color_curve_update(Main *UNUSED(bmain), Scene *UNUSED(scene), PointerRNA *ptr)
{
	RangeGPUParticleSettings *gp = (RangeGPUParticleSettings *)ptr->data;

	if (gp->use_color_curve && !gp->color_curve) {
		gp->color_curve = curvemapping_add(4, 0.0f, 0.0f, 1.0f, 1.0f);
		curvemapping_initialize(gp->color_curve);
	}
}
...
static void rna_def_object_gpu_particles(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	static const EnumPropertyItem rna_enum_gpu_particle_billboard_mode_items[] = {
		{GPU_PARTICLE_BILLBOARD_CAMERA_FACING, "CAMERA_FACING", 0, "Camera Facing", "Quad always faces the camera fully (default)"},
		{GPU_PARTICLE_BILLBOARD_VERTICAL, "VERTICAL", 0, "Vertical", "Quad stays locked to world Z, only rotates horizontally to face the camera (smoke, fire, grass)"},
		{GPU_PARTICLE_BILLBOARD_HORIZONTAL, "HORIZONTAL", 0, "Horizontal (Facing Down)", "Quad lies flat in world space, always facing down -- clouds seen from below"},
		{0, NULL, 0, NULL, NULL}
	};

	static const EnumPropertyItem rna_enum_gpu_particle_blend_mode_items[] = {
		{GPU_PARTICLE_BLEND_ALPHA, "ALPHA", 0, "Alpha", "Standard alpha blending (opaque smoke/dust)"},
		{GPU_PARTICLE_BLEND_ADDITIVE, "ADDITIVE", 0, "Additive", "Add particle color to the scene (glowing fire/sparks/electricity)"},
		{0, NULL, 0, NULL, NULL}
	};

	static const EnumPropertyItem rna_enum_gpu_particle_collision_mode_items[] = {
		{GPU_PARTICLE_COLLISION_NONE, "NONE", 0, "None", "Particles ignore collision, passing through geometry (default)"},
		{GPU_PARTICLE_COLLISION_GROUND, "GROUND", 0, "Ground Plane", "Bounce/stop against an analytic flat ground at a fixed height"},
		{GPU_PARTICLE_COLLISION_DEPTH, "DEPTH", 0, "Screen-Space", "Bounce/stop against visible scene geometry using the depth buffer (1 frame delayed, needs Depth Transparency active in the scene)"},
		{0, NULL, 0, NULL, NULL}
	};
	...
	RNA_def_struct_nested(brna, srna, "Object");
	RNA_def_struct_ui_text(srna, "GPU Particle Settings",
	                        "Settings used to seed this object's GPU particle emitter at game start");

	prop = RNA_def_property(srna, "gravity", PROP_FLOAT, PROP_XYZ);
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_text(prop, "Gravity", "Acceleration applied to particles every frame");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "lifetime", PROP_FLOAT, PROP_TIME);
	RNA_def_property_range(prop, 0.01f, 1000.0f);
	RNA_def_property_ui_text(prop, "Lifetime", "Time in seconds a particle lives before respawning");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "emitter_position", PROP_FLOAT, PROP_XYZ);
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_text(prop, "Emitter Position", "Local-space offset of the particle emitter from the object");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "emitter_radius", PROP_FLOAT, PROP_DISTANCE);
	RNA_def_property_range(prop, 0.0f, 1000.0f);
	RNA_def_property_ui_text(prop, "Emitter Radius", "Radius of the spherical emission volume");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "velocity", PROP_FLOAT, PROP_VELOCITY);
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_text(prop, "Velocity", "Base emission velocity of new particles");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "velocity_randomness", PROP_FLOAT, PROP_NONE);
	RNA_def_property_range(prop, 0.0f, 100.0f);
	RNA_def_property_ui_text(prop, "Velocity Randomness", "Random variation added to the base velocity");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "size", PROP_FLOAT, PROP_DISTANCE);
	RNA_def_property_range(prop, 0.001f, 1000.0f);
	RNA_def_property_ui_text(prop, "Size", "Billboard size of a particle at birth");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "color", PROP_FLOAT, PROP_COLOR);
	RNA_def_property_array(prop, 4);
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_ui_text(prop, "Color", "Particle color at birth");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "end_color", PROP_FLOAT, PROP_COLOR);
	RNA_def_property_array(prop, 4);
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_ui_text(prop, "End Color", "Particle color at end of life");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "end_size", PROP_FLOAT, PROP_DISTANCE);
	RNA_def_property_range(prop, 0.0f, 1000.0f);
	RNA_def_property_ui_text(prop, "End Size", "Billboard size of a particle at end of life");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "particle_count", PROP_INT, PROP_NONE);
	RNA_def_property_range(prop, 1, 100000);
	RNA_def_property_ui_text(prop, "Particle Count", "Number of particles in the pool");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "texture", PROP_STRING, PROP_FILEPATH);
	RNA_def_property_string_sdna(prop, NULL, "texture_path");
	RNA_def_property_ui_text(prop, "Texture", "Sprite texture used for each particle (empty for the default round mask)");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "emission_direction", PROP_FLOAT, PROP_XYZ);
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_text(prop, "Emission Direction", "Central axis of the emission cone");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "emission_angle", PROP_FLOAT, PROP_NONE);
	...
	RNA_def_property_pointer_sdna(prop, NULL, "size_curve");
	RNA_def_property_struct_type(prop, "CurveMapping");
	RNA_def_property_ui_text(prop, "Size Curve", "Size over lifetime curve");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "use_size_curve", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "use_size_curve", 1);
	RNA_def_property_ui_text(prop, "Size Curve", "Enable size-over-lifetime curve");
	RNA_def_property_update(prop, NC_OBJECT, "rna_GPUParticles_use_size_curve_update");

	prop = RNA_def_property(srna, "color_curve", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "color_curve");
	RNA_def_property_struct_type(prop, "CurveMapping");
	RNA_def_property_ui_text(prop, "Color Curve", "Color over lifetime curve (RGB channels)");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "use_color_curve", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "use_color_curve", 1);
	RNA_def_property_ui_text(prop, "Color Curve", "Enable color-over-lifetime curve");
	RNA_def_property_update(prop, NC_OBJECT, "rna_GPUParticles_use_color_curve_update");

	prop = RNA_def_property(srna, "use_debug_ui", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "use_debug_ui", 1);
	RNA_def_property_ui_text(prop, "Live Debug UI", "Show an in-game ImGui panel to tweak these parameters live while playing");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "enabled", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_negative_sdna(prop, NULL, "disable_emission", 1);
	RNA_def_property_ui_text(prop, "Enabled",
	                          "Emit particles when the game starts. Independent of object visibility -- "
	                          "an invisible emitter with this on still simulates and draws its particles, "
	                          "and a visible object with this off shows no particles from it");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "blend_mode", PROP_ENUM, PROP_NONE);
	RNA_def_property_enum_sdna(prop, NULL, "blend_mode");
	RNA_def_property_enum_items(prop, rna_enum_gpu_particle_blend_mode_items);
	RNA_def_property_ui_text(prop, "Blend Mode", "How particle color blends with the scene behind it");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "billboard_mode", PROP_ENUM, PROP_NONE);
	RNA_def_property_enum_sdna(prop, NULL, "billboard_mode");
	RNA_def_property_enum_items(prop, rna_enum_gpu_particle_billboard_mode_items);
	RNA_def_property_ui_text(prop, "Billboard Mode", "How particle quads orient toward the camera");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "use_backface_culling", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "use_backface_culling", 1);
	RNA_def_property_ui_text(prop, "Backface Culling", "Hide the particle quad's back face instead of drawing it double-sided");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "collision_mode", PROP_ENUM, PROP_NONE);
	RNA_def_property_enum_sdna(prop, NULL, "collision_mode");
	RNA_def_property_enum_items(prop, rna_enum_gpu_particle_collision_mode_items);
	RNA_def_property_ui_text(prop, "Collision Mode", "How particles collide with the scene");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "collision_height", PROP_FLOAT, PROP_DISTANCE);
	RNA_def_property_ui_text(prop, "Ground Height", "World-space Z height of the analytic ground plane (Ground Plane mode only, ignored when Ground Object is set)");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "collision_ground_object", PROP_POINTER, PROP_NONE);
	RNA_def_property_struct_type(prop, "Object");
	RNA_def_property_pointer_sdna(prop, NULL, "collision_ground_object");
	RNA_def_property_flag(prop, PROP_EDITABLE);
	RNA_def_property_ui_text(prop, "Ground Object", "Optional object whose world-space Z height overrides Ground Height every frame (Ground Plane mode only)");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "collision_bounce", PROP_FLOAT, PROP_FACTOR);
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_ui_text(prop, "Bounce", "Restitution on collision (0 = particle sticks, 1 = bounces at equal speed)");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "collision_friction", PROP_FLOAT, PROP_FACTOR);
	RNA_def_property_range(prop, 0.0f, 1.0f);
...
static void rna_def_object_game_settings(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "use_occlude_culling", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "gameflag", OB_OCCLUDER);
	RNA_def_property_ui_text(prop, "Occluder",
	                          "Also use this object's geometry as an occluder for occlusion culling, on top of "
	                          "its normal Physics Type (e.g. Static collision keeps working). Independent from "
	                          "Physics Type = Occluder, which replaces collision instead of adding to it");
	...
	prop = RNA_def_property(srna, "is_vehicle", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "gameflag2", OB_VEHICLE);
	RNA_def_property_boolean_funcs(prop, NULL, "rna_GameObjectSettings_is_vehicle_set");
	RNA_def_property_ui_text(prop, "Vehicle",
	                          "Mark this object as a vehicle chassis; its Vehicle Wheels list identifies the "
	...
	prop = RNA_def_property(srna, "collision_bound", PROP_POINTER, PROP_NONE);
	RNA_def_property_flag(prop, PROP_EDITABLE);
	RNA_def_property_ui_text(prop, "Collider Object", "Use an object as a collision box, leave empty to calculate the triangle mesh for this object");
...
static void rna_def_object_lodlevel(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "use_invisible_mesh", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flags", OB_LOD_USE_INVISIBLE);
	RNA_def_property_ui_text(prop, "Invisible", "When the object reaches this lodlevel it will no longer render");
	RNA_def_property_ui_icon(prop, ICON_RESTRICT_RENDER_OFF, 0);
	RNA_def_property_update(prop, NC_OBJECT | ND_LOD, NULL);

	prop = RNA_def_property(srna, "use_billboard", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flags", OB_LOD_USE_BILLBOARD);
	RNA_def_property_ui_text(prop, "Billboard", "Rotate this level of detail's mesh around Z to always face the camera (for tree/foliage impostors)");
	RNA_def_property_update(prop, NC_OBJECT | ND_LOD, NULL);

	prop = RNA_def_property(srna, "use_atlas", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flags", OB_LOD_USE_ATLAS);
	RNA_def_property_ui_text(prop, "Multi-Angle Atlas",
	                          "Billboard texture is a grid of views around Z; pick the cell matching the camera angle each frame");
	RNA_def_property_update(prop, NC_OBJECT | ND_LOD, NULL);

	prop = RNA_def_property(srna, "atlas_columns", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "atlas_cols");
	RNA_def_property_range(prop, 1, 16);
	RNA_def_property_ui_text(prop, "Atlas Columns", "Number of columns in the multi-angle impostor atlas");
	RNA_def_property_update(prop, NC_OBJECT | ND_LOD, NULL);

	prop = RNA_def_property(srna, "atlas_rows", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "atlas_rows");
	RNA_def_property_range(prop, 1, 16);
	RNA_def_property_ui_text(prop, "Atlas Rows", "Number of rows in the multi-angle impostor atlas");
	RNA_def_property_update(prop, NC_OBJECT | ND_LOD, NULL);
}

static void rna_def_object_wheel_settings(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	srna = RNA_def_struct(brna, "VehicleWheel", NULL);
	RNA_def_struct_sdna(srna, "bWheelSettings");
	RNA_def_struct_ui_text(srna, "Vehicle Wheel", "Object reference used as a wheel by a vehicle chassis");

	prop = RNA_def_property(srna, "object", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "ob");
	RNA_def_property_struct_type(prop, "Object");
	RNA_def_property_flag(prop, PROP_EDITABLE);
	RNA_def_property_ui_text(prop, "Object", "Object to use as this wheel");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);
	...
	RNA_def_property_ui_text(prop, "Radius",
	                          "Wheel radius; 0 auto-detects from the wheel object's mesh bounds");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "suspension_rest_length", PROP_FLOAT, PROP_DISTANCE);
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_range(prop, 0.0f, 1.0f, 1, 3);
	RNA_def_property_float_default(prop, 0.3f);
	RNA_def_property_ui_text(prop, "Suspension Rest Length",
	                          "Suspension rest length for this wheel; 0 uses the default (0.3m)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "has_steering", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "has_steering", 1);
	RNA_def_property_ui_text(prop, "Steering", "This wheel responds to steering input");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "has_drive", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "has_drive", 1);
	RNA_def_property_ui_text(prop, "Drive",
	                          "This wheel receives engine force (used by game logic, e.g. Drive Type FWD/RWD/AWD "
	                          "and the handbrake); has no effect on the physics engine by itself");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "show_expanded", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flags", WHEEL_SHOW_EXPANDED);
	RNA_def_property_ui_text(prop, "Expanded", "Show this wheel's tuning fields in the user interface");
	RNA_def_property_ui_icon(prop, ICON_TRIA_RIGHT, 1);
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "suspension_stiffness", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "suspension_stiffness");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_range(prop, 0.0f, 50.0f, 1, 2);
	RNA_def_property_float_default(prop, 5.88f);
	RNA_def_property_ui_text(prop, "Stiffness",
	                          "Suspension spring stiffness for this wheel; 0 uses the default (5.88)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "suspension_damping", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "suspension_damping");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_range(prop, 0.0f, 3.0f, 1, 2);
	RNA_def_property_float_default(prop, 0.88f);
	RNA_def_property_ui_text(prop, "Damping",
	                          "Suspension damping relaxation for this wheel; 0 uses the default (0.88)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "suspension_compression", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "suspension_compression");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_range(prop, 0.0f, 3.0f, 1, 2);
	RNA_def_property_float_default(prop, 0.83f);
	RNA_def_property_ui_text(prop, "Compression",
	                          "Suspension damping compression for this wheel; 0 uses the default (0.83)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "friction", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "friction");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_range(prop, 0.0f, 20.0f, 1, 2);
	RNA_def_property_float_default(prop, 10.5f);
	RNA_def_property_ui_text(prop, "Friction",
	                          "Tire friction slip for this wheel; 0 uses the default (10.5)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "roll_influence", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "roll_influence");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_range(prop, 0.0f, 1.0f, 1, 2);
	RNA_def_property_float_default(prop, 0.1f);
	RNA_def_property_ui_text(prop, "Roll Influence",
	                          "How much this wheel reduces chassis roll; 0 uses the default (0.1)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "max_suspension_travel", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "max_suspension_travel_cm");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_range(prop, 0.0f, 100.0f, 1, 2);
	RNA_def_property_float_default(prop, 500.0f);
	RNA_def_property_ui_text(prop, "Max Suspension Travel",
	                          "Maximum suspension travel, in centimeters, for this wheel; 0 uses the default (500)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "max_suspension_force", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "max_suspension_force");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_range(prop, 0.0f, 20000.0f, 10, 0);
	RNA_def_property_float_default(prop, 6000.0f);
	RNA_def_property_ui_text(prop, "Max Suspension Force",
	                          "Maximum force the suspension can apply for this wheel; 0 uses the default (6000)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);
}

static void rna_def_object_gear_ratio(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	srna = RNA_def_struct(brna, "VehicleGear", NULL);
	RNA_def_struct_sdna(srna, "bGearRatio");
	RNA_def_struct_ui_text(srna, "Vehicle Gear", "One gear ratio slot in a vehicle chassis's gearbox");

	prop = RNA_def_property(srna, "ratio", PROP_FLOAT, PROP_NONE);
	RNA_def_property_range(prop, -FLT_MAX, FLT_MAX);
	RNA_def_property_ui_range(prop, -5.0f, 5.0f, 1, 2);
	RNA_def_property_float_default(prop, 1.0f);
	RNA_def_property_ui_text(prop, "Ratio",
	                          "Gear ratio applied to engine torque; negative is reverse gear");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);
}

static void rna_def_object_animation_event(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	/* Animation Event */
	srna = RNA_def_struct(brna, "AnimationEvent", NULL);
	RNA_def_struct_sdna(srna, "AnimationEvent");

	prop = RNA_def_property(srna, "show_expanded", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", ANIMEVENT_SHOW);
	RNA_def_property_ui_text(prop, "Expanded", "Set expanded in the user interface");
	RNA_def_property_ui_icon(prop, ICON_TRIA_RIGHT, 1);
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "anim", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "action");
	RNA_def_property_struct_type(prop, "Action");
	RNA_def_property_flag(prop, PROP_EDITABLE);
	RNA_def_property_ui_text(prop, "Animation", "Animation Target");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "eventcall", PROP_STRING, PROP_NONE);
	RNA_def_property_ui_text(prop, "Animation Event", "Function that should be called if any trigger triggered (component.module)");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "triggers", PROP_COLLECTION, PROP_NONE);
	RNA_def_property_struct_type(prop, "AnimationEventTrigger");
	RNA_def_property_ui_text(prop, "", "");
	RNA_def_property_update(prop, NC_OBJECT, NULL);
}

static void rna_def_object_animation_event_trigger(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	/* Animation Event Trigger */
	srna = RNA_def_struct(brna, "AnimationEventTrigger", NULL);
	RNA_def_struct_sdna(srna, "AnimationEventTrigger");

	prop = RNA_def_property(srna, "frame", PROP_INT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Animation Trigger Frame", "Frame where the trigger should be triggered");
	RNA_def_property_update(prop, NC_OBJECT, NULL);

	prop = RNA_def_property(srna, "custom_arg", PROP_STRING, PROP_NONE);
	RNA_def_property_ui_text(prop, "Custom Argument", "Add a custom argument to use in conjunction with the function ex: def function_name(object, argument)");
	RNA_def_property_update(prop, NC_OBJECT, NULL);
}
...
/*	RNA_def_property_collection_funcs(prop, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "constraints__add", "constraints__remove"); */
	...
	/* GPU particle emitter -- top-level on Object (not GameObjectSettings) since
	 * properties_particle.py reads context.object.gpu_particles/use_gpu_particles directly. */
	prop = RNA_def_property(srna, "use_gpu_particles", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "gameflag2", OB_GPU_PARTICLES);
	RNA_def_property_ui_text(prop, "GPU Particles", "Emit GPU particles from this object");
	RNA_def_property_update(prop, NC_OBJECT, "rna_Object_use_gpu_particles_update");

	prop = RNA_def_property(srna, "use_gpu_particle_collider", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "gameflag2", OB_GPU_PARTICLE_COLLIDER);
	...
	prop = RNA_def_property(srna, "gpu_particles", PROP_POINTER, PROP_NONE);
	RNA_def_property_flag(prop, PROP_NEVER_NULL);
	...
	/* Vehicle wheels */
	prop = RNA_def_property(srna, "vehicle_wheels", PROP_COLLECTION, PROP_NONE);
	RNA_def_property_collection_sdna(prop, NULL, "vehicle_wheels", NULL);
	RNA_def_property_struct_type(prop, "VehicleWheel");
	RNA_def_property_ui_text(prop, "Vehicle Wheels",
	                          "Objects used as wheels when this object is marked as a vehicle chassis "
	                          "(Physics > Vehicle)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "vehicle_ray_cast_mask", PROP_INT, PROP_UNSIGNED);
	RNA_def_property_int_sdna(prop, NULL, "vehicle_ray_cast_mask");
	RNA_def_property_range(prop, 0, 32767);
	RNA_def_property_ui_text(prop, "Ray Cast Mask",
	                          "Collision mask used by this vehicle's suspension raycasts; 0 uses the default "
	                          "(all layers). Lower to keep the wheels from raycasting against their own "
	                          "chassis or each other");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "vehicle_steering_wheel", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "vehicle_steering_wheel");
	RNA_def_property_struct_type(prop, "Object");
	RNA_def_property_flag(prop, PROP_EDITABLE | PROP_ID_SELF_CHECK);
	RNA_def_property_ui_text(prop, "Steering Wheel",
	                          "Optional child object rotated by game logic to visually match the current "
	                          "steering value; native reference only, the rotation itself is applied by "
	                          "game logic, not by the physics engine");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "vehicle_max_torque", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "vehicle_max_torque");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	...
	RNA_def_property_float_sdna(prop, NULL, "vehicle_max_rpm");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_range(prop, 0.0f, 12000.0f, 100, 0);
	RNA_def_property_float_default(prop, 6000.0f);
	RNA_def_property_ui_text(prop, "Max RPM",
	                          "Arcade-feel max simulated engine RPM; 0 uses game logic's built-in "
	                          "default. Consumed by game logic (not the physics engine) to scale applied "
	                          "engine force");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "gearbox_type", PROP_ENUM, PROP_NONE);
	RNA_def_property_enum_sdna(prop, NULL, "gearbox_type");
	RNA_def_property_enum_items(prop, gearbox_type_items);
	RNA_def_property_ui_text(prop, "Gearbox Type",
	                          "How game logic selects a gear from vehicle_gears; consumed by game logic, "
	                          "not the physics engine");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	/* Vehicle gears */
	prop = RNA_def_property(srna, "vehicle_gears", PROP_COLLECTION, PROP_NONE);
	RNA_def_property_collection_sdna(prop, NULL, "vehicle_gears", NULL);
	RNA_def_property_struct_type(prop, "VehicleGear");
	RNA_def_property_ui_text(prop, "Vehicle Gears",
	                          "Gear ratios for this vehicle chassis's gearbox (Physics > Vehicle)");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);

	prop = RNA_def_property(srna, "vehicle_com_offset", PROP_FLOAT, PROP_XYZ);
	RNA_def_property_float_sdna(prop, NULL, "vehicle_com_offset");
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_range(prop, -10.0f, 10.0f, 1, 3);
	RNA_def_property_ui_text(prop, "Center of Mass Offset",
	                          "Local-space offset applied to the chassis center of mass; non-zero values "
	                          "wrap the chassis collision shape in a compound shape at conversion time. "
	                          "Zero (default) leaves the chassis shape untouched");
	RNA_def_property_update(prop, NC_OBJECT | ND_DRAW, NULL);
	...
	RNA_def_property_collection_sdna(prop, NULL, "animevents", NULL);
	RNA_def_property_struct_type(prop, "AnimationEvent");
	RNA_def_property_ui_text(prop, "Animation Events", "Animation events list of the object, capable of calling a python call or logic bricks sensor at the trigger moment of the animation");
	RNA_def_property_update(prop, NC_OBJECT, NULL);
...
void RNA_def_object(BlenderRNA *brna)
{
	...
	rna_def_object_wheel_settings(brna);
	rna_def_object_gear_ratio(brna);
	rna_def_object_animation_event(brna);
	rna_def_object_animation_event_trigger(brna);
}
