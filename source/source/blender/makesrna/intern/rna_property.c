...
static void rna_GameProperty_name_set(PointerRNA *ptr, const char *value)
{
	...
	ListBase *properties = (GS(id->name) == ID_WO) ? &((World *)id)->prop : &((Object *)id)->prop;
	BLI_uniquename(properties, prop, DATA_("Property"), '.', offsetof(bProperty, name), sizeof(prop->name));
}
