...
static const EnumPropertyItem sensor_type_items[] = {
	{SENS_ANIMATIONEVENT, "ANIMATIONEVENT", 0, "Animation Event", ""},
...
static const EnumPropertyItem property_sensor_evaluation_items[] = {
...
static const EnumPropertyItem runtime_property_bool_evaluation_items[] = {
	{SENS_PROP_EQUAL, "PROPEQUAL", 0, "Equal", ""},
	{SENS_PROP_NEQUAL, "PROPNEQUAL", 0, "Not Equal", ""},
	{0, NULL, 0, NULL, NULL}
};

static const EnumPropertyItem runtime_property_float_evaluation_items[] = {
	{SENS_PROP_EQUAL, "PROPEQUAL", 0, "Equal", ""},
	{SENS_PROP_NEQUAL, "PROPNEQUAL", 0, "Not Equal", ""},
	{SENS_PROP_INTERVAL, "PROPINTERVAL", 0, "Interval", ""},
	{SENS_PROP_LESSTHAN, "PROPLESSTHAN", 0, "Less Than", ""},
...
static const EnumPropertyItem runtime_property_vector_evaluation_items[] = {
	{SENS_PROP_EQUAL, "PROPEQUAL", 0, "Equal", ""},
	{SENS_PROP_NEQUAL, "PROPNEQUAL", 0, "Not Equal", ""},
	{0, NULL, 0, NULL, NULL}
};
		...
		bContext *UNUSED(C), PointerRNA *ptr, PropertyRNA *UNUSED(prop), bool *r_free)
{
	...
	*r_free = false;
	...
	if (sensor->runtime_property == SENS_RUNTIME_PROP_VISIBLE) {
		return runtime_property_bool_evaluation_items;
	}
	if (sensor->runtime_property == SENS_RUNTIME_PROP_MASS) {
		return runtime_property_float_evaluation_items;
	}
	return runtime_property_vector_evaluation_items;
}
...
const EnumPropertyItem *rna_Sensor_type_itemf(bContext *C, PointerRNA *ptr, PropertyRNA *UNUSED(prop), bool *r_free)
{
	...
	RNA_enum_items_add_value(&item, &totitem, sensor_type_items, SENS_ANIMATIONEVENT);
...
static void rna_def_sensor(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "use_dt", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_ui_text(prop, "Use DeltaTime", "Use delta time instead of frame skips in pulse mode");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
	...
	prop = RNA_def_property(srna, "tick_skip_dt", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "freq_dt");
	RNA_def_property_ui_text(prop, "Skip Time",
		"Number of logic ticks skipped between 2 active pulses "
		"(0 = pulse every logic tick, 1 = skip is 1 second, etc.)");
	...
	prop = RNA_def_property(srna, "show_description", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", SENS_SHOW_DESCR);
	RNA_def_property_ui_text(prop, "Show sensor description", "Set sensor description in the user interface");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "sensor_description", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "description");
	RNA_def_property_ui_text(prop, "Description", "Sensor description");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "color", PROP_FLOAT, PROP_COLOR_GAMMA);
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_text(prop, "Color", "Custom color for sensor");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
...
static void rna_def_near_sensor(BlenderRNA *brna)
{
	...
	RNA_def_property_boolean_sdna(prop, NULL, "debug", 1);
	RNA_def_property_ui_text(prop, "Debug Volume", "Draw trigger and reset distances while the game runs");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
}
...
static void rna_def_mouse_sensor(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem mouse_event_items[] = {
		...
		{BL_SENS_MOUSE_LEFTTHUMBMOUSE, "LEFTTHUMBMOUSECLICK", 0, "Left Thumb Button", ""},
		{BL_SENS_MOUSE_RIGHTTHUMBMOUSE, "RIGHTTHUMBMOUSECLICK", 0, "Right Thumb Button", ""},
		{BL_SENS_MOUSE_BUTTON6, "BUTTON6CLICK", 0, "Button 6", ""},
		{BL_SENS_MOUSE_BUTTON7, "BUTTON7CLICK", 0, "Button 7", ""},
...
static void rna_def_property_sensor(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem runtime_property_items[] = {
		{SENS_RUNTIME_PROP_LOCAL_POSITION, "LOCAL_POSITION", 0, "Local Position", "Object local position (Vector3)"},
		{SENS_RUNTIME_PROP_VISIBLE, "VISIBLE", 0, "Visible", "Object render visibility (Boolean)"},
		{SENS_RUNTIME_PROP_MASS, "MASS", 0, "Mass", "Physics mass (Float, read-only)"},
		{SENS_RUNTIME_PROP_LINEAR_VELOCITY, "LINEAR_VELOCITY", 0, "Linear Velocity", "Physics linear velocity (Vector3)"},
		{SENS_RUNTIME_PROP_ANGULAR_VELOCITY, "ANGULAR_VELOCITY", 0, "Angular Velocity", "Physics angular velocity (Vector3)"},
		{SENS_RUNTIME_PROP_GRAVITY, "GRAVITY", 0, "Gravity", "Physics gravity (Vector3)"},
		{SENS_RUNTIME_PROP_LOCAL_SCALE, "LOCAL_SCALE", 0, "Local Scale", "Object local scale (Vector3)"},
		{0, NULL, 0, NULL, NULL}
	};
	...
	RNA_def_property_enum_funcs(prop, NULL, NULL, "rna_PropertySensor_evaluation_type_itemf");
	...
	RNA_def_property_ui_text(prop, "Use Runtime API", "Read an exposed runtime property instead of a Game Property");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "runtime_target", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "runtime_target");
	RNA_def_property_struct_type(prop, "Object");
	RNA_def_property_ui_text(prop, "Target", "Object to read; empty uses the owner object");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "runtime_property", PROP_ENUM, PROP_NONE);
	RNA_def_property_enum_sdna(prop, NULL, "runtime_property");
	RNA_def_property_enum_items(prop, runtime_property_items);
	RNA_def_property_ui_text(prop, "Runtime Property", "Runtime property to evaluate");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "runtime_value", PROP_FLOAT, PROP_XYZ);
	RNA_def_property_float_sdna(prop, NULL, "runtime_value");
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_text(prop, "Value", "Typed value used by the runtime property comparison");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "runtime_value_max", PROP_FLOAT, PROP_XYZ);
	RNA_def_property_float_sdna(prop, NULL, "runtime_value_max");
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_text(prop, "Maximum Value", "Maximum typed value used by interval comparison");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "runtime_bool_value", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "runtime_bool_value", 1);
	RNA_def_property_ui_text(prop, "Value", "Boolean value used by the runtime property comparison");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
...
static void rna_def_animation_event_sensor(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	srna = RNA_def_struct(brna, "AnimationEventSensor", "Sensor");
	RNA_def_struct_ui_text(srna, "Animation Event Sensor", "Sensor wip");
	RNA_def_struct_sdna_from(srna, "bAnimationEventSensor", "data");

	prop = RNA_def_property(srna, "event_index", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "index");
	RNA_def_property_range(prop, 1, 128);
	RNA_def_property_int_default(prop, 1);
	RNA_def_property_ui_text(prop, "Event Index", "Choose the AnimationEvent from the object (by index), like the first AnimationEvent is 1");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "trigger_index", PROP_INT, PROP_NONE);
	RNA_def_property_range(prop, 1, 128);
	RNA_def_property_int_default(prop, 1);
	RNA_def_property_ui_text(prop, "Trigger Index", "Which trigger will activate this sensor");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "trigger_all", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_default(prop, false);
	RNA_def_property_ui_text(prop, "All", "All triggers will activate the sensor");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
}
...
static void rna_def_delay_sensor(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "delay_time", PROP_FLOAT, PROP_NONE);
	...
	prop = RNA_def_property(srna, "use_deltatime", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "use_delta", SENS_DELAY_USEDELTA);
	RNA_def_property_ui_text(prop, "Use deltaTime",
		"this changes the delay mode from frames to time. 1 delay = 1 second"
		"");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
...
static void rna_def_radar_sensor(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "use_debug", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "debug", 1);
	RNA_def_property_ui_text(prop, "Debug Volume", "Draw the radar cone while the game runs");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
}
...
static void rna_def_ray_sensor(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "use_debug", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "debug", 1);
	RNA_def_property_ui_text(prop, "Debug Ray", "Draw the ray and hit segment while the game runs");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
}
...
void RNA_def_sensor(BlenderRNA *brna)
{
	...
	rna_def_animation_event_sensor(brna);
