...
static void rna_userdef_update(Main *UNUSED(bmain), Scene *UNUSED(scene), PointerRNA *ptr)
{
	if (ptr->type == &RNA_Theme || ptr->type == &RNA_ThemeGlobal || ptr->type == &RNA_ThemeGlobalUI) {
		...
		ThemeSpace *global = &btheme->tglobal;

#define GLOBAL_THEME_COPY(space, id) \
		if (btheme->global_theme_spaces & (1 << (id))) { \
			memcpy((space)->back, global->back, sizeof((space)->back)); \
			memcpy(&(space)->title, &global->title, \
			       offsetof(ThemeSpace, panelcolors) - offsetof(ThemeSpace, title)); \
			memcpy(&(space)->panelcolors, &global->panelcolors, sizeof((space)->panelcolors)); \
			memcpy(&(space)->button, &global->button, \
			       offsetof(ThemeSpace, list) - offsetof(ThemeSpace, button)); \
			memcpy((space)->navigation_bar, global->navigation_bar, sizeof((space)->navigation_bar)); \
		}

		GLOBAL_THEME_COPY(&btheme->tv3d, 1);
		GLOBAL_THEME_COPY(&btheme->ttime, 2);
		GLOBAL_THEME_COPY(&btheme->tipo, 3);
		GLOBAL_THEME_COPY(&btheme->tact, 4);
		GLOBAL_THEME_COPY(&btheme->tnla, 5);
		GLOBAL_THEME_COPY(&btheme->tima, 6);
		GLOBAL_THEME_COPY(&btheme->tseq, 7);
		GLOBAL_THEME_COPY(&btheme->text, 8);
		GLOBAL_THEME_COPY(&btheme->tnode, 9);
		GLOBAL_THEME_COPY(&btheme->tlogic, 10);
		GLOBAL_THEME_COPY(&btheme->tbuts, 11);
		GLOBAL_THEME_COPY(&btheme->toops, 12);
		GLOBAL_THEME_COPY(&btheme->tuserpref, 14);
		GLOBAL_THEME_COPY(&btheme->tinfo, 15);
		GLOBAL_THEME_COPY(&btheme->tfile, 16);
		GLOBAL_THEME_COPY(&btheme->tconsole, 17);
		GLOBAL_THEME_COPY(&btheme->tclip, 20);

#undef GLOBAL_THEME_COPY
...
static void rna_Theme_global_init(bTheme *btheme)
{
	if (btheme->global_theme_spaces == 0 && btheme->tglobal_ui.wcol_regular.inner[3] == 0) {
...
static PointerRNA rna_Theme_global_theme_get(PointerRNA *ptr)
{
	bTheme *btheme = (bTheme *)ptr->data;
	rna_Theme_global_init(btheme);
	return rna_pointer_inherit_refine(ptr, &RNA_ThemeGlobal, &btheme->tglobal);
}

static PointerRNA rna_Theme_global_ui_get(PointerRNA *ptr)
{
	bTheme *btheme = (bTheme *)ptr->data;
	rna_Theme_global_init(btheme);
	return rna_pointer_inherit_refine(ptr, &RNA_ThemeGlobalUI, &btheme->tglobal_ui);
}
...
static void rna_def_userdef_theme_global(BlenderRNA *brna)
{
	StructRNA *srna;

	srna = RNA_def_struct(brna, "ThemeGlobal", "ThemeSpaceGeneric");
	RNA_def_struct_sdna(srna, "ThemeSpace");
	RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
	RNA_def_struct_ui_text(srna, "Global Theme", "Theme settings shared by all editors");
}

static void rna_def_userdef_theme_global_ui(BlenderRNA *brna)
{
	StructRNA *srna;

	srna = RNA_def_struct(brna, "ThemeGlobalUI", "ThemeUserInterface");
	RNA_def_struct_sdna(srna, "ThemeUI");
	RNA_def_struct_clear_flag(srna, STRUCT_UNDO);
	RNA_def_struct_ui_text(srna, "Global User Interface", "User interface theme settings shared by the interface");
}

static void rna_def_theme_global_copy_property(StructRNA *srna,
	                                             const char *identifier,
	                                             const char *name,
	                                             int bit)
{
	PropertyRNA *prop = RNA_def_property(srna, identifier, PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "global_theme_spaces", 1 << bit);
	RNA_def_property_ui_text(prop, name, "Copy the Global Theme settings to this editor");
	RNA_def_property_update(prop, 0, "rna_userdef_update");
...
static void rna_def_userdef_themes(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem active_theme_area[] = {
		...
		{21, "GLOBAL_THEME", ICON_WORLD_DATA, "Global Theme", ""},
	...
	prop = RNA_def_property(srna, "global_theme", PROP_POINTER, PROP_NONE);
	RNA_def_property_flag(prop, PROP_NEVER_NULL);
	RNA_def_property_pointer_sdna(prop, NULL, "tglobal");
	RNA_def_property_pointer_funcs(prop, "rna_Theme_global_theme_get", NULL, NULL, NULL);
	RNA_def_property_struct_type(prop, "ThemeGlobal");
	RNA_def_property_ui_text(prop, "Global Theme", "Shared theme space settings");

	prop = RNA_def_property(srna, "global_ui", PROP_POINTER, PROP_NONE);
	RNA_def_property_flag(prop, PROP_NEVER_NULL);
	RNA_def_property_pointer_sdna(prop, NULL, "tglobal_ui");
	RNA_def_property_pointer_funcs(prop, "rna_Theme_global_ui_get", NULL, NULL, NULL);
	RNA_def_property_struct_type(prop, "ThemeGlobalUI");
	RNA_def_property_ui_text(prop, "Global User Interface", "Shared user interface widget settings");

	rna_def_theme_global_copy_property(srna, "global_copy_user_interface", "User Interface", 0);

	rna_def_theme_global_copy_property(srna, "global_copy_view_3d", "3D View", 1);
	rna_def_theme_global_copy_property(srna, "global_copy_timeline", "Timeline", 2);
	rna_def_theme_global_copy_property(srna, "global_copy_graph_editor", "Graph Editor", 3);
	rna_def_theme_global_copy_property(srna, "global_copy_dopesheet", "Dope Sheet", 4);
	rna_def_theme_global_copy_property(srna, "global_copy_nla", "NLA Editor", 5);
	rna_def_theme_global_copy_property(srna, "global_copy_image_editor", "UV/Image Editor", 6);
	rna_def_theme_global_copy_property(srna, "global_copy_sequence_editor", "Video Sequence Editor", 7);
	rna_def_theme_global_copy_property(srna, "global_copy_text_editor", "Text Editor", 8);
	rna_def_theme_global_copy_property(srna, "global_copy_node_editor", "Node Editor", 9);
	rna_def_theme_global_copy_property(srna, "global_copy_logic_editor", "Logic Bricks Editor", 10);
	rna_def_theme_global_copy_property(srna, "global_copy_properties", "Properties", 11);
	rna_def_theme_global_copy_property(srna, "global_copy_outliner", "Outliner", 12);
	rna_def_theme_global_copy_property(srna, "global_copy_user_preferences", "User Preferences", 14);
	rna_def_theme_global_copy_property(srna, "global_copy_info", "Info", 15);
	rna_def_theme_global_copy_property(srna, "global_copy_file_browser", "File Browser", 16);
	rna_def_theme_global_copy_property(srna, "global_copy_console", "Python Console", 17);
	rna_def_theme_global_copy_property(srna, "global_copy_clip_editor", "Movie Clip Editor", 20);
...
static void rna_def_userdef_dothemes(BlenderRNA *brna)
{
	...
	rna_def_userdef_theme_global(brna);
	rna_def_userdef_theme_global_ui(brna);
...
static void rna_def_userdef_view(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "header_size", PROP_INT, PROP_PIXEL);
	RNA_def_property_int_sdna(prop, NULL, "header_size");
	RNA_def_property_range(prop, 20, 100);
	RNA_def_property_ui_range(prop, 20, 60, 1, -1);
	RNA_def_property_ui_text(prop, "Header Size",
	                         "Height of editor header bars, in pixels (requires no restart)");
	RNA_def_property_update(prop, 0, "rna_userdef_dpi_update");
...
#  else
	...
	RNA_def_property_ui_text(prop, "Auto Run Python Scripts",
	                         "Allow any .range file to run scripts automatically "
	                         "(unsafe with range files from an untrusted source)");
...
void RNA_def_userdef(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem user_pref_sections[] = {
		{0, "", ICON_SETTINGS, "User Preferences", ""},
		{USER_SECTION_INTERFACE, "INTERFACE", 0, "Interface", ""},
		{USER_SECTION_EDIT, "EDITING", 0, "Editing", ""},
		{USER_SECTION_INPUT, "INPUT", 0, "Input", ""},
		{USER_SECTION_ADDONS, "ADDONS", 0, "Add-ons", ""},
		{USER_SECTION_THEME, "THEMES", 0, "Themes", ""},
		//{USER_SECTION_LIGHT, "LIGHTS", 0, "Lights", ""}, // Range: 1.5, Unused.
		{0, "", ICON_LOGIC, "Game Engine", ""},
		{USER_SECTION_INPUT_SYSTEM, "INPUTSYSTEM", 0, "Input System", ""},
