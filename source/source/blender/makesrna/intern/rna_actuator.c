	...
	if (value != oa->type) {
		...
		switch (oa->type) {
			...
			case ACT_OBJECT_NORMAL_SET:
				memset(oa, 0, sizeof(bObjectActuator));
				oa->flag = ACT_DLOC_LOCAL;
				oa->type = ACT_OBJECT_NORMAL_SET;
				break;
		}
	}
}
...
static void rna_def_object_actuator(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem prop_type_items[] = {
		...
		{ACT_OBJECT_NORMAL_SET, "OBJECT_NORMAL_SET", 0, "Set Position", ""},
		{0, NULL, 0, NULL, NULL}
	};
...
static void rna_def_property_actuator(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem runtime_property_items[] = {
		...
		{ACT_RUNTIME_PROP_MASS, "MASS", 0, "Mass", "Dynamic physics mass (Float)"},
		{ACT_RUNTIME_PROP_LINEAR_VELOCITY, "LINEAR_VELOCITY", 0, "Linear Velocity", "Physics linear velocity (Vector3)"},
	...
	RNA_def_property_ui_text(prop, "Use Runtime API", "Write an exposed runtime property instead of a Game Property");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
	...
	RNA_def_property_ui_text(prop, "Runtime Property", "Runtime property to write");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "runtime_value", PROP_FLOAT, PROP_XYZ);
	RNA_def_property_float_sdna(prop, NULL, "runtime_value");
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_text(prop, "Value", "Typed value written to the runtime property");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
	...
	RNA_def_property_ui_text(prop, "Value", "Boolean value written to the runtime property");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
...
static void rna_def_edit_object_actuator(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "use_lerp_col", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_ui_text(prop, "Use lerp",
							 "Using lerp on the color, this will make it turn to the target color slowly with positive pulses");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "lerp_col", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_range(prop, 0.1f, 1.0f, 0.2f, 2);
	RNA_def_property_ui_text(prop, "Lerp", "");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "change_col", PROP_FLOAT, PROP_COLOR);
...
/*		ideally we could use ACT_GAME_START again and do a do_version() */
	...
	prop = RNA_def_property(srna, "saveloc", PROP_STRING, PROP_NONE);
	RNA_def_property_ui_text(prop, "Save Path",
	                         "The location where the file should be saved (always relative)");
	RNA_def_property_update(prop, NC_LOGIC, NULL);

	prop = RNA_def_property(srna, "extension_name", PROP_STRING, PROP_NONE);
	RNA_def_property_ui_text(prop, "Ext Name",
	                         "The name of the file extension (Default value is 'save') "
	                         "Example: SavedFile.save");
	RNA_def_property_update(prop, NC_LOGIC, NULL);
...
static void rna_def_twodfilter_actuator(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem prop_type_items[] = {
		...
		{ACT_2DFILTER_SSAO, "SSAO", 0, "Ambient Occlusion", "Screen-space ambient occlusion"},
		{ACT_2DFILTER_BLOOM, "BLOOM", 0, "Bloom", "Glow from bright areas"},
		{ACT_2DFILTER_TONEMAP, "TONEMAP", 0, "Tonemap", "HDR tone mapping"},
		{ACT_2DFILTER_LIGHTSCATTER, "LIGHTSCATTER", 0, "Light Scattering", "Volumetric light scattering"},
		{ACT_2DFILTER_SSR, "SSR", 0, "Screen Space Reflections", "Screen-space reflections"},
		{ACT_2DFILTER_FXAA, "FXAA", 0, "FXAA", "Fast approximate anti-aliasing"},
