...
static void rna_def_material_mtex(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "group_object_decal", PROP_POINTER, PROP_NONE);
	RNA_def_property_struct_type(prop, "Group");
	RNA_def_property_ui_text(prop, "Decals Group", "Group of objects to insert decals");
	RNA_def_property_flag(prop, PROP_EDITABLE);
	RNA_def_property_update(prop, 0, "rna_Material_update");
	...
	prop = RNA_def_property(srna, "use_map_roughness", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "mapto", MAP_ROUGHNESS);
	RNA_def_property_ui_text(prop, "Roughness", "Let the texture affects the roughness value");
	RNA_def_property_update(prop, 0, "rna_Material_update");

	prop = RNA_def_property(srna, "use_map_metallic", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "mapto", MAP_METALLIC);
	RNA_def_property_ui_text(prop, "Metallic", "Let the texture affects the metallic value");
	RNA_def_property_update(prop, 0, "rna_Material_update");

	prop = RNA_def_property(srna, "use_map_sss", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "mapto", MAP_SSS);
	RNA_def_property_ui_text(prop, "SSS", "Let the texture affects the sss value");
	RNA_def_property_update(prop, 0, "rna_Material_update");
	...
	prop = RNA_def_property(srna, "metallic_factor", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "metallicfac");
	RNA_def_property_ui_range(prop, 0, 1, 10, 3);
	RNA_def_property_ui_text(prop, "Metallic Factor", "Amount texture affects metallic");
	RNA_def_property_update(prop, 0, "rna_Material_update");

	prop = RNA_def_property(srna, "sss_factor", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "sssfac");
	RNA_def_property_range(prop, 0, 1);
	RNA_def_property_ui_range(prop, 0, 1, 10, 3);
	RNA_def_property_ui_text(prop, "Subsurface Scattering Factor", "Amount texture affects SSS");
	RNA_def_property_update(prop, 0, "rna_Material_update");
	...
	RNA_def_property_ui_text(prop, "Fresnel Ratio", "Fresnel mix of refraction and reflection textures");
	RNA_def_property_update(prop, 0, "rna_Material_update");
...
static void rna_def_material_gamesettings(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem prop_alpha_blend_items[] = {
		...
		{GEMAT_MULTIPLY, "ALPHA_MULTIPLY", 0, "Multiply", "Multiplies face color with the background pixels"},
...
static void rna_def_material_colors(StructRNA *srna)
{
	...
	static const EnumPropertyItem prop_ramp_input_items[] = {
		...
		{MA_RAMP_IN_ALBEDO_R, "R_ALBEDO", 0, "R Channel", ""},
		{MA_RAMP_IN_ALBEDO_G, "G_ALBEDO", 0, "G Channel", ""},
		{MA_RAMP_IN_ALBEDO_B, "B_ALBEDO", 0, "B Channel", ""},
		{0, NULL, 0, NULL, NULL}
	};
...
static void rna_def_material_diffuse(StructRNA *srna)
{
	...
	static const EnumPropertyItem prop_diff_shader_items[] = {
		...
		{MA_DIFF_LAMBERT_CUSTOM_BSDF, "LAMBERT_CUSTOM", 0, "Lambert Custom", "Use a Lambert Custom shader"},
		{MA_DIFF_BURLEY_BSDF, "BURLEY", 0, "Burley", "Use a Burley shader"},
		{0, NULL, 0, NULL, NULL}
	};
	...
	RNA_def_property_ui_text(prop, "Diffuse Fresnel", "Size of diffuse area");
	RNA_def_property_update(prop, 0, "rna_Material_update");

	prop = RNA_def_property(srna, "diffuse_fresnel_factor", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "param[0]");
	RNA_def_property_range(prop, 0.0f, 5.0f);
	RNA_def_property_ui_text(prop, "Diffuse Fresnel Factor", "Blending factor of diffuse area");
	RNA_def_property_update(prop, 0, "rna_Material_update");
	...
	prop = RNA_def_property(srna, "diffuse_metallic_bsdf", PROP_FLOAT, PROP_NONE);
...
static void rna_def_material_specularity(StructRNA *srna)
{
	...
	static const EnumPropertyItem prop_specular_shader_items[] = {
		...
		{MA_SPEC_GGX_BSDF, "GGX", 0, "Optimized GGX", "Use an optimized GGX shader"},
		{0, NULL, 0, NULL, NULL}
	};
	...
	prop = RNA_def_property(srna, "specular_anisotropy", PROP_FLOAT, PROP_FACTOR);
	RNA_def_property_float_sdna(prop, NULL, "param[3]");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_ui_text(prop, "Specular Anisotropy", "Direction of fibers in a anisotropic surface");
	RNA_def_property_update(prop, 0, "rna_Material_update");
...
void RNA_def_material(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "use_gpu_skinning", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "shade_flag", MA_SKINNING);
	RNA_def_property_ui_text(prop, "GPU Skinning", "Use special vertex shader for GPU bone deformation in game engine "
	                          "(only takes effect on objects whose armature uses the GPU Skinning vertex deform mode)");

	prop = RNA_def_property(srna, "use_foliage", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "shade_flag", MA_FOLIAGE);
	RNA_def_property_ui_text(prop, "Foliage Shader", "Use foliage shader effect");
	RNA_def_property_update(prop, 0, "rna_Material_update");

	prop = RNA_def_property(srna, "foliage_grass", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_ui_text(prop, "Grass", "Foliage shader behavior for grass type");
	RNA_def_property_update(prop, 0, "rna_Material_update");

	prop = RNA_def_property(srna, "foliage_strength", PROP_FLOAT, PROP_FACTOR);
	RNA_def_property_range(prop, 0, 1);
	RNA_def_property_ui_text(prop, "Foliage strength", "Amount of strength applied to vertex foliage");
	RNA_def_property_update(prop, 0, "rna_Material_update"); 

	prop = RNA_def_property(srna, "foliage_turbulence", PROP_FLOAT, PROP_FACTOR);
	RNA_def_property_range(prop, 0, 10);
	...
	prop = RNA_def_property(srna, "script_frag", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "fragcode");
	RNA_def_property_struct_type(prop, "Text");
	RNA_def_property_flag(prop, PROP_EDITABLE);
	RNA_def_property_ui_text(prop, "Fragment Shader", "Material pixel shader");
	RNA_def_property_update(prop, 0, "rna_Material_update");

	prop = RNA_def_property(srna, "script_vert", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "vertcode");
	RNA_def_property_struct_type(prop, "Text");
	RNA_def_property_flag(prop, PROP_EDITABLE);
	RNA_def_property_ui_text(prop, "Vertex Shader", "Define a vertex shader for this material");
	RNA_def_property_update(prop, 0, "rna_Material_update");
	...
	prop = RNA_def_property(srna, "use_two_sided_shadows", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "mode2", MA_TWOSIDEDSHADOWS);
	RNA_def_property_ui_text(prop, "Cast Shadows As Two Sided",
		"Generates shadows on both sides of an object, use with 'Translucency' slider in the Shading tab");
	RNA_def_property_update(prop, 0, "rna_Material_update");
