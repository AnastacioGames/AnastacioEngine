...
static bool rna_Lamp_use_csm_shadow_get(PointerRNA *ptr)
{
	Lamp *la = (Lamp *)ptr->data;
	return la->shadow_cascade_count != 0;
}

static void rna_Lamp_use_csm_shadow_set(PointerRNA *ptr, bool value)
{
	Lamp *la = (Lamp *)ptr->data;
	la->shadow_cascade_count = value ? 3 : 0;
}
...
static void rna_def_lamp(BlenderRNA *brna)
{
	...
	/* BGE Only: distance-based light culling ("light LOD") */
	prop = RNA_def_property(srna, "use_cull_distance", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "mode", LA_CULL_DISTANCE);
	RNA_def_property_ui_text(prop, "Light LOD",
	                         "Stop evaluating this light (shading and shadows) once it is farther "
	                         "than Cull Distance from the active camera");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "cull_distance", PROP_FLOAT, PROP_DISTANCE);
	RNA_def_property_float_sdna(prop, NULL, "cull_distance");
	RNA_def_property_range(prop, 0.0f, 100000.0f);
	RNA_def_property_ui_range(prop, 0.0f, 5000.0f, 1, 2);
	RNA_def_property_ui_text(prop, "Cull Distance",
	                         "Distance from the camera beyond which this light stops being "
	                         "evaluated entirely (light LOD). Ignored by Sun lamps");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "glow_scale", PROP_FLOAT, PROP_FACTOR);
	RNA_def_property_float_sdna(prop, NULL, "glow_scale");
	RNA_def_property_range(prop, 0.0f, 100.0f);
	RNA_def_property_ui_range(prop, 0.0f, 5.0f, 0.1, 2);
	RNA_def_property_ui_text(prop, "Glow Scale",
	                         "Size multiplier for the billboard glow shown in place of this light "
	                         "when it is culled by distance (light LOD)");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");
}
...
static void rna_def_lamp_shadow(StructRNA *srna, int spot, int area)
{
	...
	static EnumPropertyItem prop_shadow_filter_type_items[] = {
		...
		{LA_SHADOW_FILTER_DITHERING, "DITHERING", 0, "Bayer Dither", "Bayer Ordered Dithering Filter"},
		{LA_SHADOW_FILTER_CLIPPING, "CLIPPING", 0, "Factor Clipping", "Factor Clipping Filter"},
		...
		{LA_SHADOW_FILTER_PCF_PENUMBRA, "PCF_PENUMBRA", 0, "PCF Penumbra", "Percentage Closer Filtering With Penumbra"},
		{0, NULL, 0, NULL, NULL}
	};
...
static void rna_def_sun_lamp(BlenderRNA *brna)
{
	...
	/* Cascaded Shadow Mapping (BGE/Range only, Sun lamps only).
	 * Property names match the upstream Range Engine implementation the user referenced
	 * (D:\ProjetoRangeEngine\2.79\scripts\startup\bl_ui\properties_game.py, DATA_PT_shadow_game),
	 * so scenes/scripts written against that version stay compatible with this fork. */
	prop = RNA_def_property(srna, "use_csm_shadow", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_funcs(prop, "rna_Lamp_use_csm_shadow_get", "rna_Lamp_use_csm_shadow_set");
	RNA_def_property_ui_text(prop, "Enable Cascaded Shadow Mapping",
	                         "Split the shadow map into near/middle/low cascades for sharper close shadows "
	                         "over long view distances");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "use_csm_debug", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "csm_flag", LA_CSM_DEBUG);
	RNA_def_property_ui_text(prop, "Enable Debug", "Visualize the cascade splits in the 3D view/game");
	RNA_def_property_update(prop, 0, "rna_Lamp_draw_update");

	prop = RNA_def_property(srna, "cascaded_proportion_two", PROP_FLOAT, PROP_FACTOR);
	RNA_def_property_float_sdna(prop, NULL, "shadow_cascade_proportion_near");
	RNA_def_property_range(prop, 0.01f, 0.98f);
	RNA_def_property_ui_text(prop, "Cascade Proportion Near",
	                         "Fraction of the shadow clip range covered by the near cascade");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "cascaded_proportion_three", PROP_FLOAT, PROP_FACTOR);
	RNA_def_property_float_sdna(prop, NULL, "shadow_cascade_proportion_middle");
	RNA_def_property_range(prop, 0.01f, 0.98f);
	RNA_def_property_ui_text(prop, "Cascade Proportion Middle",
	                         "Fraction of the shadow clip range covered by the middle cascade "
	                         "(the remainder after Near and Middle goes to the Low/far cascade)");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "shadow_csm_two_buffer_size", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "csm_bufsize[0]");
	RNA_def_property_range(prop, 128, 10240);
	RNA_def_property_ui_text(prop, "CSM Size Medium", "Shadow buffer resolution for the Middle cascade");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "shadow_buffer_soft_two", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "csm_soft[0]");
	RNA_def_property_range(prop, 0.0f, 100.0f);
	RNA_def_property_ui_text(prop, "CSM Shadow Soft Medium", "Sampling area size for the Middle cascade");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "shadow_buffer_bias_two", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "csm_bias[0]");
	RNA_def_property_range(prop, 0.001f, 5.0f);
	RNA_def_property_ui_text(prop, "CSM Bias Medium", "Sampling bias for the Middle cascade");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "shadow_csm_three_buffer_size", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "csm_bufsize[1]");
	RNA_def_property_range(prop, 128, 10240);
	RNA_def_property_ui_text(prop, "CSM Size Low", "Shadow buffer resolution for the Low/far cascade");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "shadow_buffer_soft_three", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "csm_soft[1]");
	RNA_def_property_range(prop, 0.0f, 100.0f);
	RNA_def_property_ui_text(prop, "CSM Shadow Soft Low", "Sampling area size for the Low/far cascade");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "shadow_buffer_bias_three", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "csm_bias[1]");
	RNA_def_property_range(prop, 0.001f, 5.0f);
	RNA_def_property_ui_text(prop, "CSM Bias Low", "Sampling bias for the Low/far cascade");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");
	...
	prop = RNA_def_property(srna, "shadow_blend", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "spotblend");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_ui_text(prop, "Shadow Fade", "The softness of the shadow edge");
	RNA_def_property_update(prop, 0, "rna_Lamp_draw_update");
...
static void rna_def_hemi_lamp(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "second_color", PROP_FLOAT, PROP_COLOR);
	RNA_def_property_float_sdna(prop, NULL, "shdwr");
	RNA_def_property_array(prop, 3);
	RNA_def_property_ui_text(prop, "Second Color", "Hemi lamp second color, of the oposite hemisphere");
	RNA_def_property_update(prop, 0, "rna_Lamp_update");

	prop = RNA_def_property(srna, "hemi_cutoff", PROP_FLOAT, PROP_FACTOR);
	RNA_def_property_float_sdna(prop, NULL, "cutoff");
	RNA_def_property_range(prop, 0.0f, 0.999f);
	RNA_def_property_ui_text(prop, "Cutoff", "Hemi cutoff threshold");
	RNA_def_property_update(prop, 0, "rna_Lamp_draw_update");
