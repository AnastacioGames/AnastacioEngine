...
static void rna_SCENEFXSettings_fx_update(Main *UNUSED(bmain), Scene *scene, PointerRNA *UNUSED(ptr))
{
	BKE_scene_fx_validate(scene);
}

static void rna_SCENEFXSettings_ssr_fx_update(Main *UNUSED(bmain), Scene *scene, PointerRNA *UNUSED(ptr))
{
	GameData *gm = &scene->gm;
	// SSR need gbuffer
	if (!gm->attachments[0]) {
		RenderAttachment *attach = MEM_callocN(sizeof(RenderAttachment), "RenderAttachment");
		attach->type = GAME_ATTACHMENT_GBUFF0;
		attach->size = 3;
		attach->hdr = GAME_HDR_HALF_FLOAT;
		BLI_strncpy(attach->name, "Attachment", MAX_NAME);
		gm->attachments[0] = attach;
	}
	else {
		gm->attachments[0]->type = GAME_ATTACHMENT_GBUFF0;
		gm->attachments[0]->hdr = GAME_HDR_HALF_FLOAT;
	}
...
static void rna_def_game_render_attachment(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem type_items[] = {
		{GAME_ATTACHMENT_CUSTOM, "CUSTOM", 0, "Custom", "Output custom data to a separate render attachment"},
		{GAME_ATTACHMENT_NORMAL, "NORMAL", 0, "Normal", "Output normal and roughness to a render attachment"},
		{GAME_ATTACHMENT_ALBEDO, "ALBEDO", 0, "Albedo", "Output diffuse color and intensity to a render attachment"},
		{GAME_ATTACHMENT_SPECULAR, "SPECULAR", 0, "Specular", "Output specular color and intensity to a render attachment"},
		{GAME_ATTACHMENT_GBUFF0, "GBUFF0", 0, "Gbuff0", "Compact Gbuffer: [normal.xy, rough/metallic, spec/diffuse_scale]"},
		{0, NULL, 0, NULL, NULL}
	};
...
static void rna_def_scene_game_data(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "use_dynamic_resolution", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "useDynamicResolution", 1);
	RNA_def_property_ui_text(prop, "Dynamic Resolution",
	                         "Adjust the internal game render resolution to maintain the target frame rate");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dynamic_resolution_target_fps", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "dynamicResolutionTargetFPS");
	RNA_def_property_range(prop, 30, 240);
	RNA_def_property_int_default(prop, 60);
	RNA_def_property_ui_text(prop, "Target FPS", "Target frame rate used by Dynamic Resolution");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dynamic_resolution_min_scale", PROP_INT, PROP_PERCENTAGE);
	RNA_def_property_int_sdna(prop, NULL, "dynamicResolutionMinScale");
	RNA_def_property_range(prop, 25, 100);
	RNA_def_property_int_default(prop, 50);
	RNA_def_property_ui_text(prop, "Minimum Scale", "Lowest internal render scale Dynamic Resolution may use");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dynamic_resolution_max_scale", PROP_INT, PROP_PERCENTAGE);
	RNA_def_property_int_sdna(prop, NULL, "dynamicResolutionMaxScale");
	RNA_def_property_range(prop, 25, 100);
	RNA_def_property_int_default(prop, 100);
	RNA_def_property_ui_text(prop, "Maximum Scale", "Highest internal render scale Dynamic Resolution may use");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dynamic_resolution_step", PROP_INT, PROP_PERCENTAGE);
	RNA_def_property_int_sdna(prop, NULL, "dynamicResolutionStep");
	RNA_def_property_range(prop, 1, 25);
	RNA_def_property_int_default(prop, 5);
	RNA_def_property_ui_text(prop, "Scale Step", "Scale adjustment made when Dynamic Resolution reacts to GPU load");
	RNA_def_property_update(prop, NC_SCENE, NULL);
	...
	prop = RNA_def_property(srna, "ignore_exit_key", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", GAME_PLAYER_IGNORE_EXIT_KEY);
	RNA_def_property_ui_text(prop, "Ignore Exit Key", "Ignores the exit key (WARNING: it will not be possible to close the game, force close)");
	...
	prop = RNA_def_property(srna, "borderless_window", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "playerflag", GAME_PLAYER_BORDERLESS_WINDOW);
	RNA_def_property_ui_text(prop, "Borderless Window", "Start player in a borderless window covering the full display, avoiding the crash some drivers hit with true fullscreen mode changes");
	RNA_def_property_update(prop, NC_SCENE, NULL);
	...
	RNA_def_property_int_sdna(prop, NULL, "shadowCulling");
	RNA_def_property_range(prop, 0, 1);
	RNA_def_property_ui_range(prop, 0, 1, 1, 1);
	RNA_def_property_int_default(prop, 1);
	RNA_def_property_ui_text(prop, "Shadow Culling",
	                         "Recalculate which objects cast visible shadows every frame: 0 disables it (cheaper, "
	                         "but shadows can go stale on moving objects), any nonzero value enables it. The value "
	                         "itself has no other effect. This is a separate setting from "
	                         "logic.getMaxPhysicsFrame()/setMaxPhysicsFrame(), which is deprecated and no longer "
	                         "affects shadows");
	RNA_def_property_update(prop, NC_SCENE, NULL);
	...
	prop = RNA_def_property(srna, "show_debug_mode", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", GAME_SHOW_DEBUG_MODE);
	RNA_def_property_ui_text(prop, "Show Debug Mode",
							 "Show game debug interface");

	prop = RNA_def_property(srna, "show_console", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "flag", GAME_SHOW_CONSOLE);
	RNA_def_property_ui_text(prop, "Show Console",
	                         "Show the in-game console window with engine log messages");
	...
	prop = RNA_def_property(srna, "cursor_filepath", PROP_STRING, PROP_FILEPATH);
	RNA_def_property_string_sdna(prop, NULL, "cursorimage_path");
	RNA_def_property_ui_text(prop, "Cursor Image Path",
							 "Mouse cursor custom image path");

	prop = RNA_def_property(srna, "cursor_size", PROP_INT, PROP_NONE);
	RNA_def_property_range(prop, 10, 500);
	RNA_def_property_int_default(prop, 20);
	RNA_def_property_ui_text(prop, "Cursor Size",
							"Set custom cursor size");

	prop = RNA_def_property(srna, "cursor_offset_x", PROP_INT, PROP_NONE);
	RNA_def_property_range(prop, -500, 500);
	RNA_def_property_int_default(prop, 0);
	RNA_def_property_ui_text(prop, "Cursor Offset X",
									"Move cursor in X position with offset");

	prop = RNA_def_property(srna, "cursor_offset_y", PROP_INT, PROP_NONE);
	RNA_def_property_range(prop, -500, 500);
	RNA_def_property_int_default(prop, 0);
	RNA_def_property_ui_text(prop, "Cursor Offset Y",
									"Move cursor in Y position with offset");

	prop = RNA_def_property(srna, "cursor_mipmap", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_negative_sdna(prop, NULL, "flag", GAME_CURSOR_USE_MIPMAP);
	RNA_def_property_ui_text(prop, "Use MipMap",
								   "Use mipmap on cursor");
	...
	RNA_def_property_boolean_sdna(prop, NULL, "flag", GAME_USE_SHADING_NODES);
	RNA_def_property_ui_text(prop, "PBR Shading Nodes",
...
static void rna_def_scene_gpu_bloom_fx(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	srna = RNA_def_struct(brna, "GPUBLOOMSettings", NULL);
	RNA_def_struct_ui_text(srna, "GPU BLOOM", "Settings for GPU based bloom");
	RNA_def_struct_ui_icon(srna, ICON_RENDERLAYERS);

	prop = RNA_def_property(srna, "intensity", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Bloom Intensity", "Bloom intensity");
	RNA_def_property_range(prop, 0.0f, 10.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "threshold", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Bloom Threshold", "Bloom threshold");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
}

static void rna_def_scene_gpu_tonemap_fx(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	static const EnumPropertyItem custom_tonemap_items[] = {
		{0, "TONEMAP", 0, "TONEMAP", "Tonemap custom shader"},
		{1, "ACES", 0, "ACES", "ACES custom shader"},
		{2, "AGX", 0, "AGX", "AGX custom shader"},
		{3, "Filmic", 0, "Filmic", "Filmic custom shader"},
		{0, NULL, 0, NULL, NULL}
	};

	srna = RNA_def_struct(brna, "GPUTONEMAPSettings", NULL);
	RNA_def_struct_ui_text(srna, "GPU TONEMAP", "Settings for GPU based tonemap");
	RNA_def_struct_ui_icon(srna, ICON_RENDERLAYERS);

	prop = RNA_def_property(srna, "shadertype", PROP_ENUM, PROP_NONE);
	RNA_def_property_ui_text(prop, "shader type", "Tonemap shader type");
	RNA_def_property_enum_bitflag_sdna(prop, NULL, "shadertype");
	RNA_def_property_enum_items(prop, custom_tonemap_items);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "exposure", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "exposure", "Tonemap exposure");
	RNA_def_property_range(prop, 0.0f, 100.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "gamma", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Gamma", "Tonemap gamma");
	RNA_def_property_range(prop, 0.0f, 10.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
}

static void rna_def_scene_gpu_lightscatter_fx(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	srna = RNA_def_struct(brna, "GPULIGHTSCATTERSettings", NULL);
	RNA_def_struct_ui_text(srna, "GPU LIGHT SCATTER", "Settings for GPU based light scatter");
	RNA_def_struct_ui_icon(srna, ICON_RENDERLAYERS);

	prop = RNA_def_property(srna, "intensity", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Intensity", "Scatter intensity");
	RNA_def_property_range(prop, 0.0f, 10.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "threshold", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Threshold", "Scatter threshold");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "stepsize", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Step Size", "Scatter step size");
	RNA_def_property_range(prop, 0.0f, 10.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "stepmax", PROP_INT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Step Max", "Number of samples");
	RNA_def_property_range(prop, 1, 500);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
}

static void rna_def_scene_gpu_ssr_fx(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;
	...
	RNA_def_property_ui_text(prop, "Step Max", "SSR step max");
	RNA_def_property_range(prop, 0.0f, 1024.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "roughness", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Roughness", "SSR roughness");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "bias", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Bias", "SSR bias");
	RNA_def_property_range(prop, 0.0f, 10.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "max_distance", PROP_FLOAT, PROP_NONE);
	RNA_def_property_ui_text(prop, "Max Distance", "SSR max distance");
	RNA_def_property_range(prop, 0.0f, 1000.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
}

static void rna_def_scene_gpu_ssao_fx(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;
...
static void rna_def_scene_shaders_fx(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	static const EnumPropertyItem custom_offscreen_lod_items[] = {
		{1, "1", 0, "1", "Max quality"},
		{2, "2", 0, "2", "Half quality"},
		{4, "4", 0, "4", "Quarter quality"},
		{8, "8", 0, "8", "Eighth quality"},
		{0, NULL, 0, NULL, NULL}
	};

	rna_def_scene_gpu_bloom_fx(brna);
	rna_def_scene_gpu_tonemap_fx(brna);
	rna_def_scene_gpu_lightscatter_fx(brna);
	rna_def_scene_gpu_ssr_fx(brna);
	rna_def_scene_gpu_ssao_fx(brna);

	srna = RNA_def_struct(brna, "SCENEFXSettings", NULL);
	RNA_def_struct_ui_text(srna, "Scene GPU FX Settings", "Settings for GPU based compositing");
	RNA_def_struct_ui_icon(srna, ICON_RENDERLAYERS);

	prop = RNA_def_property(srna, "bloom", PROP_POINTER, PROP_NONE);
	RNA_def_property_flag(prop, PROP_NEVER_NULL);
	RNA_def_property_struct_type(prop, "GPUBLOOMSettings");
	RNA_def_property_ui_text(prop, "Bloom settings", ""); 

	prop = RNA_def_property(srna, "use_bloom", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "scenefx_flag", SCENE_FX_FLAG_BLOOM);
	RNA_def_property_ui_text(prop, "BLOOM", "Use bloom on viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SCENEFXSettings_fx_update");

	prop = RNA_def_property(srna, "show_expanded_bloom", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "expand_flag", SCENE_FX_FLAG_BLOOM);
	RNA_def_property_ui_text(prop, "Expanded", "Set sensor expanded in the user interface");
	RNA_def_property_ui_icon(prop, ICON_RIGHTARROW, 1);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "render_editor_bloom", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "editor_render_flag", SCENE_FX_FLAG_BLOOM);
	RNA_def_property_ui_text(prop, "Render Bloom", "Render Bloom in Viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "tonemap", PROP_POINTER, PROP_NONE);
	RNA_def_property_flag(prop, PROP_NEVER_NULL);
	RNA_def_property_struct_type(prop, "GPUTONEMAPSettings");
	RNA_def_property_ui_text(prop, "Tonemap settings", ""); 

	prop = RNA_def_property(srna, "use_tonemap", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "scenefx_flag", SCENE_FX_FLAG_TONEMAP);
	RNA_def_property_ui_text(prop, "TONEMAP", "Use tonemap on viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SCENEFXSettings_fx_update");

	prop = RNA_def_property(srna, "show_expanded_tonemap", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "expand_flag", SCENE_FX_FLAG_TONEMAP);
	RNA_def_property_ui_text(prop, "Expanded", "Set sensor expanded in the user interface");
	RNA_def_property_ui_icon(prop, ICON_RIGHTARROW, 1);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "render_editor_tonemap", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "editor_render_flag", SCENE_FX_FLAG_TONEMAP);
	RNA_def_property_ui_text(prop, "Render Tonemap", "Render Tonemap in Viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "scatter", PROP_POINTER, PROP_NONE);
	RNA_def_property_flag(prop, PROP_NEVER_NULL);
	RNA_def_property_struct_type(prop, "GPULIGHTSCATTERSettings");
	RNA_def_property_ui_text(prop, "Light Scatter settings", ""); 

	prop = RNA_def_property(srna, "use_lightscatter", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "scenefx_flag", SCENE_FX_FLAG_LIGHTSCATTER);
	RNA_def_property_ui_text(prop, "LIGHTSCATTER", "Use light scatter on viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SCENEFXSettings_fx_update");

	prop = RNA_def_property(srna, "show_expanded_lightscatter", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "expand_flag", SCENE_FX_FLAG_LIGHTSCATTER);
	RNA_def_property_ui_text(prop, "Expanded", "Set sensor expanded in the user interface");
	RNA_def_property_ui_icon(prop, ICON_RIGHTARROW, 1);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "render_editor_lightscatter", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "editor_render_flag", SCENE_FX_FLAG_LIGHTSCATTER);
	RNA_def_property_ui_text(prop, "Render Light Scatter", "Render Light Scatter in Viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "scatter_lod", PROP_ENUM, PROP_NONE);
	RNA_def_property_ui_text(prop, "Light Scattering Lod", "Resolution at which the Light Scattering will be rendered on the screen");
	RNA_def_property_enum_bitflag_sdna(prop, NULL, "scatter_lod");
	RNA_def_property_enum_items(prop, custom_offscreen_lod_items);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_boolean_sdna(prop, NULL, "scenefx_flag", SCENE_FX_FLAG_SSR);
	RNA_def_property_ui_text(prop, "SSR", "Use screen space reflections on viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SCENEFXSettings_ssr_fx_update");

	prop = RNA_def_property(srna, "show_expanded_ssr", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "expand_flag", SCENE_FX_FLAG_SSR);
	RNA_def_property_ui_text(prop, "Expanded", "Set sensor expanded in the user interface");
	RNA_def_property_ui_icon(prop, ICON_RIGHTARROW, 1);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "render_editor_ssr", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "editor_render_flag", SCENE_FX_FLAG_SSR);
	RNA_def_property_ui_text(prop, "Render SSR", "Render SSR in Viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "ssr_lod", PROP_ENUM, PROP_NONE);
	RNA_def_property_ui_text(prop, "SSR Lod", "Resolution at which the SSR will be rendered on the screen");
	RNA_def_property_enum_bitflag_sdna(prop, NULL, "ssr_lod");
	RNA_def_property_enum_items(prop, custom_offscreen_lod_items);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_boolean_sdna(prop, NULL, "scenefx_flag", SCENE_FX_FLAG_SSAO);
	RNA_def_property_ui_text(prop, "SSAO", "Use screen space ambient occlusion of field on viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SCENEFXSettings_fx_update");

	prop = RNA_def_property(srna, "show_expanded_ssao", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "expand_flag", SCENE_FX_FLAG_SSAO);
	RNA_def_property_ui_text(prop, "Expanded", "Set sensor expanded in the user interface");
	RNA_def_property_ui_icon(prop, ICON_RIGHTARROW, 1);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "render_editor_ssao", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "editor_render_flag", SCENE_FX_FLAG_SSAO);
	RNA_def_property_ui_text(prop, "Render SSAO", "Render SSAO in Viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "use_fxaa", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "scenefx_flag", SCENE_FX_FLAG_FXAA);
	RNA_def_property_ui_text(prop, "FXAA", "Use FXAA on viewport");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, "rna_SCENEFXSettings_fx_update");
...
static void rna_def_cutscene_events(BlenderRNA *brna, PropertyRNA *cprop)
{
	StructRNA *srna;

	RNA_def_property_srna(cprop, "CutsceneEvents");
	srna = RNA_def_struct(brna, "CutsceneEvents", NULL);
	RNA_def_struct_sdna(srna, "CutsceneSequence");
	RNA_def_struct_ui_text(srna, "Cutscene Events", "Events in a cutscene sequence");
}

static void rna_def_cutscene_sequence(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	srna = RNA_def_struct(brna, "CutsceneSequence", NULL);
	RNA_def_struct_sdna(srna, "CutsceneSequence");
	RNA_def_struct_ui_text(srna, "Cutscene Sequence", "Ordered collection of cutscene events");
	RNA_def_struct_ui_icon(srna, ICON_SEQUENCE);

	prop = RNA_def_property(srna, "name", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "name");
	RNA_def_property_ui_text(prop, "Name", "Sequence name");
	RNA_def_struct_name_property(srna, prop);
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "cutscene_id", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "cutscene_id");
	RNA_def_property_ui_text(prop, "Cutscene ID", "Numeric ID grouping this sequence within the scene, matching the old addon's IDCutscene");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "events", PROP_COLLECTION, PROP_NONE);
	RNA_def_property_collection_sdna(prop, NULL, "events", NULL);
	RNA_def_property_struct_type(prop, "CutsceneEvent");
	RNA_def_property_ui_text(prop, "Events", "Events in this sequence");
	rna_def_cutscene_events(brna, prop);
}

static void rna_def_cutscene_event(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	static const EnumPropertyItem event_type_items[] = {
		{CUTSCENE_EVENT_SPAWN_OBJECT, "SPAWN_OBJECT", ICON_OBJECT_DATA, "Spawn Object", "Spawn an object at the event time"},
		{CUTSCENE_EVENT_DIALOG, "DIALOG", ICON_TEXT, "Dialog", "Show a localized dialog line"},
		{CUTSCENE_EVENT_HIDE_DIALOG, "HIDE_DIALOG", ICON_TEXT, "Hide Dialog", "Hide the current dialog"},
		{CUTSCENE_EVENT_CAMERA_SHOT, "CAMERA_SHOT", ICON_CAMERA_DATA, "Camera Shot", "Jump the cutscene camera to a shot order"},
		{CUTSCENE_EVENT_LOOK_AT, "LOOK_AT", ICON_CAMERA_DATA, "Look At", "Orient the player camera towards a target"},
		{CUTSCENE_EVENT_RESTORE_GAMEPLAY_CAMERA, "RESTORE_GAMEPLAY_CAMERA", ICON_CAMERA_DATA, "Restore Gameplay Camera", "Restore the regular gameplay camera"},
		{CUTSCENE_EVENT_CAMERA_START, "CAMERA_START", ICON_CAMERA_DATA, "Camera Start", "Start the cutscene camera manager"},
		{CUTSCENE_EVENT_CAMERA_STOP, "CAMERA_STOP", ICON_CAMERA_DATA, "Camera Stop", "Stop the cutscene camera manager"},
		{CUTSCENE_EVENT_LOCK_PLAYER, "LOCK_PLAYER", ICON_LOCKED, "Lock Player", "Lock player input for the cutscene"},
		{CUTSCENE_EVENT_UNLOCK_PLAYER, "UNLOCK_PLAYER", ICON_UNLOCKED, "Unlock Player", "Unlock player input"},
		{CUTSCENE_EVENT_PLAYER_ANIM, "PLAYER_ANIM", ICON_ARMATURE_DATA, "Player Anim", "Play an animation on the player"},
		{CUTSCENE_EVENT_SHOW_MOUSE, "SHOW_MOUSE", ICON_RESTRICT_SELECT_OFF, "Show Mouse", "Show the mouse cursor"},
		{CUTSCENE_EVENT_HIDE_MOUSE, "HIDE_MOUSE", ICON_RESTRICT_SELECT_ON, "Hide Mouse", "Hide the mouse cursor"},
		{CUTSCENE_EVENT_CHANGE_SCENE, "CHANGE_SCENE", ICON_SCENE_DATA, "Change Scene", "Switch to another scene"},
	...
	srna = RNA_def_struct(brna, "CutsceneEvent", NULL);
	RNA_def_struct_sdna(srna, "CutsceneEvent");
	RNA_def_struct_ui_text(srna, "Cutscene Event", "Timed cutscene event");

	prop = RNA_def_property(srna, "name", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "name");
	RNA_def_property_ui_text(prop, "Name", "Event name");
	RNA_def_struct_name_property(srna, prop);
	RNA_def_property_update(prop, NC_SCENE, NULL);
	...
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_text(prop, "Time", "Time in seconds at which the event is dispatched");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "type", PROP_ENUM, PROP_NONE);
	RNA_def_property_enum_sdna(prop, NULL, "type");
	RNA_def_property_enum_items(prop, event_type_items);
	RNA_def_property_ui_text(prop, "Type", "Event action");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "template_object", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "template_object");
	RNA_def_property_struct_type(prop, "Object");
	RNA_def_property_flag(prop, PROP_EDITABLE | PROP_ID_REFCOUNT);
	RNA_def_property_ui_text(prop, "Template Object", "Object used as the spawn template");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "spawn_point", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "spawn_point");
	RNA_def_property_struct_type(prop, "Object");
	RNA_def_property_flag(prop, PROP_EDITABLE | PROP_ID_REFCOUNT);
	RNA_def_property_ui_text(prop, "Spawn Point", "Object whose transform defines the spawn point");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dependent_object", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "dependent_object");
	RNA_def_property_struct_type(prop, "Object");
	RNA_def_property_flag(prop, PROP_EDITABLE | PROP_ID_REFCOUNT);
	RNA_def_property_ui_text(prop, "Dependent Object", "Optional object spawned with and parented to the template");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	/* Dialog */
	prop = RNA_def_property(srna, "dialog_text_en", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "text_en");
	RNA_def_property_ui_text(prop, "English Text", "Dialog text shown when the language is English");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dialog_text_pt", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "text_pt");
	RNA_def_property_ui_text(prop, "Portuguese Text", "Dialog text shown when the language is Portuguese");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dialog_text_es", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "text_es");
	RNA_def_property_ui_text(prop, "Spanish Text", "Dialog text shown when the language is Spanish");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dialog_text_ru", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "text_ru");
	RNA_def_property_ui_text(prop, "Russian Text", "Dialog text shown when the language is Russian");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dialog_audio_path", PROP_STRING, PROP_FILEPATH);
	RNA_def_property_string_sdna(prop, NULL, "audio_path");
	RNA_def_property_ui_text(prop, "Audio", "Optional voice line audio file played with the dialog");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "dialog_live", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "param_bool", 1);
	RNA_def_property_ui_text(prop, "Live", "Use the live dialogue overlay instead of the standard one");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	/* Camera */
	prop = RNA_def_property(srna, "camera_order", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "param_int");
	RNA_def_property_ui_text(prop, "Shot Order", "Index of the camera shot to jump to");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "look_at_target", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "template_object");
	RNA_def_property_struct_type(prop, "Object");
	RNA_def_property_flag(prop, PROP_EDITABLE | PROP_ID_REFCOUNT);
	...
	/* Player */
	prop = RNA_def_property(srna, "player_anim_name", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "param_str_a");
	RNA_def_property_ui_text(prop, "Animation", "Name of the animation to play on the player");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "player_anim_blend", PROP_FLOAT, PROP_TIME);
	RNA_def_property_float_sdna(prop, NULL, "param_float");
	RNA_def_property_ui_text(prop, "Blend", "Blend time in seconds for the player animation");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	/* Scene */
	prop = RNA_def_property(srna, "change_scene_name", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "param_str_a");
	RNA_def_property_ui_text(prop, "Scene", "Name of the scene to switch to");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	/* Wait */
	prop = RNA_def_property(srna, "wait_seconds", PROP_FLOAT, PROP_TIME);
	RNA_def_property_float_sdna(prop, NULL, "param_float");
	RNA_def_property_range(prop, 0.0f, FLT_MAX);
	RNA_def_property_ui_text(prop, "Seconds", "Duration to wait before resuming the timeline");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "trigger_name", PROP_STRING, PROP_NONE);
	RNA_def_property_string_sdna(prop, NULL, "param_str_a");
	RNA_def_property_ui_text(prop, "Trigger", "Name of the message or collision trigger to wait for");
	RNA_def_property_update(prop, NC_SCENE, NULL);
}

static void rna_def_cutscene_settings(BlenderRNA *brna)
{
	StructRNA *srna;
	PropertyRNA *prop;

	srna = RNA_def_struct(brna, "CutsceneSettings", NULL);
	RNA_def_struct_sdna(srna, "CutsceneSettings");
	RNA_def_struct_ui_text(srna, "Cutscene Settings", "Cutscene data owned by a scene");

	prop = RNA_def_property(srna, "sequences", PROP_COLLECTION, PROP_NONE);
	RNA_def_property_collection_sdna(prop, NULL, "sequences", NULL);
	RNA_def_property_struct_type(prop, "CutsceneSequence");
	RNA_def_property_ui_text(prop, "Sequences", "Sequences in this scene");

	prop = RNA_def_property(srna, "active_sequence_index", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "active_sequence");
	RNA_def_property_ui_text(prop, "Active Sequence Index", "Index of the active cutscene sequence");
	RNA_def_property_update(prop, NC_SCENE, NULL);

	prop = RNA_def_property(srna, "active_event_index", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "active_event");
	RNA_def_property_ui_text(prop, "Active Event Index", "Index of the active event in the active sequence");
	RNA_def_property_update(prop, NC_SCENE, NULL);
}
...
void RNA_def_scene(BlenderRNA *brna)
{
	...
	prop = RNA_def_property(srna, "world_sun_set", PROP_POINTER, PROP_NONE);
	RNA_def_property_pointer_sdna(prop, NULL, "world_sun");
	RNA_def_property_struct_type(prop, "Object");
	RNA_def_property_flag(prop, PROP_EDITABLE | PROP_ID_SELF_CHECK);
	RNA_def_property_pointer_funcs(prop, NULL, NULL, NULL, "rna_Lamp_object_poll");
	RNA_def_property_ui_text(prop, "World Sun", "Main sun of the scene");
	RNA_def_property_update(prop, NC_SCENE | NA_EDITED, "rna_Scene_glsl_update");
	...
	// Viewport shaders
	prop = RNA_def_property(srna, "scenefx_settings", PROP_POINTER, PROP_NONE);
	RNA_def_property_ui_text(prop, "Scene FX Options", "Options used for real time compositing");
	RNA_def_property_update(prop, NC_SCENE, NULL);
	...
	prop = RNA_def_property(srna, "cutscene_settings", PROP_POINTER, PROP_NONE);
	RNA_def_property_flag(prop, PROP_NEVER_NULL);
	RNA_def_property_pointer_sdna(prop, NULL, "cutscene_settings");
	RNA_def_property_struct_type(prop, "CutsceneSettings");
	RNA_def_property_ui_text(prop, "Cutscene Settings", "Cutscene data stored in this scene");
	...
	prop = RNA_def_property(srna, "audio3d_update", PROP_INT, PROP_NONE);
	RNA_def_property_int_sdna(prop, NULL, "audio.audio3d_update");
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_range(prop, 0, 1000);
	RNA_def_property_ui_text(prop, "3D Sound Update Frequency", "Frequency that will update the 3D position of the sounds (in frames)");
	...
	rna_def_cutscene_settings(brna);
	rna_def_cutscene_sequence(brna);
	rna_def_cutscene_event(brna);
