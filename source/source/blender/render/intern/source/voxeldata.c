...
/* arbitrary but generous cap so a corrupt/malicious header can't trigger a
 * multi-terabyte allocation attempt */
#define VOXELDATA_MAX_BYTES ((size_t)2 * 1024 * 1024 * 1024)
...
static int load_frame_blendervoxel(VoxelData *vd, FILE *fp, int frame)
{
	...
	if (size > VOXELDATA_MAX_BYTES / sizeof(float))
		return 0;

	vd->dataset = MEM_mapallocN(sizeof(float) * size, "voxel dataset");
	if (vd->dataset == NULL) return 0;

	if (fseek(fp, (size_t)frame * size * sizeof(float) + offset, 0) == -1) {
