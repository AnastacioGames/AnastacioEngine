...
static FileData *blo_decode_and_check(FileData *fd, ReportList *reports)
{
	...
	if (fd->flags & FD_FLAGS_FILE_OK) {
		...
		BKE_reportf(reports, RPT_ERROR, "Failed to read range file '%s', not a range file", fd->relabase);
		blo_freefiledata(fd);
		fd = NULL;
	}
	...
	if (typeencryption <= SPINDLE_NO_ENCRYPTION) {
		...
		if (G.canOpenProtectedFile) {
			...
			sprintf(key, "d43d6c30-db11-417b-bf60-c8685c9311bd69ad9683-d89b-4d82-837c-f5cd2e106a38b54c60aa-9a35-4106-9d4f-d08ed4526831%s%.1d%.1d%s",
				"RANGE-ENGINE", RANGE_VERSION, RANGE_SUBVERSION, "2fd68263-8ca7-4d17-a4dd-18fd9446eb59dd4a199f-d16b-4f28-85ba-898456042ff583ed679a-fb33-4268-b981-9e6a4603fff621a89f43-68d9-4ed5-9ee8-3424baaf15925fd40d87-8a59-4458-b154-1dc150443817");
			const int key_size = sizeof(key) - 1;
			const int byteArray = 500;
			char buffer[1024];

			// Find the position of key in the buffer...
			gzseek(gzfile, (byteArray / 2 + SIZEOFBLENDERHEADER), SEEK_SET);
			gzread(gzfile, buffer, key_size);

			// Check if has security...
			if (memcmp(buffer, key, key_size) == 0) {
				//printf("RangeArmor Encryption Detected\n");
				gzseek(gzfile, 0, SEEK_SET);
				gzread(gzfile, buffer, key_size + byteArray + SIZEOFBLENDERHEADER);
				G.usePtrFile = true;
			}
			else {
				gzseek(gzfile, 0, SEEK_SET);
...
{
	const char *ext_test[5] = {".blend", ".ble", ".blend.gz", ".range", NULL};
...
static void direct_link_animdata(FileData *fd, AnimData *adt)
{
	...
	/* link overrides */
	link_list(fd, &adt->overrides);
	for (AnimOverride *aor = adt->overrides.first; aor; aor = aor->next) {
		aor->rna_path = newdataadr(fd, aor->rna_path);
	}
	...
	for (sock = gnode->outputs.first; sock; sock = sock->next) {
		...
		for (link = ngroup->links.first; link; link = link->next) {
			if (link->tonode == NULL && link->fromsock->own_index == old_index) {
				BLI_strncpy(sock->identifier, link->fromsock->identifier, sizeof(sock->identifier));
				/* deprecated */
	...
	for (sock = gnode->inputs.first; sock; sock = sock->next) {
		...
		for (link = ngroup->links.first; link; link = link->next) {
			if (link->fromnode == NULL && link->tosock->own_index == old_index) {
				BLI_strncpy(sock->identifier, link->tosock->identifier, sizeof(sock->identifier));
				/* deprecated */
...
static void direct_link_world(FileData *fd, World *wrld)
{
	...
	link_list(fd, &wrld->prop);
	for (bProperty *prop = wrld->prop.first; prop; prop = prop->next) {
...
static void lib_link_material(FileData *fd, Main *main)
{
	for (Material *ma = main->mat.first; ma; ma = ma->id.next) {
		if (ma->id.tag & LIB_TAG_NEED_LINK) {
			...
			ma->fragcode = newlibadr_us(fd, ma->id.lib, ma->fragcode);
			ma->vertcode = newlibadr_us(fd, ma->id.lib, ma->vertcode);
			...
			for (int a = 0; a < MAX_MTEX; a++) {
				...
				if (mtex) {
					...
					mtex->group_object_decal = newlibadr(fd, ma->id.lib, mtex->group_object_decal);
				}
			}
...
static void direct_link_material(FileData *fd, Material *ma)
{
	...
	BLI_listbase_clear(&ma->gpumaterialskinning);
}
...
// >>> XXX deprecated - old animation system
			...
			ob->collision_bound = newlibadr(fd, ob->id.lib, ob->collision_bound);
			ob->gpu_particles.collision_ground_object = newlibadr(fd, ob->id.lib, ob->gpu_particles.collision_ground_object);
...
// >>> XXX deprecated - old animation system
			...
			{
				bWheelSettings *wheel;
				for (wheel = ob->vehicle_wheels.first; wheel; wheel = wheel->next) {
					wheel->ob = newlibadr(fd, ob->id.lib, wheel->ob);
				}
			}
...
// >>> XXX deprecated - old animation system
	...
	ob->gpu_particles.size_curve = newdataadr(fd, ob->gpu_particles.size_curve);
	if (ob->gpu_particles.size_curve) {
		direct_link_curvemapping(fd, ob->gpu_particles.size_curve);
	}
	ob->gpu_particles.color_curve = newdataadr(fd, ob->gpu_particles.color_curve);
	if (ob->gpu_particles.color_curve) {
		direct_link_curvemapping(fd, ob->gpu_particles.color_curve);
	}
	...
	link_list(fd, &ob->vehicle_wheels);
	link_list(fd, &ob->vehicle_gears);

	link_list(fd, &ob->animevents);
	/* We have another ListBase saved inside each AnimationEvent animevents.. ? */
	AnimationEvent *event;
	for (event = ob->animevents.first; event; event = event->next) {
		link_list(fd, &event->triggers);
	}
...
static void lib_link_scene(FileData *fd, Main *main)
{
	...
	for (Scene *sce = main->scene.first; sce; sce = sce->id.next) {
		if (sce->id.tag & LIB_TAG_NEED_LINK) {
			...
			sce->world_sun = newlibadr_us(fd, sce->id.lib, sce->world_sun);
			sce->set = newlibadr(fd, sce->id.lib, sce->set);
			sce->gpd = newlibadr_us(fd, sce->id.lib, sce->gpd);

			if (sce->cutscene_settings) {
				for (CutsceneSequence *sequence = sce->cutscene_settings->sequences.first; sequence;
				     sequence = sequence->next)
				{
					for (CutsceneEvent *event = sequence->events.first; event; event = event->next) {
						event->template_object = newlibadr_us(fd, sce->id.lib, event->template_object);
						event->spawn_point = newlibadr_us(fd, sce->id.lib, event->spawn_point);
						event->dependent_object = newlibadr_us(fd, sce->id.lib, event->dependent_object);
					}
				}
			}
...
static void direct_link_scene(FileData *fd, Scene *sce)
{
	...
	sce->cutscene_settings = newdataadr(fd, sce->cutscene_settings);
	if (sce->cutscene_settings) {
		link_list(fd, &sce->cutscene_settings->sequences);
		for (CutsceneSequence *sequence = sce->cutscene_settings->sequences.first; sequence;
		     sequence = sequence->next)
		{
			link_list(fd, &sequence->events);
		}
	}
	...
	if (sce->scenefx_settings.bloom)
		sce->scenefx_settings.bloom = newdataadr(fd, sce->scenefx_settings.bloom);
	if (sce->scenefx_settings.tonemap)
		sce->scenefx_settings.tonemap = newdataadr(fd, sce->scenefx_settings.tonemap);
	if (sce->scenefx_settings.scatter)
		sce->scenefx_settings.scatter = newdataadr(fd, sce->scenefx_settings.scatter);
	if (sce->scenefx_settings.ssr)
		sce->scenefx_settings.ssr = newdataadr(fd, sce->scenefx_settings.ssr);
	if (sce->scenefx_settings.ssao)
		sce->scenefx_settings.ssao = newdataadr(fd, sce->scenefx_settings.ssao);
...
static BHead *read_global(BlendFileData *bfd, FileData *fd, BHead *bhead)
{
	...
	bfd->main->rangeversionfile = fg->rangeversion;
	bfd->main->rangesubversionfile = fg->rangesubversion;
	bfd->main->rangeminsubversionfile = fg->rangeminsubversion;
...
static BHead *read_userdef(BlendFileData *bfd, FileData *fd, BHead *bhead)
{
	...
	/* User struct for Range Engine versioning */
	user->rangeversionfile = bfd->main->rangeversionfile;
	user->rangesubversionfile = bfd->main->rangesubversionfile;
	user->rangeminsubversionfile = bfd->main->rangeminsubversionfile;
...
static void expand_material(FileData *fd, Main *mainvar, Material *ma)
{
	...
	if (ma->vertcode)
		expand_doit(fd, mainvar, ma->vertcode);

	if (ma->fragcode)
		expand_doit(fd, mainvar, ma->fragcode);
}
...
// XXX deprecated - old animation system (for version patching only)
	...
	if (ob->gpu_particles.collision_ground_object)
		expand_doit(fd, mainvar, ob->gpu_particles.collision_ground_object);
...

	{
		bWheelSettings *wheel;
		for (wheel = ob->vehicle_wheels.first; wheel; wheel = wheel->next) {
			expand_doit(fd, mainvar, wheel->ob);
		}
	}
...
static void expand_scene(FileData *fd, Main *mainvar, Scene *sce)
{
	...
	expand_doit(fd, mainvar, sce->world_sun);
			...
			if (mainvar_id_tag_any_check(mainptr, LIB_TAG_ID_ID)) {
				...
				if (fd == NULL) {
					...
					if (mainptr->curlib->packedfile) {
						...
						// check.. if so this file is a link and not the main file.
						if (G.usePtrFile) {
							// check if the current file link exists, if not, try rasec.
							if (!BLI_access(mainptr->curlib->filepath, R_OK) == 0 && BLI_path_extension_check(mainptr->curlib->filepath, ".range")) {
								BLI_path_extension_replace(mainptr->curlib->filepath, FILE_MAX, ".rasec");
							}
							else {
								G.usePtrFile = false;
							}
