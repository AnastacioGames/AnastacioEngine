...
static void write_object(WriteData *wd, Object *ob)
{
	if (ob->id.us > 0 || wd->use_memfile) {
		...
		if (ob->gpu_particles.size_curve) {
			write_curvemapping(wd, ob->gpu_particles.size_curve);
		}
		if (ob->gpu_particles.color_curve) {
			write_curvemapping(wd, ob->gpu_particles.color_curve);
		}
		...
		writelist(wd, DATA, bWheelSettings, &ob->vehicle_wheels);
		writelist(wd, DATA, bGearRatio, &ob->vehicle_gears);

		writelist(wd, DATA, AnimationEvent, &ob->animevents);
		AnimationEvent *event;
		for (event = ob->animevents.first; event; event = event->next) {
			writelist(wd, DATA, AnimationEventTrigger, &event->triggers);
		}
...
static void write_scene(WriteData *wd, Scene *sce)
{
	...
	if (sce->cutscene_settings) {
		writestruct(wd, DATA, CutsceneSettings, 1, sce->cutscene_settings);
		writelist(wd, DATA, CutsceneSequence, &sce->cutscene_settings->sequences);
		for (CutsceneSequence *sequence = sce->cutscene_settings->sequences.first; sequence;
		     sequence = sequence->next)
		{
			writelist(wd, DATA, CutsceneEvent, &sequence->events);
		}
	}
	if (tos->vpaint) {
	...
	if (sce->scenefx_settings.bloom) {
		writestruct(wd, DATA, GPUBLOOMSettings, 1, sce->scenefx_settings.bloom);
	}
	if (sce->scenefx_settings.tonemap) {
		writestruct(wd, DATA, GPUTONEMAPSettings, 1, sce->scenefx_settings.tonemap);
	}
	if (sce->scenefx_settings.scatter) {
		writestruct(wd, DATA, GPULIGHTSCATTERSettings, 1, sce->scenefx_settings.scatter);
	}
	if (sce->scenefx_settings.ssr) {
		writestruct(wd, DATA, GPUSSRSettings, 1, sce->scenefx_settings.ssr);
	}
	if (sce->scenefx_settings.ssao) {
		writestruct(wd, DATA, GPUSSAOSettings, 1, sce->scenefx_settings.ssao);
	}
...
static void write_global(WriteData *wd, int fileflags, Main *mainvar)
{
	...
	fg.rangeversion = RANGE_VERSION;
	fg.rangesubversion = RANGE_SUBVERSION;
	fg.rangeminsubversion = RANGE_MINSUBVERSION;
        ...
        int write_flags, const BlendThumbnail *thumb)
{
	...
	if (hasPrtFile) {
		...
		printf("Protected Range Engine file created! \n");
	}
