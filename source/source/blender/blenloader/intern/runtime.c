...
BlendFileData *BLO_read_runtime(const char *path, ReportList *reports)
{
	...
	if (actualsize < 12) {
		BKE_reportf(reports, RPT_ERROR, "Unable to read '%s' (file too small)", path);
		goto cleanup;
	}
	...
	else if ((size_t)datastart > actualsize) {
		BKE_reportf(reports, RPT_ERROR, "Unable to read '%s' (invalid runtime offset)", path);
		goto cleanup;
	}
	else if (read(fd, buf, 8) != 8) {
