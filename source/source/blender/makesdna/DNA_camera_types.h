...
typedef struct Camera {
	...
	short flag;
	short gameflag;
	/* CSM cascade-matrix cache tolerance (repurposed padding, see versioning_range.c for the
	 * 1.6.108 migration that seeds old files): 0 disables the cache, 1 only reuses last frame's
	 * cascade matrices when this camera hasn't moved at all, 2 also tolerates one frame of small
	 * movement. See KX_ShadowRenderer::Render / KX_Camera::GetCSMCacheMaxStaleFrames. */
	short csmCacheMaxStaleFrames;
