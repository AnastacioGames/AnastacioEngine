...
typedef struct Lamp {
	...
	/* Cascaded Shadow Mapping (BGE/Range only, LA_SUN only).
	 * shadow_cascade_count == 0 means CSM disabled (legacy single shadow map).
	 * When enabled, the near cascade reuses bufsize/soft/bias above; csm_bufsize/csm_soft/csm_bias
	 * index 0/1 hold the middle/low cascades respectively. */
	short shadow_cascade_count, csm_flag;
	float shadow_cascade_proportion_near, shadow_cascade_proportion_middle;
	short csm_bufsize[2];
	float csm_soft[2];
	float csm_bias[2];
	...
	/* BGE Only: distance-based light culling ("light LOD"). 0 or LA_CULL_DISTANCE unset in mode
	 * means the light is never culled by distance (always evaluated, e.g. Sun lamps). */
	float cull_distance;
	/* BGE Only: scale multiplier for the distant-light glow billboard impostor (base size is
	 * dist * 0.05). Default 1.0 keeps the original size. */
	float glow_scale;
...
#define LA_SHADOW_FILTER_CLIPPING		1
#define LA_SHADOW_FILTER_DITHERING		2
#define LA_SHADOW_FILTER_PCF			3
#define LA_SHADOW_FILTER_PCF_BAIL		4
#define LA_SHADOW_FILTER_PCF_JITTER		5
#define LA_SHADOW_FILTER_PCF_PENUMBRA	6
