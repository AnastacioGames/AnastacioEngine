...
typedef struct bPropertySensor {
	...
	char value[64];
	char maxvalue[64];
	/* Optional runtime-property path. Legacy game properties continue using name. */
	struct Object *runtime_target;
	short runtime_enabled;
	short runtime_property;
	short runtime_bool_value;
	short runtime_pad;
	float runtime_value[3];
	float runtime_value_max[3];
} bPropertySensor;

typedef struct bAnimationEventSensor {
	int type;
	int index;
	int trigger_index;
	short trigger_all;
	short pad;
} bAnimationEventSensor;
...
typedef struct bDelaySensor {
	short delay;
	short pad;
	float delay_time; /* does not reside in SCA_DelaySensor, it is passed in place of "short delay" if needed */
	short duration;
	short flag;
	short repeat_times;
	short use_delta;
...
typedef struct bSensor {
	...
	short freq, totlinks; /* freq makes reference to skipped ticks between 2 active pulses */
	float freq_dt;
	char name[64];	/* MAX_NAME */
	void *data;
...
/* bPropertySensor.runtime_property */
#define SENS_RUNTIME_PROP_LOCAL_POSITION	0
#define SENS_RUNTIME_PROP_VISIBLE			1
#define SENS_RUNTIME_PROP_MASS				2
#define SENS_RUNTIME_PROP_LINEAR_VELOCITY	3
#define SENS_RUNTIME_PROP_ANGULAR_VELOCITY	4
#define SENS_RUNTIME_PROP_GRAVITY			5
#define SENS_RUNTIME_PROP_LOCAL_SCALE		6
...
#define SENS_DELTATIME	128
#define SENS_SHOW_DESCR 256 /* Show Sensor Description */
...
#define BL_SENS_MOUSE_LEFTTHUMBMOUSE 5
#define BL_SENS_MOUSE_RIGHTTHUMBMOUSE 6
#define BL_SENS_MOUSE_BUTTON6 7
#define BL_SENS_MOUSE_BUTTON7 8
#define BL_SENS_MOUSE_WHEEL_UP       9
#define BL_SENS_MOUSE_WHEEL_DOWN     10
#define BL_SENS_MOUSE_MOVEMENT       11
