...
typedef struct AudioData {
	...
	int audio3d_update; /* Game Engine */
	int distance_model;
	short flag;
	short pad;
	float volume;
...
/* Viewport/Gameengine shaders, before in DNA_gpu_types */

/* properties for BLOOM effect */
typedef struct GPUBLOOMSettings {
  float intensity;
  float threshold;
  int pad;
} GPUBLOOMSettings;

/* properties for TONEMAP effect */
typedef struct GPUTONEMAPSettings {
  short shadertype, pad;
  float exposure;
  float gamma;
} GPUTONEMAPSettings;

/* properties for Light Scatter effect */
typedef struct GPULIGHTSCATTERSettings {
  float intensity;
  float threshold;
  float stepsize;
  int stepmax;
} GPULIGHTSCATTERSettings;

/* properties for SSR effect */
typedef struct GPUSSRSettings {
  int step_max; // samples
  float roughness;
  float bias;
  float max_distance;
} GPUSSRSettings;
...
typedef struct SCENEFXSettings {
	GPUBLOOMSettings *bloom;
	GPUTONEMAPSettings *tonemap;
	GPULIGHTSCATTERSettings *scatter;
	GPUSSRSettings *ssr;
	GPUSSAOSettings *ssao;

	char scenefx_flag;  /* eSCENEFXFlags */
	char expand_flag; /* use eSCENEFXFlags but it's for show_expanded FX in editor tab */
	char editor_render_flag; /* use eSCENEFXFlags but for enable/disable viewport FX */
...
typedef enum eSCENEFXFlags {
  SCENE_FX_FLAG_BLOOM        = (1 << 0),
  SCENE_FX_FLAG_TONEMAP      = (1 << 1),
...
#define GAME_ATTACHMENT_SPECULAR 3
#define GAME_ATTACHMENT_GBUFF0 4
...
typedef struct GameData {
	...
	/* Shadow culling on/off, split out of the legacy `maxphystep` field (which the
	 * launcher still feeds into KX_KetsjiEngine::SetMaxPhysicsFrame() as a bool, unrelated
	 * to shadows). `maxphystep` itself is kept only for the deprecated Python
	 * getMaxPhysicsFrame()/setMaxPhysicsFrame() API and old-file compatibility; it drives
	 * nothing in the shadow pipeline anymore. Replaces the unused `pad5` slot, so old files
	 * (where it was always zero) are migrated via `maxphystep != 0` in versioning. */
	short shadowCulling;
	float timeScale;
	float levelHeight;
	...
	/* Custom Mouse Cursor */
	/* Dynamic render-resolution controller. Values are percentages except target FPS. */
	short useDynamicResolution;
	short dynamicResolutionTargetFPS;
...
#define GAME_SHOW_DEBUG_MODE				(1 << 17)
#ifdef DNA_DEPRECATED
...
#define GAME_PLAYER_IGNORE_EXIT_KEY			(1 << 23)
#define GAME_CURSOR_USE_MIPMAP				(1 << 24) // Custom Cursor
#define GAME_SHOW_CONSOLE					(1 << 25)
/* Plano 8 (docs/ketsji-engine-modernization-plan.md): opt into KX_KetsjiEngine's
...
#define GAME_PLAYER_BORDERLESS_WINDOW		(1 << 2)
...
typedef enum CutsceneEventType {
	CUTSCENE_EVENT_SPAWN_OBJECT = 0,
	CUTSCENE_EVENT_DIALOG = 1,
	CUTSCENE_EVENT_HIDE_DIALOG = 2,
	CUTSCENE_EVENT_CAMERA_SHOT = 3,
	CUTSCENE_EVENT_LOOK_AT = 4,
	CUTSCENE_EVENT_RESTORE_GAMEPLAY_CAMERA = 5,
	CUTSCENE_EVENT_CAMERA_START = 6,
	CUTSCENE_EVENT_CAMERA_STOP = 7,
	CUTSCENE_EVENT_LOCK_PLAYER = 8,
	CUTSCENE_EVENT_UNLOCK_PLAYER = 9,
	CUTSCENE_EVENT_PLAYER_ANIM = 10,
	CUTSCENE_EVENT_SHOW_MOUSE = 11,
	CUTSCENE_EVENT_HIDE_MOUSE = 12,
	CUTSCENE_EVENT_CHANGE_SCENE = 13,
...
typedef struct CutsceneEvent {
	struct CutsceneEvent *next, *prev;
	...
	/* Generic reusable slots, exposed under per-action RNA property names.
	 * Only one `type` is active per event, so these never collide. */
	char param_str_a[128];
	char param_str_b[128];
	float param_float;
	int param_int;
	int param_bool;
	int _pad[2];

	/* Dialog-only: localized text and audio (replaces addon's external
	 * text/audio lookup tables so cutscenes are self-contained in DNA). */
	char text_en[256];
	char text_pt[256];
	char text_es[256];
	char text_ru[256];
	...
	struct Object *template_object;
	struct Object *spawn_point;
	struct Object *dependent_object;
} CutsceneEvent;

typedef struct CutsceneSequence {
	struct CutsceneSequence *next, *prev;
...
typedef struct CutsceneSettings {
	ListBase sequences;
	int active_sequence;
	int active_event;
} CutsceneSettings;
...
typedef struct Scene {
	...
	struct Object *camera;
	struct World *world;
	struct Object *world_sun; /* used for world background sun direction */
	short *pad4;
	...
	/* Dependency graph pointers are not saved. Cutscene settings reuse the obsolete padding slot. */
	struct Depsgraph *depsgraph;
	struct CutsceneSettings *cutscene_settings;	/* default allocated now */
	...
	struct SCENEFXSettings scenefx_settings;
