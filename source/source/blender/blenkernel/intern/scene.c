...
// Range: Init Scene Shader Effects, only for Range legacy
void BKE_scene_fx_validate(Scene *scene)
{
	SCENEFXSettings fx_settings = scene->scenefx_settings;
	
	if ((fx_settings.bloom == NULL) &&
	    (fx_settings.scenefx_flag & SCENE_FX_FLAG_BLOOM))
	{
		GPUBLOOMSettings *fx_bloom;
		fx_bloom = fx_settings.bloom = MEM_callocN(sizeof(GPUBLOOMSettings), __func__);

		/* init settings */
		fx_bloom->intensity = 2.0f;
		fx_bloom->threshold = 0.75f;
	}
	if ((fx_settings.tonemap == NULL) &&
	    (fx_settings.scenefx_flag & SCENE_FX_FLAG_TONEMAP))
	{
		GPUTONEMAPSettings *fx_tonemap;
		fx_tonemap = fx_settings.tonemap = MEM_callocN(sizeof(GPUTONEMAPSettings), __func__);

		/* init settings */
		fx_tonemap->shadertype = 0.0f;
		fx_tonemap->exposure = 2.2f;
		fx_tonemap->gamma = 1.0f;
	}
	if ((fx_settings.scatter == NULL) &&
	    (fx_settings.scenefx_flag & SCENE_FX_FLAG_LIGHTSCATTER))
	{
		GPULIGHTSCATTERSettings *fx_scatter;
		fx_scatter = fx_settings.scatter = MEM_callocN(sizeof(GPULIGHTSCATTERSettings), __func__);

		/* init settings */
		fx_settings.scatter_lod = 4;
		fx_scatter->intensity = 0.2f;
		fx_scatter->threshold = 0.75f;
		fx_scatter->stepsize = 0.15f;
		fx_scatter->stepmax = 32;
	}
	if ((fx_settings.ssr == NULL) &&
	    (fx_settings.scenefx_flag & SCENE_FX_FLAG_SSR))
	{
		GPUSSRSettings *fx_ssr;
		fx_ssr = fx_settings.ssr = MEM_callocN(sizeof(GPUSSRSettings), __func__);

		/* init settings */
		fx_settings.ssr_lod = 2;
		fx_ssr->step_max = 16;
		fx_ssr->roughness = 0.1f;
		fx_ssr->bias = 3.0f;
		fx_ssr->max_distance = 100.0f;
	}
	if ((fx_settings.ssao == NULL) &&
	    (fx_settings.scenefx_flag & SCENE_FX_FLAG_SSAO))
	{
	...
	scene->scenefx_settings = fx_settings;
}
...
CutsceneSettings *BKE_cutscene_settings_new(void)
{
	return MEM_callocN(sizeof(CutsceneSettings), "Cutscene Settings");
}

CutsceneSettings *BKE_cutscene_settings_copy(const CutsceneSettings *settings)
{
	if (settings == NULL) {
		return NULL;
	}

	CutsceneSettings *settings_copy = MEM_dupallocN(settings);
	BLI_duplicatelist(&settings_copy->sequences, &settings->sequences);

	for (const CutsceneSequence *sequence_src = settings->sequences.first,
	                            *sequence_dst = settings_copy->sequences.first;
	     sequence_src;
	     sequence_src = sequence_src->next, sequence_dst = sequence_dst->next)
	{
		BLI_duplicatelist(&sequence_dst->events, &sequence_src->events);
	}

	return settings_copy;
}

void BKE_cutscene_settings_free(CutsceneSettings *settings)
{
	if (settings == NULL) {
		return;
	}

	for (CutsceneSequence *sequence = settings->sequences.first; sequence; sequence = sequence->next) {
		BLI_freelistN(&sequence->events);
	}
	BLI_freelistN(&settings->sequences);
	MEM_freeN(settings);
}
...
void BKE_scene_copy_data(Main *bmain, Scene *sce_dst, const Scene *sce_src, const int flag)
{
	...
	sce_dst->cutscene_settings = BKE_cutscene_settings_copy(sce_dst->cutscene_settings);
	...
	if (sce_src->scenefx_settings.bloom)
		sce_dst->scenefx_settings.bloom = MEM_dupallocN(sce_src->scenefx_settings.bloom);
	if (sce_src->scenefx_settings.tonemap)
		sce_dst->scenefx_settings.tonemap = MEM_dupallocN(sce_src->scenefx_settings.tonemap);
	if (sce_src->scenefx_settings.scatter)
		sce_dst->scenefx_settings.scatter = MEM_dupallocN(sce_src->scenefx_settings.scatter);
	if (sce_src->scenefx_settings.ssr)
		sce_dst->scenefx_settings.ssr = MEM_dupallocN(sce_src->scenefx_settings.ssr);
	if (sce_src->scenefx_settings.ssao)
		sce_dst->scenefx_settings.ssao = MEM_dupallocN(sce_src->scenefx_settings.ssao);
...
void BKE_scene_free(Scene *sce)
{
	...
	BKE_cutscene_settings_free(sce->cutscene_settings);
	sce->cutscene_settings = NULL;
	...
	if (sce->scenefx_settings.bloom)
		MEM_freeN(sce->scenefx_settings.bloom);
	if (sce->scenefx_settings.tonemap)
		MEM_freeN(sce->scenefx_settings.tonemap);
	if (sce->scenefx_settings.scatter)
		MEM_freeN(sce->scenefx_settings.scatter);
	if (sce->scenefx_settings.ssr)
		MEM_freeN(sce->scenefx_settings.ssr);
	if (sce->scenefx_settings.ssao)
		MEM_freeN(sce->scenefx_settings.ssao);
...
void BKE_scene_init(Scene *sce)
{
	...
	sce->cutscene_settings = BKE_cutscene_settings_new();
	...
	sce->gm.physubstep = 1;
	sce->gm.maxphystep = 5;
	sce->gm.shadowCulling = 1;
	sce->gm.useDynamicResolution = 0;
	sce->gm.dynamicResolutionTargetFPS = 60;
	sce->gm.dynamicResolutionMinScale = 50;
	sce->gm.dynamicResolutionMaxScale = 100;
	sce->gm.dynamicResolutionStep = 5;
