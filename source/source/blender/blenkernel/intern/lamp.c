...
void BKE_lamp_init(Lamp *la)
{
	...
	la->att2 = 1.0f;
	la->mode = LA_SHAD_BUF;
	/* Game-engine Sun shadow preset. The CSM fields below complete its three cascades. */
	la->bufsize = 1024;
	la->clipsta = 0.5f;
	la->clipend = 40.0f;
	la->samp = 1;
	...
	la->shadowmap_type = LA_SHADMAP_SIMPLE;
	la->shadow_filter = LA_SHADOW_FILTER_PCF;
	...
	/* BGE/Range only: distance-based light culling ("light LOD"), disabled by default. */
	la->cull_distance = 100.0f;
	la->glow_scale = 1.0f;

	/* Keep CSM and its debug view opt-in; retain the numeric cascade preset. */
	la->shadow_cascade_count = 0;
	la->shadow_cascade_proportion_near = 0.4f;
	la->shadow_cascade_proportion_middle = 0.35f;
	la->csm_bufsize[0] = 512;
	la->csm_bufsize[1] = 128;
	la->csm_soft[0] = 5.0f;
	la->csm_soft[1] = 8.0f;
	la->csm_bias[0] = la->csm_bias[1] = 1.0f;
