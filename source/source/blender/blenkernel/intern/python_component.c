...
"This is the fake RANGE class KX_PythonComponent from fake RANGE module range.types"
);
...
"This is the fake RANGE API module used only to import the KX_PythonComponent class from range.types.KX_PythonComponent"
);
...
static struct PyModuleDef range_types_module_def = {
	PyModuleDef_HEAD_INIT, /* m_base */
	"types",  /* m_name */
	module_documentation,  /* m_doc */
	0,  /* m_size */
...
/* Component modules commonly import helper modules from the project's "scripts"
 * package (e.g. "from scripts import menu_common"). Those helper modules get
 * cached in sys.modules under their own key ("scripts.menu_common"), separate
 * from pc->module. If a helper module is imported while the fake Range.* stub
 * submodules are installed (see load_component below), it permanently binds
 * globals like "events" to the stub object. Since only pc->module itself was
 * ever removed from sys.modules, a helper module reloaded once during editor
...
static void purge_scripts_package(PyObject *sys_modules)
{
	...
	keys_to_delete = PyList_New(0);

	while (PyDict_Next(sys_modules, &pos, &key, NULL)) {
	...
	for (Py_ssize_t i = 0, size = PyList_Size(keys_to_delete); i < size; ++i) {
		PyDict_DelItem(sys_modules, PyList_GetItem(keys_to_delete, i));
	}

	Py_DECREF(keys_to_delete);
}
...
static bool load_component(PythonComponent *pc, ReportList *reports, Main *maggie)
{
	...
	#define FINISH(value) \
		...
		Py_XDECREF(mod); \
		Py_XDECREF(item); \
		purge_scripts_package(sys_modules); \
		PyDict_DelItemString(sys_modules, "Range"); \
		PyDict_DelItemString(sys_modules, "Range.types"); \
		PyDict_DelItemString(sys_modules, "Range.logic"); \
		PyDict_DelItemString(sys_modules, "Range.events"); \
		PyDict_DelItemString(sys_modules, "Range.render"); \
		PyDict_DelItemString(sys_modules, "Range.constraints"); \
		PyDict_DelItemString(sys_modules, "Range.application"); \
		PyDict_DelItemString(sys_modules, "Range.imgui"); \
	...
	PyObject *mod, *item = NULL, *sys_path, *pypath, *sys_modules, *rangemod, *rangesubmod;
	...
	// Setup range fake module and submodule.
	sys_modules = PyImport_GetModuleDict();
	rangemod = PyModule_Create(&range_module_def);
	rangesubmod = PyModule_Create(&range_types_module_def);

	PyModule_AddObject(rangemod, "types", rangesubmod);
	PyType_Ready(&PythonComponentType);
	PyModule_AddObject(rangesubmod, "KX_PythonComponent", (PyObject *)&PythonComponentType);

	PyDict_SetItemString(sys_modules, "Range", rangemod);
	PyDict_SetItemString(sys_modules, "Range.types", rangesubmod);
	// Add the other existing submodules, they will all inherit KX_PythonComponent but I don't see the need to create a new PyObject for the fake submod.
	PyDict_SetItemString(sys_modules, "Range.logic", rangesubmod);
	PyDict_SetItemString(sys_modules, "Range.events", rangesubmod);
	PyDict_SetItemString(sys_modules, "Range.render", rangesubmod);
	PyDict_SetItemString(sys_modules, "Range.constraints", rangesubmod);
	PyDict_SetItemString(sys_modules, "Range.application", rangesubmod);
	PyDict_SetItemString(sys_modules, "Range.imgui", rangesubmod);
	PyDict_SetItemString(PyModule_GetDict(rangemod), "__component__", Py_True);
