...
void BKE_object_free(Object *ob)
{
	...
	if (ob->gpu_particles.size_curve) {
		curvemapping_free(ob->gpu_particles.size_curve);
	}
	if (ob->gpu_particles.color_curve) {
		curvemapping_free(ob->gpu_particles.color_curve);
	}
	...
	BLI_freelistN(&ob->vehicle_wheels);
	BLI_freelistN(&ob->vehicle_gears);

	/* Animation Events */
	/* First we need to release triggers inside AnimationEvents,
	/* then it's safe to release animevents. */
	AnimationEvent *animEvent;
	for (animEvent = (AnimationEvent*)ob->animevents.first; animEvent; animEvent = animEvent->next) {
		BLI_freelistN(&animEvent->triggers);
	}
	BLI_freelistN(&ob->animevents);
...
void BKE_object_init(Object *ob)
{
	...
	ob->gameflag = OB_PROP | OB_COLLISION | OB_TASK_CONVERT;
	ob->gameflag2 = 0;
	ob->margin = 0.04f;
	ob->friction = 0.5;
	ob->init_state = 1;
...
void BKE_object_vehicle_wheel_add(Object *ob)
{
	bWheelSettings *wheel = MEM_callocN(sizeof(bWheelSettings), "Vehicle Wheel");

	/* Keep the stored values in sync with the defaults advertised by the
	 * Vehicle panel.  RNA defaults only describe the UI; calloc() still
	 * produces zeroes for a new list entry. */
	wheel->radius = 0.3f;
	wheel->suspension_rest_length = 0.3f;
	wheel->suspension_stiffness = 5.88f;
	wheel->suspension_damping = 0.88f;
	wheel->suspension_compression = 0.83f;
	wheel->friction = 10.5f;
	wheel->roll_influence = 0.1f;
	wheel->max_suspension_travel_cm = 500.0f;
	wheel->max_suspension_force = 6000.0f;
	wheel->flags = WHEEL_SHOW_EXPANDED;
	BLI_addtail(&ob->vehicle_wheels, wheel);
}

bool BKE_object_vehicle_wheel_remove(Object *ob, int index)
{
	bWheelSettings *rem;

	if (index < 0 || index > BLI_listbase_count(&ob->vehicle_wheels) - 1)
		return false;

	rem = BLI_findlink(&ob->vehicle_wheels, index);

	BLI_remlink(&ob->vehicle_wheels, rem);
	MEM_freeN(rem);

	return true;
}

void BKE_object_vehicle_gear_add(Object *ob)
{
	bGearRatio *gear = MEM_callocN(sizeof(bGearRatio), "Vehicle Gear");
	gear->ratio = 1.0f;
	BLI_addtail(&ob->vehicle_gears, gear);
}

bool BKE_object_vehicle_gear_remove(Object *ob, int index)
{
	bGearRatio *rem;

	if (index < 0 || index > BLI_listbase_count(&ob->vehicle_gears) - 1)
		return false;

	rem = BLI_findlink(&ob->vehicle_gears, index);

	BLI_remlink(&ob->vehicle_gears, rem);
	MEM_freeN(rem);
...
/* Animation Event */
void BKE_object_animation_event_add(Object *ob)
{
	AnimationEvent *animEvent = MEM_callocN(sizeof(AnimationEvent), "Animation Event");
	AnimationEvent *last = ob->animevents.last;

	/* By default, put the selected action */
...
bool BKE_object_animation_event_remove(Object *ob, int index)
{
	AnimationEvent *rem;
...
static void copy_object_vehicle_wheels(Object *obn, const Object *ob, const int UNUSED(flag))
{
	BLI_duplicatelist(&obn->vehicle_wheels, &ob->vehicle_wheels);
}

static void copy_object_vehicle_gears(Object *obn, const Object *ob, const int UNUSED(flag))
{
	BLI_duplicatelist(&obn->vehicle_gears, &ob->vehicle_gears);
}

static void copy_object_anim_events(Object *obn, const Object *ob, const int UNUSED(flag))
{
	BLI_duplicatelist(&obn->animevents, &ob->animevents);

	AnimationEvent *ob_event;
    AnimationEvent *obn_event;
    for (ob_event = ob->animevents.first, obn_event = obn->animevents.first; ob_event; ob_event = ob_event->next, obn_event = obn_event->next) {
        BLI_duplicatelist(&obn_event->triggers, &ob_event->triggers);
    }
}
...
void BKE_object_copy_data(Main *UNUSED(bmain), Object *ob_dst, const Object *ob_src, const int flag)
{
	...
	ob_dst->gpu_particles.size_curve = ob_src->gpu_particles.size_curve ?
		curvemapping_copy(ob_src->gpu_particles.size_curve) : NULL;
	ob_dst->gpu_particles.color_curve = ob_src->gpu_particles.color_curve ?
		curvemapping_copy(ob_src->gpu_particles.color_curve) : NULL;
	...
	copy_object_vehicle_wheels(ob_dst, ob_src, flag_subdata);
	copy_object_vehicle_gears(ob_dst, ob_src, flag_subdata);

	copy_object_anim_events(ob_dst, ob_src, flag_subdata);
                                      ...
                                      RigidBodyWorld *rbw, float r_originmat[3][3])
{
	...
	if (ob->parent) {
		 ...
		 */
		/* XXX Range: Terrible.. Removed XXX  */
