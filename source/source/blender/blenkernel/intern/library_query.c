	...
	for (; id != NULL; id = (flag & IDWALK_RECURSE) ? BLI_LINKSTACK_POP(data.ids_todo) : NULL) {
		...
		switch ((ID_Type)GS(id->name)) {
			...
			case ID_SCE:
			{
				...
				if (scene->cutscene_settings) {
					for (CutsceneSequence *sequence = scene->cutscene_settings->sequences.first; sequence;
					     sequence = sequence->next)
					{
						for (CutsceneEvent *event = sequence->events.first; event; event = event->next) {
							CALLBACK_INVOKE(event->template_object, IDWALK_CB_USER);
							CALLBACK_INVOKE(event->spawn_point, IDWALK_CB_USER);
							CALLBACK_INVOKE(event->dependent_object, IDWALK_CB_USER);
						}
					}
				}
