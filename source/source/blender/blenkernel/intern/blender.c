...
/* Game Protected File */
bool canOpenProtectedFile = false;
bool usePtrFile = false;
...
void BKE_range_version_string(char *version_str, size_t maxncpy, short version, short subversion, short minsubversion, bool v_prefix, bool include_subversion)
{
	const char *prefix = v_prefix ? "v" : "";

	if (include_subversion) {
		// Preview Version.
		if (minsubversion < 100) {
			BLI_snprintf(version_str, maxncpy, "%s%d.%dPre%d", prefix, version, subversion, minsubversion);
			return;
		}
		...
		if (minsubversion > 100) {
			// A and B Releases.
			BLI_snprintf(version_str, maxncpy,
						(minsubversion == 101) ? "%s%d.%da" : "%s%d.%db",
						prefix, version, subversion);
		}
		else {
			/* 100 is treated as the final version. */
...
void BKE_blender_globals_init(void)
{
	...
	BKE_range_version_string(range_versionstr, sizeof(versionstr), RANGE_VERSION, RANGE_SUBVERSION, RANGE_MINSUBVERSION, true, true);
