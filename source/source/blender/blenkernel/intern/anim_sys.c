...
KS_Path *BKE_keyingset_add_path(KeyingSet *ks, ID *id, const char group_name[], const char rna_path[], int array_index, short flag, short groupmode)
{
	...
	/* validate the array index against the actual RNA property, when it can
	 * be resolved already; fall back to "all components" (-1) rather than
	 * silently keeping an out-of-range index */
	if (array_index > 0) {
		PointerRNA id_ptr, r_ptr;
		PropertyRNA *r_prop;

		RNA_id_pointer_create(id, &id_ptr);

		if (RNA_path_resolve_property(&id_ptr, rna_path, &r_ptr, &r_prop)) {
			int array_len = RNA_property_array_length(&r_ptr, r_prop);

			if (array_index >= array_len) {
				if (G.debug & G_DEBUG) {
					printf("WARNING: Keying Set path array index out of range. '%s[%d]', array length is %d\n",
					       rna_path, array_index, array_len);
				}
				ksp->array_index = -1;
			}
		}
