	...
	if (RNA_struct_is_a(srna, &RNA_Space)) {
		                 ...
		                 &cb_regiontype, "bpy_struct.callback_add()") == -1)
		{
			...
			if (spaceid == SPACE_EMPTY) {
				...
				Py_INCREF(args);
#else
				UNUSED_VARS(spaceid, cb_event);
				PyErr_SetString(PyExc_NotImplementedError, "draw_handler_add(): not available in the player runtime");
				return NULL;
#endif
			}
		}
...
#else
		UNUSED_VARS(customdata, cb_regiontype, cb_regiontype_str);
		PyErr_SetString(PyExc_NotImplementedError, "draw_handler_remove(): not available in the player runtime");
		return NULL;
#endif
	}
	else {
