...
#undef SIZE
	...
	/* CaptureStackBackTrace + SymFromAddr work fine in Release builds too
	 * (they don't need frame pointers or debug-only CRT state) — this used
	 * to be gated to !NDEBUG, which meant Release crash logs only ever said
	 * "not supported", even though the exception handler had already
	 * captured its own inline backtrace. Enabling it here gives every
	 * caller of BLI_system_backtrace() (not just the SEH handler) a real
	 * stack. */
