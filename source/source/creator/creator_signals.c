...
LONG WINAPI windows_exception_handler(EXCEPTION_POINTERS *ExceptionInfo)
{
	...
	if (EXCEPTION_STACK_OVERFLOW != ExceptionInfo->ExceptionRecord->ExceptionCode) {
		...
		/* Always write the crash backtrace (blender.crash.txt / <file>.crash.txt in the
		 * temp dir), even in Release/NDEBUG builds. It used to be skipped here (straight
		 * to TerminateProcess), which meant Release-build access violations left no trace
		 * beyond the bare address/module above — invisible once the console window closes. */
