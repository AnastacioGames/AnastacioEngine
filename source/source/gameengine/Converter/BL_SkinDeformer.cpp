...
void BL_SkinDeformer::EnsureDfnrToPCCache()
{
	if (!m_dfnrToPC.empty()) {
		return;
	}
...
void BL_SkinDeformer::BGEDeformVerts(bool recalcNormal, bool fullNormals)
{
	...
	else if (recalcNormal) {
		RecalcNormals();
	}
}

void BL_SkinDeformer::UpdateBoneMatrixPalette()
{
	EnsureDfnrToPCCache();
	const unsigned short defbase_tot = m_dfnrToPC.size();

	/* Same armature-space pre/post transform as BGEDeformVerts: folding it into each palette
	 * matrix here means the shader can do a plain linear blend skin (see USE_SKINNING in
	 * gpu_shader_vertex.glsl) and still match the CPU path exactly. */
	const Eigen::Matrix4f post_mat = Eigen::Matrix4f::Map((float*)m_obmat).inverse() * Eigen::Matrix4f::Map((float*)m_armobj->GetArmatureObject()->obmat);
	const Eigen::Matrix4f pre_mat = post_mat.inverse();

	m_boneMatrices.resize(defbase_tot * 16);

	for (unsigned short i = 0; i < defbase_tot; ++i) {
		bPoseChannel *pchan = m_dfnrToPC[i];
		Eigen::Matrix4f mat;
		if (pchan) {
			mat.noalias() = post_mat * Eigen::Matrix4f::Map((float*)pchan->chan_mat) * pre_mat;
		}
		else {
			mat = Eigen::Matrix4f::Identity();
		}
		memcpy(&m_boneMatrices[i * 16], mat.data(), sizeof(float) * 16);
	}
}

bool BL_SkinDeformer::GetBoneMatrices(const float **matrices, int *count) const
{
	if (m_boneMatrices.empty()) {
		return false;
	}

	*matrices = m_boneMatrices.data();
	*count = m_boneMatrices.size() / 16;
	return true;
}

bool BL_SkinDeformer::IsGPUSkinned() const
{
	return m_armobj && m_armobj->GetVertDeformType() == ARM_VDEF_BGE_GPU;
}

static bool mesh_has_gpu_skinning_material(Mesh *bmesh)
{
	for (int i = 0; i < bmesh->totcol; ++i) {
		Material *ma = bmesh->mat[i];
		if (ma && (ma->shade_flag & MA_SKINNING)) {
			return true;
		}
	}
	return false;
	...
	if (PoseUpdated()) {
		...
		switch (m_armobj->GetVertDeformType()) {
			...
			case ARM_VDEF_BGE_CPU: {
				BGEDeformVerts(recalcNormal, false);
				break;
			}
			case ARM_VDEF_BGE_NORMALS_CPU: {
				BGEDeformVerts(recalcNormal, true);
				break;
			}
			case ARM_VDEF_BGE_GPU: {
				EnsureDfnrToPCCache();
				if (m_dfnrToPC.size() > GPU_MAX_SKINNING_BONES) {
					CM_Warning("armature \"" << m_armobj->GetArmatureObject()->id.name + 2 << "\" has " << m_dfnrToPC.size() <<
							" deform groups, exceeding the GPU skinning budget of " << GPU_MAX_SKINNING_BONES <<
							"; falling back to CPU skinning");
					BGEDeformVerts(recalcNormal, false);
				}
				else if (!mesh_has_gpu_skinning_material(m_bmesh)) {
					CM_Warning("mesh \"" << m_bmesh->id.name + 2 << "\" is deformed by armature \"" <<
							m_armobj->GetArmatureObject()->id.name + 2 << "\" using RanGE GPU Skinning, but none of its "
							"materials have \"GPU Skinning\" enabled (Material Properties > Options); "
							"falling back to CPU skinning");
					BGEDeformVerts(recalcNormal, false);
				}
				else {
					UpdateBoneMatrixPalette();
					gpuSkinned = true;
				}
				break;
			}
		...
		if (gpuSkinned) {
			/* Vertex positions in the VBO stay in bind pose; only the bone-matrix uniform
			 * changes per frame, so this mesh can be treated as static again. */
			m_bDynamic = false;
		}
		else {
