...
protected:
	...
	void BGEDeformVerts(bool recalcNormal, bool fullNormals);
	/// Build m_dfnrToPC (deform-group index -> pose channel) once, shared by BGEDeformVerts and UpdateBoneMatrixPalette.
	void EnsureDfnrToPCCache();
	/// Fill m_boneMatrices from the current pose, for the GPU skinning vertex deform mode.
	void UpdateBoneMatrixPalette();
