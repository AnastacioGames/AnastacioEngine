...
/* shared bProperty -> EXP_Value conversion for the non-timer property types */
static EXP_Value *BL_CreatePropertyValue(bProperty *prop)
{
	switch (prop->type) {
		case GPROP_BOOL:
			return new EXP_BoolValue((bool)(prop->data != 0));
		case GPROP_INT:
			return new EXP_IntValue((int)prop->data);
		case GPROP_FLOAT:
		{
			float floatprop = *((float *)&prop->data);
			return new EXP_FloatValue(floatprop);
		}
		case GPROP_STRING:
			return new EXP_StringValue((char *)prop->poin, "");
		default:
			return nullptr;
	}
}
...
void BL_ConvertProperties(Object *object, KX_GameObject *gameobj, SCA_TimeEventManager *timemgr, SCA_IScene *scene, bool isInActiveLayer)
{
	...
	while (prop) {
		...
		switch (prop->type) {
			...
			case GPROP_STRING:
			{
				propval = BL_CreatePropertyValue(prop);
...
void BL_ConvertWorldProperties(World *blenderworld, KX_WorldInfo *worldinfo)
{
	for (bProperty *prop = (bProperty *)blenderworld->prop.first; prop; prop = prop->next) {
		EXP_Value *propval = BL_CreatePropertyValue(prop);
		if (propval) {
			worldinfo->SetProperty(prop->name, propval);
			propval->Release();
		}
	}
	...
	for (const auto &status : wo_status_bools) {
		if (worldinfo->GetProperty(status.name)) {
			EXP_Value *propval = new EXP_BoolValue(status.bval);
	...
	for (const auto &status : wo_status_floats) {
		if (worldinfo->GetProperty(status.name)) {
			EXP_Value *propval = new EXP_FloatValue(status.fval);
			worldinfo->SetProperty(status.name, propval);
			propval->Release();
		}
	}
