						   ...
						   BL_SceneConverter& converter)
{
	...
	while (sens) {
		if (!(sens->flag & SENS_DEACTIVATE)) {
			...
			skipped_ticks = sens->use_dt ? sens->freq_dt : sens->freq;
			invert    = !(sens->invert == 0);
			use_dt	  = !(sens->use_dt == 0);
			level     = !(sens->level == 0);
			tap       = !(sens->tap == 0);
			...
			switch (sens->type) {
				...
				case  SENS_DELAY:
				{
					...
					if (eventmgr) {
						...
						gamesensor = new SCA_DelaySensor(eventmgr,
						                                 gameobj,
														(delaysensor->use_delta & SENS_DELAY_USEDELTA) != 0 ? delaysensor->delay_time : delaysensor->delay,
						                                 delaysensor->duration,
														 delaysensor->repeat_times,
														(delaysensor->flag & SENS_DELAY_REPEAT) != 0, 
														(delaysensor->use_delta & SENS_DELAY_USEDELTA) != 0);
					}
					break;
				}
				...
				case SENS_PROPERTY:
				{
					...
					if (eventmgr) {
						...
						SCA_IObject *runtimeTarget = nullptr;
						std::string runtimeProperty;
						if (blenderpropsensor->runtime_enabled) {
							if (blenderpropsensor->runtime_target) {
								runtimeTarget = converter.FindGameObject(blenderpropsensor->runtime_target);
							}
							static const char *runtimeProperties[] = {
								"transform.local_position", "render.visible", "physics.mass", "physics.linear_velocity", "physics.angular_velocity", "physics.gravity", "transform.local_scale"};
							if (blenderpropsensor->runtime_property >= SENS_RUNTIME_PROP_LOCAL_POSITION &&
							    blenderpropsensor->runtime_property <= SENS_RUNTIME_PROP_LOCAL_SCALE) {
								runtimeProperty = runtimeProperties[blenderpropsensor->runtime_property];
								char value[64];
								char maxvalue[64];
								if (blenderpropsensor->runtime_property == SENS_RUNTIME_PROP_VISIBLE) {
									std::snprintf(value, sizeof(value), "%s", blenderpropsensor->runtime_bool_value ? "true" : "false");
								}
								else if (blenderpropsensor->runtime_property == SENS_RUNTIME_PROP_MASS) {
									std::snprintf(value, sizeof(value), "%g", blenderpropsensor->runtime_value[0]);
									std::snprintf(maxvalue, sizeof(maxvalue), "%g", blenderpropsensor->runtime_value_max[0]);
								}
								else {
									std::snprintf(value, sizeof(value), "%g,%g,%g", blenderpropsensor->runtime_value[0], blenderpropsensor->runtime_value[1], blenderpropsensor->runtime_value[2]);
								}
								propval = value;
								if (blenderpropsensor->runtime_property == SENS_RUNTIME_PROP_MASS) {
									propmaxval = maxvalue;
								}
							}
						}
						...
						gamesensor = new SCA_PropertySensor(eventmgr, gameobj, propname, propval, propmaxval,
						                                         propchecktype, runtimeTarget, runtimeProperty);
					}
				...
				{
					bAnimationEventSensor *blenderaessensor = (bAnimationEventSensor *)sens->data;
					// we will reuse the property event manager, there is nothing special with this sensor
					SCA_EventManager *eventmgr
					    = logicmgr->FindEventManager(SCA_EventManager::BASIC_EVENTMGR);
					if (eventmgr) {
						int eventIndex = blenderaessensor->index - 1;
			...
			if (gamesensor) {
				...
				gamesensor->SetDeltaTime(use_dt);
