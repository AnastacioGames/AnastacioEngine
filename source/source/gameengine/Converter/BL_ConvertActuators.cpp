	...
	while (bact)
	{
		...
		switch (bact->type) {
			case ACT_OBJECT:
			{
				...
				bitLocalFlag.SetPositionMode = bool(obact->type == ACT_OBJECT_NORMAL_SET);
			...
			case ACT_EDIT_OBJECT:
			{
				...
				switch (editobact->type) {
					case ACT_EDOB_ADD_OBJECT:
					{
						...
						// Does the original for replication exist and is it in a
						// non-active layer? A linked object registered through the
						// Outliner's External Files entry has not been converted yet,
						// so convert it into this scene's inactive list on demand.
						KX_GameObject *originalval = nullptr;
						if (editobact->ob) {
							originalval = converter.FindGameObject(editobact->ob);
							if (originalval && (editobact->ob->lay & activeLayerBitInfo)) {
								CM_Warning("object \"" << objectname << "\" from AddObject actuator \"" << uniquename
								                       << "\" is not in a hidden layer.");
							}
							else if (!originalval && editobact->ob->id.lib) {
								originalval = KX_GetActiveEngine()->GetConverter()->FindOrConvertMainObject(
								        editobact->ob->id.name + 2, scene);
							}
						}
					...
					{
						KX_ChangeColorActuator *tmpcolact = new KX_ChangeColorActuator(
							gameobj, 
							(mathfu::vec4)editobact->change_col,
			...
			case ACT_GAME:
			{
				...
				std::string savelocation = gameact->saveloc;
				std::string extensionname = gameact->extension_name;
			...
			case ACT_2DFILTER:
			{
				...
				switch (_2dfilter->type) {
					...
					case ACT_2DFILTER_SSAO: filtermode = RAS_2DFilterManager::FILTER_SSAO; break;
					case ACT_2DFILTER_BLOOM: filtermode = RAS_2DFilterManager::FILTER_BLOOM; break;
					case ACT_2DFILTER_TONEMAP: filtermode = RAS_2DFilterManager::FILTER_TONEMAP; break;
					case ACT_2DFILTER_LIGHTSCATTER: filtermode = RAS_2DFilterManager::FILTER_LIGHTSCATTER; break;
					case ACT_2DFILTER_SSR: filtermode = RAS_2DFilterManager::FILTER_SSR; break;
					case ACT_2DFILTER_FXAA: filtermode = RAS_2DFilterManager::FILTER_FXAA; break;
