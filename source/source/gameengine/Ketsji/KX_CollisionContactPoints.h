...
private:
	...
	bool m_firstObject;
	/// Whether this instance owns m_collData and must delete it on destruction.
	bool m_ownsCollData;

public:
	/**
	 * \param ownsCollData Pass true only when collData was allocated specifically for this
	 * instance and no other code will free it (e.g. PHY_IPhysicsEnvironment::CheckCollision
	 * results). Collision-callback usage shares collData across multiple short-lived instances
	 * and frees it separately, so it must stay false there.
	 */
	KX_CollisionContactPointList(const PHY_ICollData *collData, bool firstObject, bool ownsCollData = false);
