...
PyObject *KX_MeshBuilderSlot::PyAddVertex(PyObject *args, PyObject *kwds)
{
	...
	if (pyuvs) {
		...
		for (unsigned short i = 0; i < size; ++i) {
			PyObject *item = PySequence_GetItem(pyuvs, i);
			if (!PyVecTo(item, uvs[i])) {
				Py_XDECREF(item);
				return nullptr;
			}
			Py_XDECREF(item);
	...
	if (pycolors) {
		...
		for (unsigned short i = 0; i < size; ++i) {
			PyObject *item = PySequence_GetItem(pycolors, i);
			mt::vec4 color;
	...
	const unsigned index = m_array->AddVertex(pos, normal, tangent, uvs, colors, m_origIndexCounter++, 0,
			mt::vec4_packed(mt::zero4), mt::vec4_packed(mt::zero4));
...
PyObject *KX_MeshBuilderSlot::PyAddIndex(PyObject *value)
{
	...
	for (unsigned int i = 0, size = PySequence_Size(value); i < size; ++i) {
		...
		const int val = PyLong_AsLong(item);
		Py_DECREF(item);

		if (val < 0 && PyErr_Occurred()) {
			PyErr_Format(PyExc_TypeError, "expected a list of positive integers");
			return nullptr;
		}
...
static PyObject *py_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	...
	layersInfo.activeColor = 0;
	layersInfo.activeUv = 0;
...
PyObject *KX_MeshBuilder::PyAddSlot(PyObject *args, PyObject *kwds)
{
	...
	if (!ELEM(primitive, RAS_DisplayArray::POINTS, RAS_DisplayArray::LINES, RAS_DisplayArray::TRIANGLES)) {
