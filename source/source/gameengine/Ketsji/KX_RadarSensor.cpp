...
void KX_RadarSensor::DrawDebug()
{
	const mt::vec3 axis = (m_cone_target - m_cone_origin).SafeNormalized(mt::axisY3);
	const mt::vec3 helper = (std::fabs(axis.z) < 0.9f) ? mt::axisZ3 : mt::axisX3;
	const mt::vec3 tangent = mt::cross(axis, helper).SafeNormalized(mt::axisX3);
	const mt::vec3 bitangent = mt::cross(axis, tangent).SafeNormalized(mt::axisY3);
	const mt::vec4 color(0.15f, 0.6f, 1.0f, 1.0f);
	const int segments = 24;
	mt::vec3 previous = m_cone_target + tangent * m_coneradius;
	for (int index = 1; index <= segments; ++index) {
		const float angle = (float)index * 2.0f * (float)M_PI / (float)segments;
		const mt::vec3 point = m_cone_target + (tangent * std::cos(angle) + bitangent * std::sin(angle)) * m_coneradius;
		KX_RasterizerDrawDebugLine(previous, point, color);
		if (index % (segments / 4) == 0) {
			KX_RasterizerDrawDebugLine(m_cone_origin, point, color);
		}
		previous = point;
	}
}
