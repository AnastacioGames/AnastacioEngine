...
float KX_BoundingBox::GetVolume() const
{
	// Update AABB to make sure we have the last one.
	m_owner->UpdateBounds(false);
	const SG_BBox &box = m_owner->GetCullingNode().GetAabb();
	const mathfu::vec3 min = box.GetMin() * m_owner->NodeGetWorldScaling();
	const mathfu::vec3 max = box.GetMax() * m_owner->NodeGetWorldScaling();
...
PyAttributeDef KX_BoundingBox::Attributes[] = {
	...
	EXP_PYATTRIBUTE_RO_FUNCTION("volume", KX_BoundingBox, pyattr_get_volume),
...
PyObject *KX_BoundingBox::pyattr_get_volume(EXP_PyObjectPlus *self_v, const EXP_PYATTRIBUTE_DEF *attrdef)
{
	KX_BoundingBox *self = static_cast<KX_BoundingBox *>(self_v);
	if (!self->IsValidOwner()) {
		return nullptr;
	...
	return PyFloat_FromDouble(self->GetVolume());
}
