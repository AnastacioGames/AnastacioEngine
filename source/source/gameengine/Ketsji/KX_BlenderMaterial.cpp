	...
	m_userDefBlend(false)
{
	...
	m_rasMode |= (mat->material_type == MA_TYPE_WIRE) ? RAS_WIRE : (mat->material_type == MA_TYPE_HALO) ? RAS_POINTS : 0;
...
void KX_BlenderMaterial::UpdateObjectMatrix(RAS_MeshUser *meshUser, RAS_Rasterizer *rasty, const float mat[16])
{
	if (m_blenderShader && m_blenderShader->Ok()) {
		m_blenderShader->UpdateObjectMatrix(meshUser, m_passIndex, rasty, mat);
	}
}
