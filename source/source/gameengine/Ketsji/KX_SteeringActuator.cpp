...
bool KX_SteeringActuator::Update(double curtime)
{
	...
	if (apply_steerforce) {
		...
		if (isdyna || m_mode == KX_STEERING_PATHFOLLOWING) {
			m_steerVec.z = 0.0f;
		}
...
static bool getNavmeshNormal(dtNavMesh *navmesh, dtNavMeshQuery *navquery, const mt::vec3& pos, mt::vec3& normal)
{
	...
	navquery->findNearestPoly(spos.Data(), polyPickExt, &filter, &sPolyRef, nullptr);
	if (sPolyRef == 0) {
		return false;
	}
	...
	const dtPoly *p;
	if (!dtStatusSucceed(navmesh->getTileAndPolyByRef(sPolyRef, &tile, &p))) {
		return false;
	}
	const int polyIdx = (int)(p - tile->polys);
	...
	for (int i = 0; i < pd->triCount; ++i) {
		...
		for (int j = 0; j < 3; ++j) {
			if (t[j] < p->vertCount) {
				v[j] = &tile->verts[p->verts[t[j]] * 3];
			}
			else {
				v[j] = &tile->detailVerts[(pd->vertBase + (t[j] - p->vertCount)) * 3];
			}
		}
	...
	if (idxMin >= 0) {
		const unsigned char *t = &tile->detailTris[(pd->triBase + idxMin) * 4];
		const float *v[3];
		for (int j = 0; j < 3; ++j) {
			if (t[j] < p->vertCount) {
				v[j] = &tile->verts[p->verts[t[j]] * 3];
...
void KX_SteeringActuator::HandleActorFace(const mt::vec3& velocity)
{
	...
	if (m_navmesh && m_normalUp) {
		dtNavMesh *navmesh = m_navmesh->GetNavMesh();
		dtNavMeshQuery *navquery = m_navmesh->GetNavMeshQuery();
		mt::vec3 normal;
		mt::vec3 trpos = m_navmesh->TransformToLocalCoords(curobj->NodeGetWorldPosition());
		if (navmesh && navquery && getNavmeshNormal(navmesh, navquery, trpos, normal)) {
