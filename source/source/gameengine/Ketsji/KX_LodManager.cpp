	...
	m_distanceFactor(ob->lodfactor)
{
	...
	for (LodLevel *lod = (LodLevel *)ob->lodlevels.first; lod; lod = lod->next) {
		if (!(lod->flags & OB_LOD_USE_INVISIBLE) && (!lod->source || lod->source->type != OB_MESH)) {
			continue;
		}
		unsigned short flag = 0;
		bool use_invisible = (lod->flags & OB_LOD_USE_INVISIBLE);
		...
		if (!use_invisible) {
			...
			if (lod->flags & OB_LOD_USE_BILLBOARD) {
				flag |= KX_LodLevel::USE_BILLBOARD;
			}

			if (lod->flags & OB_LOD_USE_ATLAS) {
				flag |= KX_LodLevel::USE_ATLAS;
			}
		}
		else {
			flag |= KX_LodLevel::USE_INVISIBLE;
		}

		m_levels.emplace_back(lod->distance, lod->obhysteresis, level++,
		                      BL_ConvertMesh(lodmesh, lodmatob, scene, converter), flag,
		                      (unsigned short)lod->atlas_cols, (unsigned short)lod->atlas_rows);
	}

	/* updatePhysics */
	m_updatePhysics = (ob->gameflag & OB_LOD_UPDATE_PHYSICS);
}
...
const bool KX_LodManager::GetUpdatePhysics() {
...
PyAttributeDef KX_LodManager::Attributes[] = {
	...
	EXP_PYATTRIBUTE_BOOL_RW("updatePhysics", KX_LodManager, m_updatePhysics),
