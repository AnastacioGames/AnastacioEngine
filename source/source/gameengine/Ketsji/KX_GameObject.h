...
public:
	...
	{
		bool m_render; // Do profiling render
		float m_time_components;
...
protected:
	...
	KX_AnimationEventManager			*m_animationEventManager;
	...
	/// Stores the object's time spent in some aspects (Logic, Rasterizer...), for debugging.
	DebugProfilingData m_debugProfilingData;

	std::unique_ptr<PHY_IPhysicsController> m_physicsController;
	std::unique_ptr<PHY_IGraphicController> m_graphicController;

	/// Constraint id of the vehicle auto-created from game.is_vehicle, or -1 if none.
	int m_vehicleConstraintId;
	std::string m_vehicleSteeringWheelName;

	/// Arcade-feel powertrain params from game.vehicle_max_torque/vehicle_max_rpm,
	/// 0 if unset (Python component falls back to its own default in that case).
	float m_vehicleMaxTorque;
	float m_vehicleMaxRPM;

	/// Gearbox ratios from game.vehicle_gears (empty if none defined) and
	/// gearbox_type (0=automatic, 1=manual), copied during scene conversion.
	std::vector<float> m_vehicleGearRatios;
	short m_vehicleGearboxType;
	...
	/// Fase I.2: per-object GPU particle emitter, created by SetupGPUParticles() at scene
	/// conversion time for objects with use_gpu_particles set (Object.gpu_particles), null
	...
	/// main loop). Null if unset -- falls back to the fixed height from DNA.
	KX_GameObject *m_collisionGroundObject;
...
public:
	...
	/**
	 * Sets the layerweight in an action
	 */
	void SetActionLayerWeight(short layer, float layer_weight);

	/**
	 * Sets the speed in an action
	 */
	void SetActionLayerSpeed(short layer, float speed);
	...
	/// Fase I.2: creates and configures this object's GPU particle emitter from its DNA
	/// settings. Called once from BL_GameObjectFromBlenderObject at scene conversion time,
	...
	GetMass();
	/**
	 * Return the distance to camera of the Activity Culling object
	 */
		float
	...
	/**
	 * Constraint id of the vehicle auto-created from this object's
	 * game.is_vehicle/vehicle_wheels DNA data during scene conversion,
	 * or -1 if this object isn't a vehicle chassis.
	 */
	void SetVehicleConstraintId(int constraintId);
	int GetVehicleConstraintId() const;

	/**
	 * Name of the live object resolved from game.vehicle_steering_wheel during
	 * scene conversion, or empty if this chassis has no steering wheel object.
	 * Looked up by name (not cached as a pointer) so a steering wheel object
	 * removed independently of the chassis is simply reported as missing,
	 * never dereferenced dangling.
	 */
	void SetVehicleSteeringWheelName(const std::string &name);
	const std::string &GetVehicleSteeringWheelName() const;

	/**
	 * Arcade-feel powertrain params copied from game.vehicle_max_torque/
	 * vehicle_max_rpm during scene conversion. 0 means unset.
	 */
	void SetVehicleMaxTorque(float torque);
	float GetVehicleMaxTorque() const;
	void SetVehicleMaxRPM(float rpm);
	float GetVehicleMaxRPM() const;

	/**
	 * Gear ratios copied from game.vehicle_gears during scene conversion
	 * (empty if none defined), and the gearbox_type selector (0=automatic,
	 * 1=manual, matching OB_GEARBOX_* in DNA_object_types.h).
	 */
	void SetVehicleGearRatios(const std::vector<float> &ratios);
	const std::vector<float> &GetVehicleGearRatios() const;
	void SetVehicleGearboxType(short type);
	short GetVehicleGearboxType() const;
	...
	std::vector<KX_GameObject *> GetParentRecursive() const;
	...
	/// Get the object's time profiling.
	DebugProfilingData GetDebugTimeProfiling()
	{
		return m_debugProfilingData;
	}
	void SetDebugTimeProfiling(int type, float time);
	...
	EXP_PYMETHOD(KX_GameObject,LookAt);
	...
	EXP_PYMETHOD_NOARGS(KX_GameObject,GetVehicle);
	EXP_PYMETHOD_NOARGS(KX_GameObject,GetVehicleSteeringWheel);
	EXP_PYMETHOD_NOARGS(KX_GameObject,GetVehicleMaxTorque);
	EXP_PYMETHOD_NOARGS(KX_GameObject,GetVehicleMaxRPM);
	EXP_PYMETHOD_NOARGS(KX_GameObject,GetVehicleGearRatios);
	EXP_PYMETHOD_NOARGS(KX_GameObject,GetVehicleGearboxType);
	...
	EXP_PYMETHOD_DOC(KX_GameObject, setActionLayerWeight);
	EXP_PYMETHOD_DOC(KX_GameObject, setActionLayerSpeed);
	...
	static PyObject*	pyattr_get_parent_recursive(EXP_PyObjectPlus *self_v, const EXP_PYATTRIBUTE_DEF *attrdef);
	...
	static PyObject*	pyattr_get_activitycullingdistance(EXP_PyObjectPlus *self_v, const EXP_PYATTRIBUTE_DEF *attrdef);
	...
	static PyObject*	pyattr_get_animationEventsManager(EXP_PyObjectPlus *self_v, const EXP_PYATTRIBUTE_DEF *attrdef);
	/// Fase I.2: KX_ParticleSystem proxy wrapping this object's GPU particle buffer, or None
	/// if use_gpu_particles is unset. Mirrors the removed KX_Scene::pyattr_get_particles.
	static PyObject*	pyattr_get_particles(EXP_PyObjectPlus *self_v, const EXP_PYATTRIBUTE_DEF *attrdef);
