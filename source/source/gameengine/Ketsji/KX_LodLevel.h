...
private:
	...
	KX_Mesh *m_mesh;
	unsigned short m_atlasCols;
	unsigned short m_atlasRows;

public:
	KX_LodLevel(float distance, float hysteresis, unsigned short level, KX_Mesh *mesh, unsigned short flag,
	            unsigned short atlasCols = 0, unsigned short atlasRows = 0);
	virtual ~KX_LodLevel();
	...
	unsigned short GetAtlasColumns() const;
	unsigned short GetAtlasRows() const;
	...
	enum {
		...
		USE_MATERIAL = (1 << 2),
		/// Once the object reaches this level level, it will no longer render.
		USE_INVISIBLE = (1 << 3),
		/// Rotate the mesh around Z to always face the camera (tree/foliage impostor).
		USE_BILLBOARD = (1 << 4),
		/// Billboard texture is a grid of views around Z; pick the cell matching the camera heading.
		USE_ATLAS = (1 << 5)
	};
