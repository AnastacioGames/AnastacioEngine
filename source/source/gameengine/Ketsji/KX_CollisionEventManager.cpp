...
bool KX_CollisionEventManager::RegisterSensor(SCA_ISensor *sensor)
{
	if (SCA_EventManager::RegisterSensor(sensor)) {
		...
		// Temp hack fix: check if this sensor is from LibLoad. if true we prevent it from being use delete in KX_NearSensor.
		collisionsensor->m_physCtrl_Libload = sensor->m_replaced_with_Libload;
		return true;
	}
...
void KX_CollisionEventManager::ClearAllEvent()
{
	RemoveNewCollisions(); // Clear NewCollisions, avoid invalid interactions.
}
