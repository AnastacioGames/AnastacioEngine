...
using PhysicsControllerId = unsigned long long;

PhysicsControllerId g_nextPhysicsControllerId = 1;
std::unordered_map<PHY_IPhysicsController *, PhysicsControllerId> g_physicsControllerIds;
std::unordered_map<PhysicsControllerId, PHY_IPhysicsController *> g_physicsControllers;
...
unsigned long long KX_GetPhysicsControllerId(PHY_IPhysicsController *controller)
{
	...
	const auto found = g_physicsControllerIds.find(controller);
	if (found != g_physicsControllerIds.end()) {
		return found->second;
	}

	const PhysicsControllerId id = g_nextPhysicsControllerId++;
	g_physicsControllerIds[controller] = id;
	g_physicsControllers[id] = controller;
	return id;
}

PHY_IPhysicsController *KX_GetPhysicsControllerById(unsigned long long id)
{
	const auto found = g_physicsControllers.find(id);
	return (found != g_physicsControllers.end()) ? found->second : nullptr;
}
...
void KX_UnregisterPhysicsControllerId(PHY_IPhysicsController *controller)
{
	const auto found = g_physicsControllerIds.find(controller);
	if (found != g_physicsControllerIds.end()) {
		g_physicsControllers.erase(found->second);
		g_physicsControllerIds.erase(found);
	}
}
                                     ...
                                     PyObject *kwds)
{
	...
	if (KX_GetPhysicsEnvironment()) {
		PHY_IPhysicsController *physctrl = KX_GetPhysicsControllerById(physicsid);
		PHY_IPhysicsController *physctrl2 = KX_GetPhysicsControllerById(physicsid2);
		if (!physctrl || (physicsid2 && !physctrl2)) {
			PyErr_SetString(PyExc_ValueError, "physics id is invalid or its object has been removed");
			return nullptr;
		}

		{
			if (constrainttype == PHY_VEHICLE_CONSTRAINT) {
				...
				if (!vehicle) {
					PyErr_SetString(PyExc_ValueError, "createConstraint(...): chassis object has no rigid body to attach a vehicle to");
					return nullptr;
				}
...
static PyObject *gPyCreateVehicle(PyObject *self, PyObject *args)
{
	...
	PHY_IVehicle *vehicle = KX_GetPhysicsEnvironment()->CreateVehicle(physctrl);
	if (!vehicle) {
		PyErr_SetString(PyExc_ValueError, "createVehicle(...): chassis object has no rigid body to attach a vehicle to");
		return nullptr;
	}
