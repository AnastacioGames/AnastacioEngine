...
KX_PythonComponent::~KX_PythonComponent()
{
	...
	// Clear all remaining attributes from the self.
	PyObject *pycomp = GetProxy();
	if (pycomp && PyObject_HasAttrString(pycomp, "__dict__")) {
		PyObject *dict = PyObject_GetAttrString(pycomp, "__dict__");
...
void KX_PythonComponent::Awake()
{
	...
	PyObject *ret = PyObject_CallMethod(GetProxy(), "awake", "O", arg_dict);
...
{
	if (m_failed) { // Initialization failure, prevent update() to avoid console spam.
		return;
	}
...
int KX_PythonComponent::GetActiveState() {
	return m_pc->flag_toggle_exec;
}
