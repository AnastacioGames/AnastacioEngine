...
void LA_Launcher::InitEngine()
{
	...
	const KX_KetsjiEngine::FlagType flags = (KX_KetsjiEngine::FlagType)
	                                        ((fixed_framerate ? KX_KetsjiEngine::FIXED_FRAMERATE : 0) |
											 ...
											 (debugMode ? KX_KetsjiEngine::SHOW_DEBUG_MODE : 0) |
											 (ignoreExitKey ? KX_KetsjiEngine::IGNORE_EXIT_KEY : 0));
	...
	// Create the Dear ImGui (Graphical User interface).
	m_imgui = new KX_Imgui();

	// Create the Debug Mode.
	m_debugMode = new KX_DebugMode();
	m_debugMode->SetShowConsole(showConsole);
	...
	m_ketsjiEngine->SetImgui(m_imgui);
	m_ketsjiEngine->SetDebugMode(m_debugMode);
	...
	m_ketsjiEngine->SetShadowCulling(gm.shadowCulling);
	m_ketsjiEngine->SetDynamicResolution(gm.useDynamicResolution != 0,
	                                     gm.dynamicResolutionTargetFPS,
	                                     gm.dynamicResolutionMinScale,
...
KX_ExitInfo LA_Launcher::EngineNextFrame()
{
	...
	if (m_inputDevice->GetInput(exitKey).Find(SCA_InputEvent::ACTIVE) &&
	    !m_inputDevice->GetHookExitKey() && !(m_ketsjiEngine->GetFlags() & KX_KetsjiEngine::IGNORE_EXIT_KEY)) {
