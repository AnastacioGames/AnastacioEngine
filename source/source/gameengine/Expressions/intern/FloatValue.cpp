	...
	switch (dtype) {
		case VALUE_INT_TYPE:
		{
			switch (op) {
				...
				{
					if (m_float == 0) {
						///giving a error is never good, maybe some other fix can be made
						ret = new EXP_FloatValue(0.0f);
						//new EXP_ErrorValue("Division by zero");
					}
					else {
						ret = new EXP_FloatValue(((EXP_IntValue *)val)->GetInt() / m_float);
					}
...
{
	if (fl > -0.001 && fl < 0.001) {
		//CM_Warning("set float fix");
		m_float = 0.0f;
	}
	else {
		m_float = fl;
...
{
	if (m_float > -0.001 && m_float < 0.001) {
		//CM_Warning("get float fix");
		return 0.0f;
	}
	else {
		return m_float;
...
{
	if (m_float > -0.001 && m_float < 0.001) {
		//CM_Warning("GetNumber float fix");
		return 0.0f;
	}
	else{
		return m_float;
...
{
	if (m_float > -0.001 && m_float < 0.001) {
		//CM_Warning("string get float fix");
