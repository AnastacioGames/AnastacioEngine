...
void KX_BlenderCanvas::SetMouseState(RAS_MouseState mousestate)
{
	...
	switch (mousestate) {
		...
		{
			// Custom Mouse Cursor.
			if (KX_GetActiveEngine() && KX_GetActiveEngine()->HasCustomMouseCursor()) {
				KX_GetActiveEngine()->GetCustomMouseCursor()->m_visible = false;
			}
		...
		case MOUSE_NORMAL:
		{
			// Custom Mouse Cursor Override.
			if (KX_GetActiveEngine() && KX_GetActiveEngine()->HasCustomMouseCursor()) {
				KX_GetActiveEngine()->GetCustomMouseCursor()->m_visible = true;
				WM_cursor_set(m_win, CURSOR_NONE);
				break;
			}
...
void KX_BlenderCanvas::SetMousePosition(int x, int y, bool debugModeCamera)
{
	int winX = m_area.GetLeft();
	int winY = m_area.GetBottom();
	int winMaxY = m_area.GetMaxY();
	
	if ((debugModeCamera ? true : !GetMouseVisibleRequested())) {
