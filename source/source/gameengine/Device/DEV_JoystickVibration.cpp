...
bool DEV_Joystick::RumblePlay(float strengthLeft, float strengthRight, unsigned int duration)
{
	...
	// Some Game Controllers only supports large/small magnitude motors using a custom effect
		...
		m_private->m_hapticCustomData[0] = (Uint16)(strengthLeft * 0x7FFF);
		m_private->m_hapticCustomData[1] = (Uint16)(strengthRight * 0x7FFF);
		...
		m_private->m_hapticeffect.custom.data = m_private->m_hapticCustomData;
