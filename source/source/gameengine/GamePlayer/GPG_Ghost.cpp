...
static GHOST_IWindow *startBorderlessWindow(GHOST_ISystem *system,
                                            std::string& title,
                                            const bool stereoVisual,
                                            const int alphaBackground)
{
	...
	GHOST_IWindow *window = startWindow(system, title, 0, 0, (int)sysWidth, (int)sysHeight,
	                                    stereoVisual, alphaBackground);

#ifdef WIN32
	/* Strip the window decorations and stretch the window over the full display instead of
	 * using exclusive fullscreen (GHOST_kWindowStateFullScreen / ChangeDisplaySettings),
	 * which crashes on some drivers when combined with a core profile GL context. */
	HWND ghost_hwnd = findGhostWindowHWND(window);
	if (ghost_hwnd != nullptr) {
		LONG_PTR style = GetWindowLongPtr(ghost_hwnd, GWL_STYLE);
		...
		style |= WS_POPUP;
		SetWindowLongPtr(ghost_hwnd, GWL_STYLE, style);
		SetWindowPos(ghost_hwnd, HWND_TOP, 0, 0, (int)sysWidth, (int)sysHeight,
		            SWP_NOZORDER | SWP_FRAMECHANGED);
	}
#endif  // WIN32

	window->setClientSize(sysWidth, sysHeight);
...
static void usage(const std::string& program, bool isBlenderPlayer)
{
	...
	if (isBlenderPlayer) {
		example_filename = "filename.range";
#ifdef _WIN32
		example_pathname = "c:\\";
#else
	...
	CM_Message("       show_debug_mode                0         Show game debug interface");
	CM_Message("       show_console                   0         Show the in-game console window");
	CM_Message("       ignore_exit_key                0         Ignore the exit key");
...
static void get_filename(int argc, char **argv, char *filename)
{
	...
	srclen -= ::strlen("MacOS/RangeRuntime");
	if (srclen > 0) {
...
/* abort()/std::terminate() (uncaught C++ exception, CRT invalid-parameter
 * in Release builds, assert failures, etc.) don't go through
 * SetUnhandledExceptionFilter at all, so without these the process was
 * dying silently mid-shutdown with nothing in range_runtime.log.txt and no
 * crash.txt — exactly what was seen testing the ESC-close bug. */
static void sigHandleAbort(int signum)
{
	fputs("\nAbort: process received SIGABRT (uncaught exception, assert, "
	      "or CRT invalid-parameter abort)\n", stderr);
	fflush(stderr);
...
#  endif
...
#ifdef WIN32
	/* The Windows console is hidden shortly after startup (see the
	 * system->toggleConsole(0) call below), so anything printed to
	 * stdout/stderr after that point (including a crash backtrace written
	 * by windowsExceptionHandler) is invisible unless the console happened
	 * to be reopened with "-console". Mirror both streams to a persistent
	 * log file in the temp dir so the run can always be inspected after
	 * the fact, regardless of how the process exited. */
	{
	...
	BLF_load_mem("default", (unsigned char *)datatoc_roboto_medium_ttf, datatoc_roboto_medium_ttf_size);
	if (blf_mono_font == -1) {
		blf_mono_font = BLF_load_mem_unique("monospace", (unsigned char *)datatoc_roboto_mono_medium_ttf, datatoc_roboto_mono_medium_ttf_size);
	}
...
#if defined(DEBUG)
					CM_Debug("XWindows ID = " << parentWindow);
#endif // defined(DEBUG)
					break;
				}
...
#if 0 // Use this for test standalone more faster.
				strcpy(filename, (std::string(BKE_appdir_program_dir()) + "\\game.range").c_str());
#else
				...
				do {
					...
					if (!bfd) {
						...
						if ((!fullScreenParFound) && (!windowParFound)) {
							...
							if ((scene->gm.playerflag & GAME_PLAYER_BORDERLESS_WINDOW)) {
								fullScreen = false;
								borderlessWindow = true;
							}
							else if ((scene->gm.playerflag & GAME_PLAYER_FULLSCREEN)) {
								fullScreen = true;
							...
							if (fullScreen) {
...
#endif
								{
									if (parentWindow != 0) {
										...
										window = startBorderlessWindow(system, title, stereoWindow, alphaBackground);
									}
									else {
