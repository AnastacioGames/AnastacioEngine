...
SCA_PropertyActuator::SCA_PropertyActuator(SCA_IObject *gameobj, SCA_IObject *sourceObj, const std::string& propname, const std::string& expr, int acttype, const std::string& runtimeProperty, EXP_Value *worldPropOwner)
...
bool SCA_PropertyActuator::Update()
{
	...
	RemoveAllEvents();
	EXP_Value *propowner = m_worldPropOwner ? m_worldPropOwner : GetParent();
	if (m_useRuntimeProperty && !bNegativeEvent) {
		...
		else if (m_runtimeProperty == "transform.local_position" ||
		         m_runtimeProperty == "transform.local_scale" ||
		         m_runtimeProperty == "physics.linear_velocity" ||
		         m_runtimeProperty == "physics.angular_velocity" ||
		         m_runtimeProperty == "physics.gravity") {
			value.type = SCA_RUNTIME_PROPERTY_VECTOR3;
			if (std::sscanf(m_exprtxt.c_str(), "%f,%f,%f", &value.vectorValue[0], &value.vectorValue[1], &value.vectorValue[2]) != 3) {
				return false;
			}
		}
		else {
			value.type = SCA_RUNTIME_PROPERTY_FLOAT;
			if (!CM_StringTo(m_exprtxt, value.floatValue)) return false;
		}
		if (m_type == KX_ACT_PROP_ADD) {
			SCA_RuntimePropertyValue current;
			if (!target->GetRuntimeProperty(m_runtimeProperty.c_str(), current, &error) || current.type != value.type) {
				return false;
			}
			if (value.type == SCA_RUNTIME_PROPERTY_FLOAT) {
				value.floatValue += current.floatValue;
			}
			else if (value.type == SCA_RUNTIME_PROPERTY_VECTOR3) {
				value.vectorValue[0] += current.vectorValue[0];
				value.vectorValue[1] += current.vectorValue[1];
				value.vectorValue[2] += current.vectorValue[2];
			}
			else {
				return false;
			}
		}
		else if (m_type != KX_ACT_PROP_ASSIGN) {
			return false;
		}
		if (!target->SetRuntimeProperty(m_runtimeProperty.c_str(), value, &error)) {
			return false;
		}
		return false;
	}
