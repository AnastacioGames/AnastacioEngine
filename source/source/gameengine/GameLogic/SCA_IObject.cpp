...
SCA_IObject::~SCA_IObject()
{
	...
	for (SCA_ISensor *sensor : m_registeredSensors) {
		sensor->UnlinkObject(this);
	}
...
bool SCA_IObject::GetRuntimeProperty(const char *, SCA_RuntimePropertyValue&, const char **error)
{
	if (error) {
		*error = "This object type does not expose runtime properties.";
	}
	return false;
}

bool SCA_IObject::SetRuntimeProperty(const char *, const SCA_RuntimePropertyValue&, const char **error)
{
	if (error) {
		*error = "This object type does not expose writable runtime properties.";
	}
	return false;
}
...
void SCA_IObject::RegisterSensor(SCA_ISensor *sens)
{
	// Don't increase ref count, it would create a dead lock.
	m_registeredSensors.push_back(sens);
}

void SCA_IObject::UnregisterSensor(SCA_ISensor *sens)
{
	CM_ListRemoveIfFound(m_registeredSensors, sens);
}
