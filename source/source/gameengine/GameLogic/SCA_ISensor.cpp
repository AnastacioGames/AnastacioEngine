...
bool SCA_ISensor::UnlinkObject(SCA_IObject *)
{
	return false;
}
...
void SCA_ISensor::SetPulseMode(bool posmode, bool negmode, float skippedticks)
{
...
void SCA_ISensor::SetDeltaTime(bool lvl)
{
	m_use_dt = lvl;
}
...
void SCA_ISensor::Replace_EventManager(class SCA_LogicManager *logicmgr)
{
	...
	if (m_links) {
		...
		/* Temp hack fix: currently Replace_EventManager is only used to merge logic bricks with LibLoad,	*/
		/* so we know that SCA_ISensor->m_replaced_Libload is true.											*/
		this->m_replaced_with_Libload = true;
		m_eventmgr->RegisterSensor(this);
	}
	else {
		m_eventmgr = logicmgr->FindEventManager(m_eventmgr->GetType());
...
void SCA_ISensor::Activate(class SCA_LogicManager *logicmgr)
{
	...
	if (m_links && !m_suspended) {
		...
		if (result) {
			...
			if (m_state || !m_tap) {
				this->ActivateControllers(logicmgr);
			 ...
			 * not set :( */
			if (m_pos_pulsemode) {
				if (m_use_dt) { m_pos_ticks += KX_GetActiveEngine()->GetEngineDeltaTime(); }
				else { m_pos_ticks++; } // Normal
			...
			if (m_neg_pulsemode && !m_tap) {
				if (m_use_dt) { m_neg_ticks += KX_GetActiveEngine()->GetEngineDeltaTime(); }
				else { m_neg_ticks++; } // Normal
...
PyAttributeDef SCA_ISensor::Attributes[] = {
	...
	EXP_PYATTRIBUTE_BOOL_RW("useDeltaTime", SCA_ISensor, m_use_dt),
