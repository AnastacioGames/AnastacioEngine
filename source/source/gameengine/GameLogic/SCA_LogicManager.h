...
class SCA_LogicManager
{
	...
	/** Logic execution counters from the last BeginFrame()/UpdateFrame() call: how many
	 * controllers were triggered and how many actuators were updated this frame. Sensor
	 * count is the total registered across all event managers (not a per-frame evaluated
	 * count -- each event manager subclass decides internally which of its sensors to
	 * evaluate). Used only for the Debug Mode "Logic" counters.
	 */
	int		m_lastControllersTriggered;
	int		m_lastActuatorsUpdated;
public:
	SCA_LogicManager();
	...
	// Called by libfree, needed to clear all events to avoid crashs with invalid interactions.
	void ClearAllEvents();
	...
	/// Logic counters from the last frame. See m_lastControllersTriggered.
	int		GetLastControllersTriggered() const { return m_lastControllersTriggered; }
	int		GetLastActuatorsUpdated() const { return m_lastActuatorsUpdated; }
	int		GetTotalRegisteredSensors() const;
};
