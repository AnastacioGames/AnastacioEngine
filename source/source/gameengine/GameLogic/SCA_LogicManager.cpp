...
SCA_LogicManager::SCA_LogicManager()
	:m_lastControllersTriggered(0),
	m_lastActuatorsUpdated(0)
{
}
...
int SCA_LogicManager::GetTotalRegisteredSensors() const
{
	int total = 0;
	for (const std::unique_ptr<SCA_EventManager>& mgr : m_eventmanagers) {
		total += (int)mgr->GetSensorCount();
	}
	return total;
}

void SCA_LogicManager::ClearAllEvents()
{
