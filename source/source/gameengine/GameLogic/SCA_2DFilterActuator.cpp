...
bool SCA_2DFilterActuator::Update()
{
	...
	RAS_2DFilter *filter = m_filterManager->GetFilterPass(m_int_arg, false);
	switch (m_type) {
		...
		case RAS_2DFilterManager::FILTER_FXAA:
		case RAS_2DFilterManager::FILTER_BLOOM:
		case RAS_2DFilterManager::FILTER_TONEMAP:
		case RAS_2DFilterManager::FILTER_SSAO:
		case RAS_2DFilterManager::FILTER_LIGHTSCATTER:
		case RAS_2DFilterManager::FILTER_SSR:
		{
			if (!m_filterManager->SetBuiltinFilterEnabled(
					static_cast<RAS_2DFilterManager::FILTER_MODE>(m_type), true)) {
				CM_LogicBrickWarning(this, "Built-in 2D filter is unavailable.");
			}
			break;
		}
		default:
		{
			if (!filter) {
				...
				m_filterManager->AddFilter(info, false);
			}
			else {
