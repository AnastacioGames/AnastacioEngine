...
public:
	VehicleClosestRayResultCallback(const btVector3& rayFrom,
	                              const btVector3& rayTo,
	                              unsigned short mask,
	                              const btCollisionObject *ignoredObject)
	...
	virtual bool needsCollision(btBroadphaseProxy *proxy0) const
	{
		...
		// A raycast wheel must never use its own rigid body as ground. This is
		// especially important for a Convex Hull chassis: a suspension ray can
		// cross the hull near its attachment point and otherwise launch the car.
		if (object == m_ignoredObject) {
			return false;
		}
...
private:
	...
	unsigned short m_mask;
	const btCollisionObject *m_chassisObject;

public:
	BlenderVehicleRaycaster(btDynamicsWorld *world, const btCollisionObject *chassisObject)
	...
	virtual void *castRay(const btVector3& from, const btVector3& to, btVehicleRaycasterResult& result)
	{
		...
		VehicleClosestRayResultCallback rayCallback(from, to, m_mask, m_chassisObject);
...
public:
	...
	~WrapperVehicle()
	{
		if (m_invalidationCallback) {
			m_invalidationCallback(m_invalidationClient);
		}
	...
	virtual PHY_IPhysicsController *GetChassisController() const
	{
		return m_chassis;
	}
	                      ...
	                      bool hasSteering)
	{
		...
		config.connectionPoint = connectionPoint;
		config.downDirection = downDirection.Normalized();
		config.axleDirection = axleDirection.Normalized();
		config.suspensionRestLength = suspensionRestLength;
		config.wheelRadius = wheelRadius;
		config.hasSteering = hasSteering;
		config.isDriveWheel = false;
		m_wheelConfigs.push_back(config);
	}

	virtual void SetWheelIsDriveWheel(int wheelIndex, bool isDriveWheel)
	{
		if ((wheelIndex < 0) || (wheelIndex >= (int)m_wheelConfigs.size())) {
			return;
		}
		m_wheelConfigs[wheelIndex].isDriveWheel = isDriveWheel;
	}
	...
	virtual bool GetWheelConfig(int wheelIndex, PHY_VehicleWheelConfig *out) const
	{
		if ((wheelIndex < 0) || (wheelIndex >= (int)m_wheelConfigs.size())) {
			return false;
		}
		*out = m_wheelConfigs[wheelIndex];
		return true;
	}

	virtual bool GetWheelState(int wheelIndex, PHY_VehicleWheelState *out) const
	{
		if ((wheelIndex < 0) || (wheelIndex >= m_vehicle->getNumWheels())) {
			return false;
		}
		const btWheelInfo& info = m_vehicle->getWheelInfo(wheelIndex);
		out->worldPosition = ToMt(info.m_worldTransform.getOrigin());
		out->worldOrientation = ToMt(info.m_worldTransform.getRotation());
		out->rotation = info.m_rotation;
		out->isInContact = info.m_raycastInfo.m_isInContact;
		out->contactNormal = ToMt(info.m_raycastInfo.m_contactNormalWS);
		out->contactPoint = ToMt(info.m_raycastInfo.m_contactPointWS);
		out->suspensionForce = info.m_wheelsSuspensionForce;
		out->physicsTick = m_physicsTick;
		out->hardPointWS = ToMt(info.m_raycastInfo.m_hardPointWS);
		out->wheelDirectionWS = ToMt(info.m_raycastInfo.m_wheelDirectionWS);
		return true;
	}

	virtual unsigned long long GetPhysicsTick() const
	{
		return m_physicsTick;
	}

	virtual float GetCurrentSpeedKmHour() const
	{
		return m_vehicle->getCurrentSpeedKmHour();
	}

	virtual float GetCurrentSpeedMps() const
	{
		return m_vehicle->getCurrentSpeedKmHour() / 3.6f;
	}

	virtual mt::vec3 GetForwardVector() const
	{
		return ToMt(m_vehicle->getForwardVector());
	}

	virtual void GetCoordinateSystem(int *rightIndex, int *upIndex, int *forwardIndex) const
	{
		*rightIndex = m_vehicle->getRightAxis();
		*upIndex = m_vehicle->getUpAxis();
		*forwardIndex = m_vehicle->getForwardAxis();
	}
	...
	virtual void SetMaxSuspensionTravel(float maxSuspensionTravelCm, int wheelIndex)
	{
		if ((wheelIndex >= 0) && (wheelIndex < m_vehicle->getNumWheels())) {
			btWheelInfo& info = m_vehicle->getWheelInfo(wheelIndex);
			info.m_maxSuspensionTravelCm = maxSuspensionTravelCm;
		}
	}

	virtual void SetMaxSuspensionForce(float maxSuspensionForce, int wheelIndex)
	{
		if ((wheelIndex >= 0) && (wheelIndex < m_vehicle->getNumWheels())) {
			btWheelInfo& info = m_vehicle->getWheelInfo(wheelIndex);
			info.m_maxSuspensionForce = maxSuspensionForce;
		}
	}

	virtual float GetMaxSuspensionTravel(int wheelIndex) const
	{
		if ((wheelIndex >= 0) && (wheelIndex < m_vehicle->getNumWheels())) {
			return m_vehicle->getWheelInfo(wheelIndex).m_maxSuspensionTravelCm;
		}
		return 0.0f;
	}

	virtual float GetMaxSuspensionForce(int wheelIndex) const
	{
		if ((wheelIndex >= 0) && (wheelIndex < m_vehicle->getNumWheels())) {
			return m_vehicle->getWheelInfo(wheelIndex).m_maxSuspensionForce;
		}
		return 0.0f;
	}

	virtual float GetWheelFriction(int wheelIndex) const
	{
		if ((wheelIndex >= 0) && (wheelIndex < m_vehicle->getNumWheels())) {
			return m_vehicle->getWheelInfo(wheelIndex).m_frictionSlip;
		}
		return 0.0f;
	}

	virtual float GetSuspensionStiffness(int wheelIndex) const
	{
		if ((wheelIndex >= 0) && (wheelIndex < m_vehicle->getNumWheels())) {
			return m_vehicle->getWheelInfo(wheelIndex).m_suspensionStiffness;
		}
		return 0.0f;
	}

	virtual float GetSuspensionDamping(int wheelIndex) const
	{
		if ((wheelIndex >= 0) && (wheelIndex < m_vehicle->getNumWheels())) {
			return m_vehicle->getWheelInfo(wheelIndex).m_wheelsDampingRelaxation;
		}
		return 0.0f;
	}

	virtual float GetSuspensionCompression(int wheelIndex) const
	{
		if ((wheelIndex >= 0) && (wheelIndex < m_vehicle->getNumWheels())) {
			return m_vehicle->getWheelInfo(wheelIndex).m_wheelsDampingCompression;
		}
		return 0.0f;
	}

	virtual float GetRollInfluence(int wheelIndex) const
	{
		if ((wheelIndex >= 0) && (wheelIndex < m_vehicle->getNumWheels())) {
			return m_vehicle->getWheelInfo(wheelIndex).m_rollInfluence;
		}
		return 0.0f;
	}

	virtual float GetChassisMass() const
	{
		return m_chassis->GetMass();
	}

	virtual void QueueParameterCommand(const PHY_VehicleParameterCommand &command)
	{
		if (!std::isfinite(command.value)) {
			return;
		}
		const bool perWheel = (command.id != PHY_VEHICLE_PARAM_CHASSIS_MASS) &&
		                      (command.id != PHY_VEHICLE_PARAM_RESET_SUSPENSION);
		if (perWheel && ((command.wheelIndex < 0) || (command.wheelIndex >= m_vehicle->getNumWheels()))) {
			return;
		}
	...
	{
		for (const PHY_VehicleParameterCommand &cmd : m_pendingCommands) {
			switch (cmd.id) {
				case PHY_VEHICLE_PARAM_SUSPENSION_STIFFNESS:
					SetSuspensionStiffness(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_SUSPENSION_DAMPING_RELAXATION:
					SetSuspensionDamping(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_SUSPENSION_DAMPING_COMPRESSION:
					SetSuspensionCompression(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_WHEEL_FRICTION:
					SetWheelFriction(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_ROLL_INFLUENCE:
					SetRollInfluence(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_MAX_SUSPENSION_TRAVEL:
					SetMaxSuspensionTravel(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_MAX_SUSPENSION_FORCE:
					SetMaxSuspensionForce(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_STEERING:
					SetSteeringValue(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_ENGINE_FORCE:
					ApplyEngineForce(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_BRAKE:
					ApplyBraking(cmd.value, cmd.wheelIndex);
					break;
				case PHY_VEHICLE_PARAM_CHASSIS_MASS:
					m_chassis->SetMass(cmd.value);
					break;
	...
	virtual void SetInvalidationCallback(void (*callback)(void *client), void *client)
	{
		m_invalidationCallback = callback;
		m_invalidationClient = client;
	}

private:
	void (*m_invalidationCallback)(void *client) = nullptr;
	void *m_invalidationClient = nullptr;
	std::vector<PHY_VehicleWheelConfig> m_wheelConfigs;
	std::vector<PHY_VehicleParameterCommand> m_pendingCommands;
	unsigned long long m_physicsTick = 0;
};
...
//changed here 5-9-2025
void CcdPhysicsEnvironment::ProceedDeltaTime(double timeStep, double interval)
{
	//std::set<CcdPhysicsController *>::iterator it;
	...
	if (m_numTimeSubSteps > 1) {
		subStep = timeStep / double(m_numTimeSubSteps);
	}
	else {
		subStep = timeStep;
	}
	...
	if (i < 1) {
		//CM_Warning("subStep " << i << " times");
		return;
	}
	for (unsigned short r=0;r<i;r++) {
	...
	// Sync wheel transforms from the raycast results of the step that just ran,
	// not the previous tick's (matches ProceedDeltaTimeCar's already-correct order).
	for (c = 0; c < m_wrapperVehicles.size(); c++) {
		WrapperVehicle *veh = m_wrapperVehicles[c];
		veh->SyncWheels();
	}
...
void CcdPhysicsEnvironment::ProceedDeltaTimeCar(double timeStep, double interval)
{
	...
	double subStep = timeStep / double(m_numTimeSubSteps);
	i = m_dynamicsWorld->stepSimulation(interval, 25, subStep);//perform always a full simulation step
...
void CcdPhysicsEnvironment::ProcessFhSprings()
{
	if (m_numFhControllers == 0) {
		return;
	}
...
void CcdPhysicsEnvironment::SetNumTimeSubSteps(int numTimeSubSteps)
{
	if (numTimeSubSteps < 1) {
		numTimeSubSteps = 1;
	}
	if (numTimeSubSteps > 25) {
		CM_Warning("physics substeps (" << numTimeSubSteps << ") exceed Bullet's maxSubSteps (25); "
		           "simulation will fall behind under load instead of using the requested substep size");
	}
	m_numTimeSubSteps = numTimeSubSteps;
}

void CcdPhysicsEnvironment::SetFixedTimeStep(bool useFixedTimeStep, float fixedTimeStep)
{
	m_fixedTimeStep = useFixedTimeStep ? fixedTimeStep : 0.f;
...
int CcdPhysicsEnvironment::GetNumVehicles() const
{
	return (int)m_wrapperVehicles.size();
}

PHY_IVehicle *CcdPhysicsEnvironment::GetVehicleFromIndex(int index) const
{
	if ((index < 0) || (index >= (int)m_wrapperVehicles.size())) {
		return nullptr;
	}
	return m_wrapperVehicles[index];
}
...
PHY_IPhysicsController *CcdPhysicsEnvironment::CreateSphereController(float radius, const mt::vec3& position)
{
	...
	cinfo.m_collisionShape = new btSphereShape(radius); // freed by CcdPhysicsController::DeleteControllerShape() when the controller is destroyed
...
PHY_IVehicle *CcdPhysicsEnvironment::CreateVehicle(PHY_IPhysicsController *ctrl)
{
	...
	btRigidBody *chassisBody = ((CcdPhysicsController *)ctrl)->GetRigidBody();
	if (!chassisBody) {
		return nullptr;
	}

	const btRaycastVehicle::btVehicleTuning tuning = btRaycastVehicle::btVehicleTuning();
	BlenderVehicleRaycaster *raycaster = new BlenderVehicleRaycaster(m_dynamicsWorld, chassisBody);
	btRaycastVehicle *vehicle = new btRaycastVehicle(tuning, chassisBody, raycaster);
...
void CcdPhysicsEnvironment::DestroyVehicle(PHY_IVehicle *vehicle)
{
	if (!vehicle) {
		return;
	}

	RemoveVehicle((WrapperVehicle *)vehicle, true);
}
                                          ...
                                          int activeLayerBitInfo, bool isCompoundChild, bool hasCompoundChildren)
{
	...
	// a compound shape with the original shape shifted by -offset, so the
	// compound's own origin (which becomes the rigid body/COM transform)
	// sits at +offset from the visible mesh. CcdPhysicsController pairs this
	// with a motion-state compensation so the object doesn't visibly move;
	// see BL_BlenderDataConversion.cpp's vehicle_com_offset handling and
	// CcdPhysicsController::SetCenterOfMassOffset.
	const bool wantsComOffset = (blenderobject->gameflag2 & OB_VEHICLE) != 0 && !isCompoundChild && !hasCompoundChildren &&
	                             (blenderobject->vehicle_com_offset[0] != 0.0f ||
	                              blenderobject->vehicle_com_offset[1] != 0.0f ||
	                              blenderobject->vehicle_com_offset[2] != 0.0f);
	if (wantsComOffset) {
		CcdShapeConstructionInfo *comShapeInfo = new CcdShapeConstructionInfo();
		comShapeInfo->m_shapeType = PHY_SHAPE_COMPOUND;
		comShapeInfo->AddShape(shapeInfo);

		btVector3 comOffset(blenderobject->vehicle_com_offset[0],
		                     blenderobject->vehicle_com_offset[1],
		                     blenderobject->vehicle_com_offset[2]);
		shapeInfo->m_childTrans.setIdentity();
		shapeInfo->m_childTrans.setOrigin(-comOffset);

		btCompoundShape *comCompoundShape = new btCompoundShape();
		comCompoundShape->addChildShape(shapeInfo->m_childTrans, bm);

		bm = comCompoundShape;
		shapeInfo->Release();
		shapeInfo = comShapeInfo;
	}
