...
struct CcdConstructionInfo {
	...
	CcdConstructionInfo()
		...
		m_soft_kSHR(1.0f),
		m_soft_kAHR(0.7f),
		m_ccd_motion_threshold(0.0f),
		m_ccd_swept_sphere_radius(0.0f),
		m_collisionFlags(0),
		m_bDyna(false),
		m_bRigid(false),
		m_bSoft(false),
...
protected:
	...
	/// Non-null while this controller's shape is embedded in another controller's compound shape
	/// via AddCompoundChild(); used to auto-detach on destruction if RemoveCompoundChild() was
	/// never called explicitly (e.g. the object was deleted while still dynamically parented).
	CcdPhysicsController *m_compoundParent;
...
public:
	...
	/**
	 * Shift this rigid body's center of mass away from its node/graphics
	 * origin by \a localOffset (in the object's local space), without
	 * moving the visible object. Callers must have already wrapped the
	 * collision shape in a compound shape with the chassis child placed at
	 * -localOffset (see BL_BlenderDataConversion.cpp's vehicle_com_offset
	 * handling); this only teaches the motion-state bridge to compensate
	 * for that shift when syncing with Bullet.
	 */
	virtual void SetCenterOfMassOffset(const mt::vec3& localOffset);
