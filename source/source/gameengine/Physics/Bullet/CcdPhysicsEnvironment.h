...
protected:
	...
	/// value passed to SetFixedTimeStep, returned as-is by GetFixedTimeStep (0.f if not using a fixed timestep)
...
public:
	...
	/// Perform an integration step of duration 'timeStep'.
	virtual void ProceedDeltaTimeCar(double timeStep, double interval);
	virtual void ProceedDeltaTime(double timeStep, double interval);
...
//		virtual bool		proceedDeltaTimeOneStep(float timeStep);
	...
	virtual void DestroyVehicle(PHY_IVehicle *vehicle);
	...
	virtual PHY_IVehicle *GetVehicleFromIndex(int index) const;
...
protected:
	...
	/** Number of controllers in m_controllers using Fh springs (m_do_fh or m_do_rot_fh),
	 * kept in sync in AddCcdPhysicsController/RemoveCcdPhysicsController so ProcessFhSprings
	 * can skip its per-frame scan of m_controllers when no object uses the feature. */
	int m_numFhControllers;
