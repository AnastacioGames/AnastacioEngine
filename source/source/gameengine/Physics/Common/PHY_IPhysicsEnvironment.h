...
public:
	...
	virtual void ProceedDeltaTimeCar(double timeStep, double interval) = 0;
	virtual void ProceedDeltaTime(double timeStep, double interval) = 0;
	...
	virtual void DestroyVehicle(PHY_IVehicle *vehicle) = 0;
	...
	/* Read-only enumeration of live vehicles, for debug draw / inspection.
	 * Never exposes the backend's concrete vehicle type, only the interface. */
	virtual int GetNumVehicles() const = 0;
	virtual PHY_IVehicle *GetVehicleFromIndex(int index) const = 0;
