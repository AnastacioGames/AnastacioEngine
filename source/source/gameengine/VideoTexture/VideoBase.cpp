...
int Video_setRange(PyImage *self, PyObject *value, void *closure)
{
	...
	const bool valid = PySequence_Fast_GET_SIZE(sequence) == 2 &&
	                   PyFloat_Check(PySequence_Fast_GET_ITEM(sequence, 0)) &&
	                   PyFloat_Check(PySequence_Fast_GET_ITEM(sequence, 1));
	if (!valid) {
		Py_DECREF(sequence);
		PyErr_SetString(PyExc_TypeError, "The value must be a sequence of 2 float");
		return -1;
	...
	// set range
	getVideo(self)->setRange(PyFloat_AsDouble(PySequence_Fast_GET_ITEM(sequence, 0)),
