...
static int setMatrix(PyFilter *self, PyObject *value, void *closure)
{
	...
	ColorMatrix mat;
	PyObject *matrix = value ? PySequence_Fast(value, nullptr) : nullptr;
	// check validity of parameter
	bool valid = matrix != nullptr && PySequence_Fast_GET_SIZE(matrix) == 4;
	// check rows
	for (int r = 0; valid && r < 4; ++r)
	{
		// get row object
		PyObject *row = PySequence_Fast(PySequence_Fast_GET_ITEM(matrix, r), nullptr);
		// check sequence
		valid = row != nullptr && PySequence_Fast_GET_SIZE(row) == 5;
		// check items
...
static int setLevels(PyFilter *self, PyObject *value, void *closure)
{
	...
	for (int r = 0; valid && r < 4; ++r)
	{
		...
		PyObject *row = PySequence_Fast(PySequence_Fast_GET_ITEM(matrix, r), nullptr);
		// check sequence
		valid = row != nullptr && PySequence_Fast_GET_SIZE(row) == 2;
		// check items
