...
bool ImageRender::Render()
{
	...
	if (m_updateShadowBuffer) {
		m_engine->GetShadowRenderer()->Render(m_scene);
	}
	...
	if (m_scene->GetWorldInfo()) {
		...
		m_scene->GetWorldInfo()->UpdateBackGround(m_rasterizer, m_scene->GetWorldSun());
	...
	const std::vector<KX_GameObject *> objects = m_scene->CalculateVisibleMeshes(m_camera, camFrustum, 0, false);

	if (m_engine->UpdateAnimations(m_scene)) {
...
static int setHorizon(PyImage *self, PyObject *value, void *closure)
{
	...
	if (value == nullptr || (sequence = PySequence_Fast(value, nullptr)) == nullptr ||
	    PySequence_Fast_GET_SIZE(sequence) != 4 ||
	    (!PyFloat_Check(PySequence_Fast_GET_ITEM(sequence, 0)) && !PyLong_Check(PySequence_Fast_GET_ITEM(sequence, 0))) ||
	    (!PyFloat_Check(PySequence_Fast_GET_ITEM(sequence, 1)) && !PyLong_Check(PySequence_Fast_GET_ITEM(sequence, 1))) ||
	    (!PyFloat_Check(PySequence_Fast_GET_ITEM(sequence, 2)) && !PyLong_Check(PySequence_Fast_GET_ITEM(sequence, 2))) ||
	    (!PyFloat_Check(PySequence_Fast_GET_ITEM(sequence, 3)) && !PyLong_Check(PySequence_Fast_GET_ITEM(sequence, 3)))) {
		Py_XDECREF(sequence);
	...
	getImageRender(self)->setHorizon(
		PyFloat_AsDouble(PySequence_Fast_GET_ITEM(sequence, 0)),
		PyFloat_AsDouble(PySequence_Fast_GET_ITEM(sequence, 1)),
		PyFloat_AsDouble(PySequence_Fast_GET_ITEM(sequence, 2)),
		PyFloat_AsDouble(PySequence_Fast_GET_ITEM(sequence, 3)));
	Py_DECREF(sequence);
	// success
	return 0;
}
