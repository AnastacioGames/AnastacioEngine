...
public:
	...
	enum DrawType {
		...
		RAS_RENDERER,
		RAS_SHADOW,
		/* Depth-only pass restricted to objects flagged use_gpu_particle_collider (see
		 * RAS_BucketManager::Renderbuckets()), independent of any material flag -- feeds
...
private:
	...
	/// Uniform locations shared by the shadow (black/VSM) override shaders, core profile only.
	struct OverrideShaderShadowInterface
	{
		int viewMatLoc;
		int objectMatLoc;
...
public:
	...
	/**
	 * Sets wireframe width.
	 */
	void SetLinesWidth(float size);
	 ...
	 * \param useShadow		Enable shadows.
	...
	/// Upload the per-object matrix uniforms used by the shadow override shaders (core profile only).
	void UpdateOverrideShaderObjectMatrix(const float obmat[16]);
	 ...
	 */
	void PrintHardwareInfo(bool fullInfo);
