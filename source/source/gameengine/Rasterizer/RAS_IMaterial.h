...
public:
	...
	/** Re-upload the per-object model matrix uniform after RAS_Rasterizer::GetTransform() has
	 * adjusted it (e.g. RAS_HALO/RAS_BILLBOARD camera-facing rotation). Needed because that
	 * adjusted matrix is otherwise only applied through the legacy fixed-function matrix stack
	 * (RAS_Rasterizer::MultMatrix), which is a no-op under WITH_GL_PROFILE_CORE. Default no-op
	 * for materials that don't bind an object matrix uniform of their own (e.g. text). */
	virtual void UpdateObjectMatrix(RAS_MeshUser *, RAS_Rasterizer *, const float[16]) {}

	bool IsAlpha() const;
	bool IsAlphaDepth() const;
	bool IsZSort() const;
	bool IsWire() const;
	bool IsHaloPoints() const;
	bool IsText() const;
