...
protected:
	enum BucketType {
		...
		/* Subset of ALPHA_DEPTH_BUCKET/ALPHA_DEPTH_INSTANCING_BUCKET restricted to true cutout
		 * materials (Clip/Alpha-to-Coverage). Used only to drive the Z-prepass, which writes
		 * real hardware depth per fragment -- safe for binary-alpha cutout materials, but wrong
		 * for smooth alpha-blended ones (see Renderbuckets()/FindBucket()). Kept separate from
