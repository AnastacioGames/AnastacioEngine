...
RAS_DebugDraw::LightGlow::LightGlow(const mt::vec3& pos, float size, const mt::vec4& color)
	:Shape(color),
	m_size(size)
{
	pos.Pack(m_pos);
...
void RAS_DebugDraw::DrawLightGlow(const mt::vec3& pos, float size, const mt::vec4& color)
{
	m_lightGlows.emplace_back(pos, size, color);
}

void RAS_DebugDraw::Flush(RAS_Rasterizer *rasty, RAS_ICanvas *canvas)
{
	if ((m_lines.size() + m_aabbs.size() + m_frustums.size() + m_texts2d.size() + m_boxes2d.size() + m_lightGlows.size()) == 0) {
		return;
	}
