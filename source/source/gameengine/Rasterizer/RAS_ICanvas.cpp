...
int RAS_ICanvas::GetRenderWidth() const
{
	return std::max(1, (int)std::lround(GetWidth() * m_renderScale));
}

int RAS_ICanvas::GetRenderHeight() const
{
	return std::max(1, (int)std::lround(GetHeight() * m_renderScale));
}
...
void RAS_ICanvas::RequestMouseVisible(int requester, bool visible)
{
	if (visible) {
		m_mouseVisibleRequesters |= requester;
	}
	else {
		m_mouseVisibleRequesters &= ~requester;
	}
...
{
	const unsigned int width = GetRenderWidth();
	const unsigned int height = GetRenderHeight();
...
void RAS_ICanvas::SetRenderScale(float scale)
{
	const float clampedScale = std::max(0.25f, std::min(scale, 1.0f));
	if (std::abs(m_renderScale - clampedScale) < 0.0001f) {
		return;
	}
...
float RAS_ICanvas::GetRenderScale() const
{
