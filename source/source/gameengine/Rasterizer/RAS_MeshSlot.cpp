...
void RAS_MeshSlot::RunNode(const RAS_MeshSlotNodeTuple& tuple)
{
	...
	if (materialData->m_text) {
		...
		if (displayArrayData->m_applyMatrix) {
			...
			if (managerData->m_shaderOverride) {
				rasty->UpdateOverrideShaderObjectMatrix(mat);
			}
			else if (materialData->m_drawingMode != RAS_IMaterial::RAS_NORMAL) {
				// RAS_HALO/RAS_BILLBOARD: mat above already carries the camera-facing rotation,
				// but that only reaches the vertex shader through the legacy MultMatrix() stack
				// (a no-op under WITH_GL_PROFILE_CORE). Re-upload it as the real object-matrix
				// uniform too, so it isn't lost under core profile.
