...
public:
	...
	/// Bitmask of systems that want the OS mouse cursor left alone (visible, not re-centered).
	enum RAS_MouseCursorRequester
	{
		MOUSE_REQ_DEBUG = (1 << 0),
		MOUSE_REQ_GAME_UI = (1 << 1)
	};
	...
	int GetWidth() const;
	int GetHeight() const;
	/// Dimensions used by the internal 3D render targets. The visible canvas stays at GetWidth()/GetHeight().
	...
	virtual void SetMousePosition(int x, int y, bool debugModeCamera = false) = 0;

	/// Set or clear a requester's bit; cursor stays free while any bit is set.
	void RequestMouseVisible(int requester, bool visible);

	void ToggleDebugModeAllowMouse()
	{
		RequestMouseVisible(MOUSE_REQ_DEBUG, !GetDebugModeAllowMouse());
	}

	virtual bool GetDebugModeAllowMouse()
	{
		return (m_mouseVisibleRequesters & MOUSE_REQ_DEBUG) != 0;
	}

	/// True while any requester (debug, game UI, ...) wants the cursor left alone.
	bool GetMouseVisibleRequested() const
	{
		return m_mouseVisibleRequesters != 0;
	}
	...
	/// Change only the internal render-target scale; presentation, input and UI retain native canvas size.
...
protected:
	...
	/* "Smart mouse", responsible for disabling the set mouse position of the entire canvas.
	Bitmask of RAS_MouseCursorRequester; cursor stays free while any bit is set. */
	int m_mouseVisibleRequesters = 0;
