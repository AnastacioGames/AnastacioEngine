...
void main(){
	...
	vec4 color = texture(bgl_RenderedTexture, coord);
	vec4 Attach = texture(bgl_DataTextures[0], coord);
	vec2 outl = vec2(Attach.a, 0);
	vec4 AttachOut;

	if (Attach.a < 0.1){
		for (int i = 0; i < 4; ++i){
			AttachOut = texture(bgl_DataTextures[0], coord + outl);
			if (AttachOut.a > .9){
