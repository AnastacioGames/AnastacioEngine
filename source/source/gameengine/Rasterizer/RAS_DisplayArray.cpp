			...
			const mt::vec4_packed& boneIndices, const mt::vec4_packed& boneWeights)
{
	...
	if (m_format.hasBoneData) {
		m_vertexData.boneIndices.push_back(boneIndices);
		m_vertexData.boneWeights.push_back(boneWeights);
	}
...
void RAS_DisplayArray::RemoveVertex(unsigned int start, unsigned int end)
{
	...
	if (m_format.hasBoneData) {
		removeRange(m_vertexData.boneIndices, start, end);
		removeRange(m_vertexData.boneWeights, start, end);
	}
...
void RAS_DisplayArray::Clear()
{
	...
	if (m_format.hasBoneData) {
		m_vertexData.boneIndices.clear();
		m_vertexData.boneWeights.clear();
	}
...
void RAS_DisplayArray::SortPolygons(const mt::mat3x4& transform, unsigned int *indexmap)
{
	...
	if (!indexmap || totpoly <= 1 || m_type == LINES || m_type == POINTS) {
		return;
	}
...
void RAS_DisplayArray::UpdateFrom(RAS_DisplayArray *other, int flag)
{
	...
	if ((flag & BONE_MODIFIED) && m_format.hasBoneData) {
		m_vertexData.boneIndices = other->m_vertexData.boneIndices;
		m_vertexData.boneWeights = other->m_vertexData.boneWeights;
	}
...
RAS_DisplayArrayLayout RAS_DisplayArray::GetLayout() const
{
	...
	if (m_format.hasBoneData) {
		layout.boneIndices = offset;
		offset += sizeof(mt::vec4_packed) * size;

		layout.boneWeights = offset;
		offset += sizeof(mt::vec4_packed) * size;
	}
