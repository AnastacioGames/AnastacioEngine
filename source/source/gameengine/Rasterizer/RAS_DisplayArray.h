...
public:
	...
	enum {
		...
		BONE_MODIFIED = 1 << 5, // Vertex bone indices/weights modified.
		SIZE_MODIFIED = 1 << 6, // Vertex and index array changed of size.
		STORAGE_INVALID = 1 << 7, // Storage not yet created.
		AABB_MODIFIED = POSITION_MODIFIED,
		MESH_MODIFIED = POSITION_MODIFIED | NORMAL_MODIFIED | UVS_MODIFIED |
						COLORS_MODIFIED | TANGENT_MODIFIED | BONE_MODIFIED,
	...
	struct Format
	{
		...
		{
			return (uvSize == other.uvSize && colorSize == other.colorSize && hasBoneData == other.hasBoneData);
		}
...
protected:
	...
	struct VertexData
	{
		...
		/// Per-vertex bone deform-group indices (up to 4 influences), used for GPU skinning.
		std::vector<mt::vec4_packed> boneIndices;
		/// Per-vertex bone weights matching boneIndices, renormalized to sum to 1.
		std::vector<mt::vec4_packed> boneWeights;
...
public:
	...
	inline mt::vec4_packed& GetBoneIndices(const unsigned int index)
	{
		return m_vertexData.boneIndices[index];
	}

	inline mt::vec4_packed& GetBoneWeights(const unsigned int index)
	{
		return m_vertexData.boneWeights[index];
	}
	...
	inline void SetBoneIndices(const unsigned int index, const mt::vec4_packed& value)
	{
		m_vertexData.boneIndices[index] = value;
	}

	inline void SetBoneWeights(const unsigned int index, const mt::vec4_packed& value)
	{
		m_vertexData.boneWeights[index] = value;
	}
