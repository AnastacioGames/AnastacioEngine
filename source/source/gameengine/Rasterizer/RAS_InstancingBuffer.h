...
private:
	...
	/// The OpenGL VBO.
	GPUBuffer *m_vbo;
	/// Number of instances the VBO is currently allocated to hold (may exceed the last requested size).
	...
	/// True if the instancing vertex attributes are already bound on the current VAO
	/// with a shader/override matching m_boundOverride/m_boundOverrideType below.
	bool m_bound;
...
public:
	...
	/// True if RunInstancingNode must (re)issue the vertex-attribute-pointer calls.
	bool NeedsRebind(bool override_, int overrideType) const
	{
		return !m_bound || m_boundOverride != override_ ||
		       (override_ && m_boundOverrideType != overrideType);
	}
	/// Record that the attribute bindings now match the current buffer/VAO state.
	void MarkBound(bool override_, int overrideType)
	{
		m_bound = true;
		m_boundOverride = override_;
		m_boundOverrideType = overrideType;
	}
	/// Force the next RunInstancingNode call to rebind (VAO was recreated elsewhere).
