...
public:
	...
	/** Return the flattened, column-major bone matrix palette for GPU skinning, if this
	 * deformer currently drives one (see BL_SkinDeformer, ARM_VDEF_BGE_GPU).
	 * \param matrices Set to the start of a contiguous array of 4x4 matrices (16 floats each).
	 * \param count Set to the number of matrices in the array.
	 * \return false (with matrices/count left untouched) when this deformer has no GPU skinning data.
	 */
	virtual bool GetBoneMatrices(const float **matrices, int *count) const
	{
		return false;
	}

	/// True when this deformer is configured for GPU skinning (see BL_SkinDeformer, ARM_VDEF_BGE_GPU).
