...
void RAS_StorageVbo::CopyVertexData(const RAS_DisplayArrayLayout& layout, unsigned int modifiedFlag)
{
	...
	if ((modifiedFlag & RAS_DisplayArray::BONE_MODIFIED) && format.hasBoneData) {
		copySubData(layout.boneIndices, data.boneIndices);
		copySubData(layout.boneWeights, data.boneWeights);
	}
}
