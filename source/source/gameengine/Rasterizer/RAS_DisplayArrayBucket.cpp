	...
	if (m_displayArray) {
		...
		if (modifiedFlag != RAS_DisplayArray::NONE_MODIFIED) {
			if (modifiedFlag & RAS_DisplayArray::STORAGE_INVALID) {
				...
				m_attribArray.Clear();
				// The VAO(s) cleared above are about to be rebuilt from scratch; any
				// instancing vertex-attribute bindings cached against the old VAO are
				// gone too, even though the instancing buffer's own data/offsets are
				// still valid (unlike the material-reload case below, no need to
				// recreate the buffer itself here, just its VAO-side binding record).
				for (std::unique_ptr<RAS_InstancingBuffer>& buffer : m_instancingBuffer) {
					if (buffer) {
		...
		if (m_materialUpdateClient.GetInvalidAndClear()) {
			...
			// Force the instancing buffer(s) to be recreated with fresh attribs next time
			// they're needed: mat->GetInstancingAttribs() may have changed now that the
			// material was (re)loaded (e.g. its GPU shader just became Ok()). Without this,
			// a buffer created before the shader was ready stays locked to DEFAULT_ATTRIBS
			// (missing color/layer/info) for its whole lifetime - see UpdateActiveMeshSlots's
			// lazy-init check below, which only creates a buffer when the slot is null.
			for (std::unique_ptr<RAS_InstancingBuffer>& buffer : m_instancingBuffer) {
...
void RAS_DisplayArrayBucket::RunInstancingNode(const RAS_DisplayArrayNodeTuple& tuple)
{
	...
	// Bind all vertex attributes for the used material and the given buffer offset,
	// but only if they aren't already bound with a matching shader/location set.
	// buffer->InvalidateBinding() is called whenever that would no longer hold: when
	// Realloc() actually reallocates the VBO, and when the mesh's VAO gets rebuilt
	// (RAS_DisplayArrayBucket::UpdateActiveMeshSlots, SIZE_MODIFIED/material-reload).
	const bool useOverride = managerData->m_shaderOverride;
	const int overrideType = useOverride ? (int)rasty->GetOverrideShader() : 0;
	if (buffer->NeedsRebind(useOverride, overrideType)) {
		if (useOverride) {
		...
		buffer->MarkBound(useOverride, overrideType);
	}
