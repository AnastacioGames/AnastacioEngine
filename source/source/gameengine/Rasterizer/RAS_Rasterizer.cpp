...
public:
	RAS_PopMatrixGuard(RAS_Rasterizer *rasty)
		:m_rasty(rasty)
	{
	}
	~RAS_PopMatrixGuard()
...
void RAS_Rasterizer::ResetDrawCallCounters()
{
...
void RAS_Rasterizer::IncMaterialChangeCount()
{
		...
		case RAS_STEREO_ABOVEBELOW:
		{
			switch (eye) {
				case RAS_STEREO_LEFTEYE:
				{
					...
					area.SetLeft(0);
					area.SetBottom(canvas->GetRenderHeight() - (canvas->GetRenderHeight() - m_noOfScanlines - 1) / 2);

					area.SetRight(canvas->GetRenderWidth() - 1);
					area.SetTop(canvas->GetRenderHeight() - 1);
					break;
				}
				case RAS_STEREO_RIGHTEYE:
				{
					...
					area.SetBottom(0);
					area.SetRight(canvas->GetRenderWidth() - 1);
					area.SetTop((canvas->GetRenderHeight() - 1 - m_noOfScanlines) / 2);
					break;
				}
				default:
				{
		...
		case RAS_STEREO_3DTVTOPBOTTOM:
		{
			switch (eye) {
				case RAS_STEREO_LEFTEYE:
				{
					// upper half of window
					area.SetLeft(0);
					area.SetBottom(canvas->GetRenderHeight() - canvas->GetRenderHeight() / 2);
				...
				case RAS_STEREO_RIGHTEYE:
				{
					...
					area.SetBottom(0);
					area.SetRight(canvas->GetRenderWidth() - 1);
					area.SetTop((canvas->GetRenderHeight() - 1) / 2);
					break;
				}
				default:
				{
		...
		case RAS_STEREO_SIDEBYSIDE:
		{
			switch (eye) {
				case RAS_STEREO_LEFTEYE:
				{
					...
					area.SetLeft(0);
					area.SetBottom(0);
					area.SetRight((canvas->GetRenderWidth() - 1) / 2);
					area.SetTop(canvas->GetRenderHeight() - 1);
					break;
				}
				case RAS_STEREO_RIGHTEYE:
				{
					...
					area.SetLeft(canvas->GetRenderWidth() / 2);
					area.SetBottom(0);
					area.SetRight(canvas->GetRenderWidth() - 1);
					area.SetTop(canvas->GetRenderHeight() - 1);
					break;
...
const mt::mat4& RAS_Rasterizer::GetProjectionMatrix() const
{
	return m_projmatrix;
}
...
void RAS_Rasterizer::IndexPrimitivesText(RAS_MeshSlot *ms)
{
	...
	for (unsigned short int i = 0, size = textUser->GetTexts().size(); i < size; ++i) {
		...
		RenderText3D(textUser->GetFontId(), textUser->GetTexts()[i], textUser->GetSize(), textUser->GetDpi(),
		             textUser->GetColor().Data(), mat, textUser->GetAspect(), textUser->GetUseShadows(), textUser->GetShadowOffset().Data(), textUser->GetShadowColor().Data());
	}
}
...
void RAS_Rasterizer::SetLinesWidth(float size)
{
  m_impl->SetLinesWidth(size);
}
...
void RAS_Rasterizer::InitOverrideShadersInterface()
{
	...
	// Shadow (black/VSM) override shaders.
	{
		GPUBuiltinShader shadowShaderTypes[] = {
			GPU_SHADER_BLACK, GPU_SHADER_BLACK_INSTANCING,
			GPU_SHADER_VSM_STORE, GPU_SHADER_VSM_STORE_INSTANCING
		};
		for (unsigned int i = 0; i < ARRAY_SIZE(shadowShaderTypes); ++i) {
			GPUShader *shader = GPU_shader_get_builtin_shader(shadowShaderTypes[i]);
			if (shader && !GPU_shader_get_interface(shader)) {
				OverrideShaderShadowInterface *interface =
				        (OverrideShaderShadowInterface *)MEM_mallocN(sizeof(OverrideShaderShadowInterface), "OverrideShaderShadowInterface");

				interface->viewMatLoc = GPU_shader_get_uniform(shader, "unfviewmat");
				interface->objectMatLoc = GPU_shader_get_uniform(shader, "unfobmat");
				interface->projectionMatLoc = GPU_shader_get_uniform(shader, "unfprojmat");
...
void RAS_Rasterizer::ProcessLighting(bool uselights, const mt::mat3x4& viewmat)
{
	...
	if (layer >= 0) {
		...
		PushMatrix();
		/* Calls PopMatrix on scope exit, so a future early return/continue added inside the loop
		 * below (e.g. an error check alongside light->ApplyFixedFunctionLighting) can't leave the
		 * matrix stack unbalanced. */
		RAS_PopMatrixGuard popMatrixGuard(this);
...
{
	m_impl->RenderText3D(fontid, text, size, dpi, color, mat, aspect, useShadow, shadowOffset, shadow_color);
}
...
void RAS_Rasterizer::UpdateOverrideShaderObjectMatrix(const float obmat[16])
{
	if (!ELEM(m_overrideShader, RAS_OVERRIDE_SHADER_BLACK, RAS_OVERRIDE_SHADER_BLACK_INSTANCING,
	          RAS_OVERRIDE_SHADER_SHADOW_VARIANCE, RAS_OVERRIDE_SHADER_SHADOW_VARIANCE_INSTANCING)) {
		return;
	}
	...
	OverrideShaderShadowInterface *interface = (OverrideShaderShadowInterface *)GPU_shader_get_interface(shader);
	if (!interface) {
		return;
	}

	if (interface->viewMatLoc != -1) {
		GPU_shader_uniform_vector(shader, interface->viewMatLoc, 16, 1, (const float *)m_viewmatrix.Data());
	}
	if (interface->objectMatLoc != -1) {
		GPU_shader_uniform_vector(shader, interface->objectMatLoc, 16, 1, obmat);
	}
	if (interface->projectionMatLoc != -1) {
		GPU_shader_uniform_vector(shader, interface->projectionMatLoc, 16, 1, (const float *)m_projmatrix.Data());
	}
}
...
{
	/* Always blit to a dedicated off screen instead of sampling offScreen's depth
	 * texture directly: offScreen stays bound for the rest of the frame (cutout
	 * Z-prepass, then the alpha pass that reads this texture for soft particles),
	 * so its depth texture keeps being depth-tested against -- and written to by
	 * the cutout Z-prepass -- while still attached as the active depth buffer.
	 * Sampling a texture that is simultaneously bound as the current depth
	 * attachment is undefined per the GL spec, and manifested as blocky
	 * corruption in smoke/spark VFX close to intersecting geometry. The blit
	 * (already required for multisample off screens) sidesteps that hazard for
	 * every case by giving the shader a separate, already-resolved snapshot. */
	RAS_OffScreen *dstOffScreen = canvas->GetOffScreen(RAS_OffScreen::RAS_OFFSCREEN_BLIT_DEPTH);
	offScreen->Blit(dstOffScreen, true);
	// Restore original off screen as the active render target.
	offScreen->Bind();

	GPUTexture* gputex = dstOffScreen->GetDepthTexture();
	GPU_texture_set_global_depth(gputex);

	// GPU particle screen-space collision (Fase O): cache this frame's view*projection
	// alongside the depth texture, so next frame's particle simulation (which runs before
	// render) can reproject world-space particle positions into this texture's screen space.
	const mt::mat4 viewProj = m_projmatrix * m_viewmatrix;
	GPU_texture_set_global_depth_viewproj((const float *)viewProj.Data());
}
...
void RAS_Rasterizer::PrintHardwareInfo(bool fullInfo)
{
	m_impl->PrintHardwareInfo(fullInfo);
}
...
const unsigned char *RAS_Rasterizer::GetGraphicsCardRenderer()
{
  return m_impl->GetGraphicsCardRenderer();
}
