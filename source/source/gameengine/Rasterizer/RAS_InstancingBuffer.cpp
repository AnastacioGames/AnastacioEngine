...
{
	// Buffer already big enough: reuse it as-is, offsets stay valid for m_capacity.
	if (m_vbo && size <= m_capacity) {
		return;
	}

	// Grow with slack so a steadily increasing instance count (e.g. objects being
	// spawned over time) doesn't reallocate the VBO on every single frame.
	const unsigned int capacity = size + (size / 4) + 1;

	// Offset of next memory block.
	uintptr_t offset = 0;

	// Compute memory block offsets, sized for the new capacity (not just this frame's count).

	m_matrixOffset = offset;
	offset += MATRIX_MEMORY_SIZE * capacity;

	m_positionOffset = offset;
	offset += POSITION_MEMORY_SIZE * capacity;

	if (m_attribs & COLOR_ATTRIB) {
		m_colorOffset = offset;
		offset += COLOR_MEMORY_SIZE * capacity;
	}
	if (m_attribs & LAYER_ATTRIB) {
		m_layerOffset = offset;
		offset += LAYER_MEMORY_SIZE * capacity;
	}
	if (m_attribs & INFO_ATTRIB) {
		m_infoOffset = offset;
		offset += INFO_MEMORY_SIZE * capacity;
	}
