...
func _on_ButtonGetRanGE_pressed() -> void:
	# The authoring panel can run on Windows while build-linux/bin was made by
	# WSL. Ask the script to copy every runtime it can find, rather than using
	# the panel host OS as the only destination.
	_run_script("res://release/scripts/get_rangeengine_currentplatform.py", ["--all-platforms"])
...
func _update_task_buttons() -> void:
	...
	for platform in globals.DEFAULT_PLATFORMS:
		...
		if python_valid:
			...
			# Python is an authoring-tool dependency of the current host only. A
			# packaged Linux runtime no longer carries its own Python install, so
			# requiring PythonLinux64 here would permanently keep Linux disabled
			# in a Windows Panel.
			if cur_dir.file_exists(project["Engine" + platform]):
				...
				button_export.disabled = true
				button_run.disabled = true
				cur_tooltip = "Range Engine executable must be set for this platform."
...
func _get_python_current_os() -> String:
	...
	if (result == ""):
		...
		if cur_dir.file_exists(globals.current_project_data["Python" + cur_os + "64"]):
