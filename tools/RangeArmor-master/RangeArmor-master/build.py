...
def build(target):
    ...
    toolchain = "x86_64-pc-windows-msvc" if target == "windows" else "x86_64-unknown-linux-gnu"
    ...
    if subprocess.call(args) != 0:
        raise RuntimeError("Cargo could not build launcher for " + toolchain)
    os.chdir(curDir.as_posix())
    
    print("> Copying executable to:", targetExe.as_posix())
    if not sourceExe.is_file():
        raise RuntimeError("Cargo completed without producing " + sourceExe.as_posix())
    shutil.copy2(sourceExe.as_posix(), targetExe.as_posix())
    targetExe.chmod(targetExe.stat().st_mode | 0o111)
    
    if target != "windows" and shutil.which("upx"):
        args = ["upx", "-5", targetExe.as_posix()]
        print("> Running upx on executable:", " ".join(args))
        subprocess.call(args)
    elif target != "windows":
        print("> UPX not found; keeping the Linux launcher uncompressed.")
