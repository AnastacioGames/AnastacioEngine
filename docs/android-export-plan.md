...
**Decisão 2026-09-24:** overlay HTML (Pointer Events) alimentando um pad virtual lido pela engine por `EM_JS`, com alvo gamepad (índice 0, analógico) ou tecla (origem separada do teclado físico), estendendo o Range Input System em vez de criar sistema paralelo. SDL virtual joystick não está habilitado na porta Emscripten. Plano e etapas T0–T4 em [android-touch-controls-plan.md](android-touch-controls-plan.md). **Estado 2026-09-24:** T0–T3 feitas; o layout fica na config do export Web (o APK herda), não no `android-export.json`; os critérios abaixo passam no navegador com toque emulado (`tools/web/verify-touch.cjs`) e foram aprovados no Find X3 Pro com a cena `pad` e com o First Person no layout `wasd` ([android-manual-tests.md](android-manual-tests.md)).
