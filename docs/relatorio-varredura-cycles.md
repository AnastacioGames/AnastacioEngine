...
- CYC-006: validado no build OpenCL isolado para zero argumentos e índice não numérico: `_cycles.opencl_compile()` retornou `False` e o processo encerrou normalmente. Ainda vale ampliar a matriz para cinco, sete e índice negativo quando houver suíte de testes do binding.
...
- CYC-006/CYC-007/CYC-010: build isolado `build_opencl_validate/` configurado com `WITH_CYCLES_DEVICE_OPENCL=ON`; `device_opencl.cpp`, `opencl_util.cpp` e `opencl_split.cpp` foram compilados com `-DWITH_OPENCL`, e os alvos `cycles_kernel`, `cycles_device` e `RangeEngine` linkaram com sucesso em modo serial. A RX 6800M e a Radeon integrada expõem OpenCL 2.1 e são aceitas pelo backend. No `RangeEngine` isolado, `_cycles.get_device_types()` confirmou OpenCL ativo e listou as duas GPUs; `_cycles.opencl_compile()` com zero argumentos e com índice textual retornou `False` sem encerrar o processo (CYC-006). A cópia maior que 2 GiB e a compilação externa com caracteres especiais permanecem testes funcionais pendentes.
