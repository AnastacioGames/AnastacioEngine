...
| D — Convergência (**aceite manual feito em 2026-09-18**: cubo com material, textura difusa, normal map, Sun e Point, sensor de teclado e atuador Motion rodando no navegador; falta teste automatizado no navegador e conferir o controlador Python `MODULE` com sensor no Web) | Consumir runtime da Etapa 6, manifesto e pacote mínimo | Cubo da engine + Python controller + A/D no navegador. PoC C→Python isolada não fecha este marco. |
| E — Validação Web (**parcial**: leitura do pré-voo em `preflight.py` e página `?preflight=1` do pacote, exercitada no Chrome com `web-smoke`; calibrada com import inexistente, ValueError e GLSL inválido; o relatório entra no editor pelo botão "Importar pré-voo Web" (JSON manual, preservado ao revalidar); falta rodar o pré-voo sozinho no Validar/Exportar) | Pré-voo, logs, testes de shader e execução | Falhas de contexto/arquivo/import chegam ao relatório; desktop Play identificado como desktop. |
