...
| E — Validação Web (**parcial**: leitura do pré-voo em `preflight.py` e página `?preflight=1` do pacote, exercitada no Chrome com `web-smoke`; falta ligar o relatório ao Validar/Exportar e calibrar erros de shader/Python) | Pré-voo, logs, testes de shader e execução | Falhas de contexto/arquivo/import chegam ao relatório; desktop Play identificado como desktop. |
