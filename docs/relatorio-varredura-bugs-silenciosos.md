...
> **Nota da parte 5:** achados obtidos por inspeção estática. PHYS-001 foi investigado e reclassificado como falso positivo (invariante estrutural garante tamanhos iguais; assert defensivo adicionado). PHYS-002 também foi reclassificado como falso positivo: as duas expressões de tamanho são equivalentes. AVI-001 é uma lacuna direta de propagação de I/O, já corrigida.
...
| ID | Local | Risco silencioso | Evidência e reparo/validação sugeridos |
|---|---|---|---|
| GPU-001 | `source/source/blender/gpu/intern/gpu_texture.c:80-92, 196` | **Estado: corrigido; validação de runtime interativa pendente.** `GPU_texture_convert_pixels()` agora recebe `size_t length` (e usa `size_t` internamente para `len`/índice de loop); o call site 2D passa `(size_t)w * (size_t)h`. O call site 3D (linha ~361) está dentro de `#if 0` (ver GPU-002) e não precisou de mudança. | A reauditoria de 2026-09-24 constatou que o teste antigo não era válido: `bpy.data.images.new(float_buffer=True)` não materializa dados nesta base e, com um HDR real, `gl_load()` bloqueia quando invocado cedo via `--python`; em `--background` não há contexto GL. O script agora pula explicitamente o caso, que exige sessão interativa já aberta. Cenário de overflow real (textura >2^31 pixels) continua fora de alcance de automação segura. |
...
- 26 itens estão registrados no relatório: 20 correções mecânicas e localizadas e 6 falsos positivos/código morto (PY-002, BLN-002, PHYS-001, PHYS-002, GPU-002 e BLN-004; PHYS-001 recebeu também um assert defensivo). Nenhum item permanece como dívida registrada sem decisão.
- A regressão `tools/tests/bugfix_regression/run_all.py` confirma ANIM-001, MOD-001 e RND-001. **GPU-001** fica explicitamente pendente de teste manual em sessão interativa já aberta: não há contexto GL em `--background` e chamar `gl_load()` cedo via `--python` bloqueia nesta base. **ANIM-002** exige um evento ESC dentro de operador modal; **BLN-003** não tem criador de `adt->overrides` nesta base. Esses casos permanecem validados por inspeção/compilação ou requerem uma janela real.
...
## Reauditoria de 2026-09-24

- Os 26 candidatos foram relidos. Foram confirmados como falsos positivos PHYS-001, PHYS-002, GPU-002, PY-002, BLN-002 e BLN-004. A suspeita adicional de que o cabeçalho AVI de stream ficaria inválido após o primeiro frame foi descartada: o primeiro frame o regrava corretamente.
- Quatro caminhos incompletos foram corrigidos em commits isolados: propagação de `fseek`/escrita/fechamento no AVI; liberação dos `AnimOverride` carregados; limites/overflow/alocação na reconstrução de tracking; e leitura/escrita/fechamento parciais no `datatoc`.
- O build isolado de `RangeEngine` e `RangeRuntime` passou após cada alteração. Também foram executados uma exportação AVI RAW válida, uma conversão real por `datatoc` e a regressão automatizada. GPU-001 permanece pendente somente da validação manual/interativa descrita acima.
