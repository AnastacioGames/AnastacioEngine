...
- [x] **AVI-001 — exportação AVI não propaga erro de `fwrite`/`fseek`** — **Estado: corrigido (2026-09-03).**
  ...
  - **Correção:** `awrite()` agora retorna `bool` (sucesso se `fwrite` escreveu a contagem esperada, no caminho big-endian e no direto). Os 15 call sites em `avi.c` (`AVI_open_compress`, `AVI_write_frame`, `AVI_close_compress`) acumulam o resultado em `write_ok` e retornam `AVI_ERROR_WRITING` em vez de `AVI_ERROR_NONE` quando alguma escrita falha.
  - **Validação:** build completo de `RangeEngine` sem erros/warnings novos (`avi_endian.c.obj`, `avi.c.obj`). Não foi possível forçar falha de disco às cegas nesta rodada; caminho de sucesso normal (export AVI) segue idêntico, mudança é aditiva.
...
| BLN-003 | `blenloader/intern/readfile.c:2814-2818` e `writefile.c:910-918` | **Estado: corrigido (2026-09-03).** | `direct_link_animdata()` tinha `// TODO...` sem lógica onde deveria ler `adt->overrides` de volta, enquanto `write_animdata()` já gravava cada `AnimOverride`. Implementado `link_list(fd, &adt->overrides)` + `aor->rna_path = newdataadr(fd, aor->rna_path)` para cada override, no mesmo padrão já usado duas linhas abaixo para `nla_tracks`. `AnimOverride` (`value`, `array_index`) não tem outros ponteiros a resolver. Build de `RangeEngine`/`bf_blenloader` sem erros. | Não testável hoje, nem manualmente: buscado em todo `anim_sys.c`/`readfile.c`/`writefile.c` e confirmado que nada nesta base de código jamais escreve em `adt->overrides` (recurso do Blender parcialmente implementado desde 2009, `animsys_evaluate_overrides()` só lê uma lista que nunca é populada). O fix está correto por inspeção (espelha exatamente `write_animdata`), mas fica sem cenário de reprodução possível até que algo passe a criar `AnimOverride`. |
...
- [x] **MEDIA-001 — intervalo de frames da reconstrução é multiplicado sem validação** — **Estado: corrigido (2026-09-03).** `source/source/blender/blenkernel/intern/tracking_solver.c:190-194`. A rotina aloca `efra - sfra + 1` câmeras diretamente e depois percorre o mesmo intervalo; não há checagem local de intervalo negativo, overflow da expressão ou limite de memória antes de `MEM_callocN`. **Correção:** adicionada guarda `if (efra < sfra) return ok;` antes da alocação, e o cálculo do tamanho passou a usar `size_t` explícito. Build de `RangeEngine` sem erros; reprodução com intervalo invertido/extremo real não realizada nesta rodada.
...
- **DATATOC-001 — tamanho de entrada pode ficar inválido após falha de seek/tamanho** — **Estado: corrigido (2026-09-03).** `source/source/blender/datatoc/datatoc.c:65-67`. `fseek` e `ftell` eram usados sem validar retorno antes de calcular/usar `size`. **Correção:** `fseek(SEEK_END)` e `ftell` agora são validados juntos (`ftell < 0` tratado como erro), abortando com `exit(1)` e mensagem antes de prosseguir. Build e link de `datatoc.exe` sem erros; entrada não-seekável real não testada nesta rodada.
