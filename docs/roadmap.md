...
- **Export para Web (WebGL/WebAssembly)**:
  ...
  **Export com validação testado no navegador (2026-09-18):** `bom_cubo.blend` (cubo com textura difusa, normal map,
  ...
  Cook-Torrance com Roughness 0). Abertos: normal map `.dds` conferido no pacote 8211 (cena do `startup.blend`, `tools/create_web_normalmap_scene.py`) e aceito pelo usuário em 2026-09-20 como igual ao desktop; aviso
  `glBlitFramebuffer` depth/stencil que já aparece uma vez no console; o pré-voo do marco E roda sozinho depois do Exportar Web (opção "Pré-voo após exportar", Chrome/Edge sem janela, cerca de 15 s) e no botão "Testar pacote no navegador"; "Importar pré-voo Web" segue para JSON manual, e o Validar preserva os resultados de pré-voo (só o Exportar/Testar os refazem; o Validar sozinho não abre navegador porque não há pacote) e a extração de erros de shader/Python é heurística sobre o texto do runtime (calibrada com um import inexistente, um ValueError e um GLSL inválido; estágio/material do shader não são informados); marco G: pacotes de teste manual 8201–8206 (áudio, capacidades, save, smoke, render avançado, dispositivos; v0.1.1) aceitos pelo usuário em 2026-09-19 (áudio via Audaspace+SDL, WAV, MP3 e OGG (dr_mp3 e stb_vorbis chamados pelo leitor WAV, pacotes 8208 e 8209, músicas do usuário aceitas); módulo Python `aud` (Sound/Device/Handle, pacote 8210, sem `Sound.data()`/`buffer()` por falta de numpy); sem OpenAL/efeitos; save: SAVED na 1ª abertura, LOADED após F5, o "could not open /saves/ci.save" da 1ª abertura é esperado); transição de cenas aceita no pacote 8207 (`tools/create_web_scene_transition_scene.py`: `scene.replace` A↔B e overlay HUD ligado/desligado), o que fecha os itens de teste manual do marco G. Detalhes no [changelog](changelog.md).
  **Marco G, próximos passos (2026-09-20):** áudio Web completo (WAV/MP3/OGG e módulo `aud`, pacotes 8201–8211 aceitos). Restam: deploy real fora do `localhost` (CORS, cabeçalhos do `.wasm`, download de ~46 MB; ver [web-deploy.md](web-deploy.md)), efeitos OpenAL/áudio 3D (só se algum jogo precisar) e a origem do aviso `glBlitFramebuffer` (sem reprodução). Pacotes de teste locais: `build-web/dist/iniciar-testes.bat` (portas 8201–8211).
