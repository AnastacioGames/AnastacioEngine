...
- **APK WebView mínimo (A0b)**: template em `tools/android/webview-template/` rodando a cena `motion` no
  ...
  (pointer lock do WebView neutralizado; ~60 fps parado, medidas variando). Próximo: confirmar
  música e toque, save/IDBFS, comparação com o Chrome do aparelho com mais medidas.
