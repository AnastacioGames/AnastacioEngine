...
- **Export para Web (WebGL/WebAssembly)**:
  ...
  **Export com validação testado no navegador (2026-09-18):** `bom_cubo.blend` (cubo com textura difusa, normal map,
  ...
  `glBlitFramebuffer` depth/stencil que já aparece uma vez no console; o pré-voo do marco E roda sozinho depois do Exportar Web (opção "Pré-voo após exportar", Chrome/Edge sem janela, cerca de 15 s) e no botão "Testar pacote no navegador"; "Importar pré-voo Web" segue para JSON manual, e o Validar preserva os resultados de pré-voo (só o Exportar/Testar os refazem; o Validar sozinho não abre navegador porque não há pacote) e a extração de erros de shader/Python é heurística sobre o texto do runtime (calibrada com um import inexistente, um ValueError e um GLSL inválido; estágio/material do shader não são informados); marco G: áudio Web feito (WAV via Audaspace+SDL, sem OGG/MP3/OpenAL/`aud`; provado por sonda Web Audio, falta aceite auditivo), render avançado aceito visualmente pelo usuário em 2026-09-19 (xadrez, iluminação, especular, névoa e sombras no pacote `web-render`); save aceito (8203: SAVED na 1ª abertura, LOADED após F5; o "could not open /saves/ci.save" da 1ª abertura é esperado); demais itens (transições, dispositivos, áudio, capacidades) com pacotes de teste manual (v0.1.1, runtime corrigido) aguardando aceite. Detalhes no [changelog](changelog.md).
